...
| SYNC-4: ReLU `-O0` baseline certification | Mandatory pre-ReLU refresh and polynomial approximation | Freeze the approved degree, interval, coefficients, evaluation scheme, and error authority. Then `bootstrap=auto|on` inserts one required pre-ReLU boundary per surviving `common.relu`; `manual` requires explicit compatible boundaries; `off` rejects surviving ReLU; no `-O0` movement, merging, deduplication, or profitability placement occurs. |
...
| SYNC-4 | Preserve and print `common.relu` source, result, descriptor, approximation, and provenance evidence through conversion. | Obtain explicit polynomial-policy approval, then materialize mandatory pre-ReLU refresh followed by the approved polynomial; enforce `auto|on`, `manual`, and `off` behavior. | Artifacts prove approved coefficients/range/error and no `-O0` boundary movement, merging, deduplication, or profitability placement. |
