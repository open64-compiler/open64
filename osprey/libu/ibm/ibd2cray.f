...
C
      ...
      INTEGER FUNCTION
     +IBD2CRAY@(TYPE, NUM, FORN, BITOFF, CRAY, STRIDE, CRAYCH)

C     The IBD2CRAY@ function converts IBM 32-bit data types to CRAY Fortran 
C     data types.  The REAL and DOUBLE PRECISION data representations
C     are both 64 bits.   This function is a close variant of IBM2CRAY.
C
...
C             overflowed or completely under-flowed.
      ...
      IBD2CRAY@ = IBM2CRAY(NTYPE, NUM, FORN, BITOFF, CRAY, INC, CRAYCH)
      RETURN

 9000 CONTINUE
      IBD2CRAY@ = -1                !  Some parameter error
      RETURN
CDIR$ ID "@(#) libu/ibm/ibd2cray.f	92.0	10/08/98 14:57:41"
      END
