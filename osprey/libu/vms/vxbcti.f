...
C
      ...
      SUBROUTINE VXBCTI(FPN,DEST,ISB,NUM,IER,INC)
C
C     The VXBCTI subroutine converts Cray double precision floating-
C     point numbers to VAX D_format floating-point numbers.  Numbers
C     that produce an underflow when converted to VAX format are
C     converted to 32 binary zeros.  Numbers that are in overflow on
C     the CRAY are converted to a "reserved" floating-point
C     representation with the sign bit set if negative.  Numbers
C     that are valid on the CRAY but overflow on the VAX are
C     to the most positive possible number or most negative possible
C     number, depending on the sign.
C
...
C
      DOUBLE PRECISION FPN(1)     ! Variable or array of any length and
C                                 ! type real, containing Cray 64-bit,
C                                 ! double-precision, floating-point
C                                 ! numbers to convert.
C
      INTEGER DEST(1)             ! Variable or array of any type (well,
C                                 ! complex won't work too well) to
...
C                                 ! Value is 0 if all Cray values    
C                                 ! convert to VAX values without
...
CDIR$ SHORTLOOP
   ...
   80 CONTINUE
      ...
      RETURN
CDIR$ ID "@(#) libu/vms/vxbcti.f	92.0	10/08/98 14:57:41"
      END
