...
C
      ...
      SUBROUTINE VXBCTC(DPN,ISB,DEST,NUM,INC)
C
C     The VXBCTC subroutine converts VAX 64-bit D_format numbers
C     into CRAY double-precision numbers.
C
      INTEGER DPN(1)              ! Variable or array of any 
C                                 ! non-character type and of any 
C                                 ! length containing VAX D_format
...
C
      DOUBLE PRECISION DEST(1)    ! Variable or array of type real to
...
C     The stride is doubled since DEST is DOUBLE PRECISION

      CALL VXUNPACK(DPN,ISB,DEST,NUM,64,2*JNC)

      IOUT = 1

      DO 60 I = 1, NUM
        DEST(IOUT) = VD64I(DEST(IOUT))
        IOUT       = IOUT + JNC
   60 CONTINUE

      RETURN
CDIR$ ID "@(#) libu/vms/vxbctc.f	92.0	10/08/98 14:57:41"
      END
