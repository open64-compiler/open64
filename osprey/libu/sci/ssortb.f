...
C
      ...
      subroutine ssortb( ad, n, x, incx, id, incd )
      implicit none
...
*     ... Begin Execution ...
*
CDIR$ ID "@(#) libu/sci/ssortb.f	92.1	10/20/99 11:17:46"
*

*       This is used to determine if the number of arguments connotes
*       in-place or indexed sorting.
        iarg = numarg()
...
*       Test input parameters
        ...
        elseif( iarg .eq. 6) then
          if( incd .le. 0 ) info = 6
        elseif( iarg.ne.6 .and. iarg.ne.4 ) then
          info = 7
        endif

        if( info .ne. 0 ) then
          call xersor( 'SSORTB', info )
          goto 1500
        endif
...
*
*         This performs the steps per discussion in Knuth on pg.
*         113.   The first set of steps is to split the array
*         into say chunks of 8.  Within each chunk, each of the 
*         first 4 elements are compared with the corresponding
*         element from the second half of the chunk.  Then you
*         move to the next chunk of 8 and the next...  This 
*         consitutes a doubly nested DO-loop which vectors on
*         the inner loop and the outer one can be parallelized.
*         
          do 300 ll = ldn1, 0, -1
            iord = 2**ll
