...
/*
 * VHO builds the AIO-12 runtime-only variant and guard contract. The first
 * slice wraps the selected AIO-11 cuBLASLt matmul implementation with two
 * operand alignment guards and retains the direct implementation as an
 * unconditional fallback. Common/com only materializes RuntimeVariantIR. See
...
void
VHO_DSL_Runtime_Variant_Control_Init
        (VHO_DSL_RUNTIME_VARIANT_CONTROL *control)
{
    if (control == NULL)
        return;
...
VHO_DSL_Runtime_Variant_Create
        (struct pu_info *pu, const DSL_TENSOR_EVOLUTION_GRAPH *graph,
         const DSL_PHYSICAL_PLAN_ANALYSIS *physical,
         const VHO_DSL_RUNTIME_VARIANT_CONTROL *control, FILE *diagnostic)
{
...
void
VHO_DSL_Runtime_Variant_Destroy (DSL_RUNTIME_VARIANT_ANALYSIS *analysis)
{
    if (analysis == NULL)
        return;
    for (UINT32 i = 0; i < analysis->plan_contexts.size(); ++i)
        DSL_opt_plan_destroy(analysis->plan_contexts[i]);
    DSL_runtime_variant_ir_destroy(analysis->ir);
    delete analysis;
}

static BOOL
DSL_Runtime_Materialize_IR
        (DSL_RUNTIME_VARIANT_ANALYSIS *analysis, FILE *diagnostic)
{
    DSL_RUNTIME_VARIANT_IR_CREATE_INFO info;
    ...
    info.variants = analysis->variants.empty() ? NULL : &analysis->variants[0];
    info.variant_count = analysis->variants.size();
    info.guards = analysis->guards.empty() ? NULL : &analysis->guards[0];
    info.guard_count = analysis->guards.size();
    analysis->ir = DSL_runtime_variant_ir_create(&info, diagnostic);
    return analysis->ir != NULL;
}

BOOL
VHO_DSL_Runtime_Variant_Build
        (DSL_RUNTIME_VARIANT_ANALYSIS *analysis, FILE *diagnostic)
{
    ...
    if (!analysis->control.generate_variants ||
        ...
        analysis->built = TRUE;
        return DSL_Runtime_Materialize_IR(analysis, diagnostic) &&
               VHO_DSL_Runtime_Variant_Verify(analysis, diagnostic);
    }
...
BOOL
VHO_DSL_Runtime_Variant_Verify
        (const DSL_RUNTIME_VARIANT_ANALYSIS *analysis, FILE *diagnostic)
{
    if (!DSL_Runtime_Variant_Active(analysis) || !analysis->built ||
        analysis->ir == NULL ||
        !DSL_Runtime_Control_Valid(analysis->control) ||
        !DSL_runtime_variant_ir_verify(analysis->ir, diagnostic) ||
        DSL_runtime_variant_ir_owner(analysis->ir) !=
            analysis->owner_pu_st ||
        DSL_runtime_variant_ir_site_count(analysis->ir) !=
            analysis->sites.size() ||
        DSL_runtime_variant_ir_variant_count(analysis->ir) !=
    ...
    for (UINT32 i = 0; i < analysis->sites.size(); ++i) {
        ...
        DSL_RUNTIME_VARIANT_SITE_RECORD ir_site;
        DSL_PHYSICAL_SITE_RECORD physical_site;
        ...
        for (UINT32 j = 0; j < site.variant_count; ++j) {
            ...
            DSL_RUNTIME_VARIANT_RECORD ir_variant;
            ...
            for (UINT32 k = 0; k < variant.guard_count; ++k) {
                ...
                DSL_RUNTIME_GUARD_RECORD ir_guard;
                if (guard.id != expected_guard ||
                    !DSL_runtime_variant_ir_get_guard
                         (analysis->ir, guard.id, &ir_guard) ||
                    memcmp(&guard, &ir_guard, sizeof(guard)) != 0 ||
...
DSL_Runtime_Find_Observation
        (const VHO_DSL_RUNTIME_GUARD_OBSERVATION *observations,
         UINT32 observation_count, UINT32 operand_ordinal,
         const VHO_DSL_RUNTIME_GUARD_OBSERVATION **observation,
         FILE *diagnostic)
{
    *observation = NULL;
...
VHO_DSL_Runtime_Variant_Evaluate
        (const DSL_RUNTIME_VARIANT_ANALYSIS *analysis,
         ...
         VHO_DSL_RUNTIME_VARIANT_EVALUATION_RESULT *result, FILE *diagnostic)
{
    if (result != NULL)
        memset(result, 0, sizeof(*result));
    if (!VHO_DSL_Runtime_Variant_Verify(analysis, diagnostic) || result == NULL ||
...
void
VHO_DSL_Runtime_Variant_Print
        (FILE *file, const DSL_RUNTIME_VARIANT_ANALYSIS *analysis)
{
    if (file == NULL || !DSL_Runtime_Variant_Active(analysis))
        return;
    DSL_runtime_variant_ir_print(file, analysis->ir);
    fprintf(file,
            "VHORuntimeVariantDecision: owner=0x%x target=%s stage=G15 "
...
VHO_DSL_Runtime_Variant_Print_Evaluation
        (FILE *file, const char *label,
         const VHO_DSL_RUNTIME_VARIANT_EVALUATION_RESULT *result)
{
...
const DSL_RUNTIME_VARIANT_IR *
VHO_DSL_Runtime_Variant_Get_IR
        (const DSL_RUNTIME_VARIANT_ANALYSIS *analysis)
{
