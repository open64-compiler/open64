...
C
      ...
      subroutine MINLOC_JP6@(fresult,source,mask,maskflg,ext)
*
CDIR$ ID "@(#) libfi/mpp_reduce/minloc_p_6@.f	92.0	10/08/98 14:37:14"
************************************************************************
*
...
*          6. It returns an array result. This routine is called by
*          the C routine _MINLOC_JP6.
*
...
************************************************************************
      ...
      integer result(0:n$pes-1,6), maskflg, ext(6), local_idx(6)
      integer fresult(8)
      integer mod_factor, offset, mype
      integer tripcnt, far_idx(6), min_value(0:n$pes-1), in_parallel
      logical mask(ext(1),ext(2),ext(3),ext(4),ext(5),ext(6))
      integer source(ext(1),ext(2),ext(3),ext(4),ext(5),ext(6))
...
*
      ...
      subroutine MINLOC_SP6@(fresult,source,mask,maskflg,ext)
************************************************************************
*
* Purpose: Determine the location of the first element of source
*          having the minimum value of the elements identified by
...
************************************************************************
      ...
      integer fresult(8)
      integer mod_factor, offset, mype
      integer tripcnt, far_idx(6), in_parallel
      logical mask(ext(1),ext(2),ext(3),ext(4),ext(5),ext(6))
      real source(ext(1),ext(2),ext(3),ext(4),ext(5),ext(6))
      real local_min, far_min, min_value(0:n$pes-1)
      real HUGE_FLT
*
cdir$ unknown_shared source, mask
cdir$ shared result(:block(1),:), min_value(:block(1))
...
*
      ...
      subroutine MINLOC_NM_JP6@(fresult,source,ext)
************************************************************************
*
* Purpose: Determine the location of the first element of source
...
************************************************************************
      intrinsic my_pe, in_parallel
      integer result(0:n$pes-1,6), ext(6), local_idx(6)
      integer fresult(8)
      integer mod_factor, offset, mype
      integer tripcnt, far_idx(6), min_value(0:n$pes-1), in_parallel
      integer source(ext(1),ext(2),ext(3),ext(4),ext(5),ext(6))
...
*
      local_min = HUGE_INT()
      include "minloc_nomask_6@.fh"
*
      return
      end


      subroutine MINLOC_NM_SP6@(fresult,source,ext)
************************************************************************
*
* Purpose: Determine the location of the first element of source
*          having the minimum value of the elements in the array.
