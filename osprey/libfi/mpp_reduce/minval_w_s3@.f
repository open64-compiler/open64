...
C
      ...
      subroutine MINVAL_W_JS3@(result,source,dim,mask,maskflg,rext,
     +                   sext,blkcnts,npes,hi1,low1,hi2,low2,lmext,
     +                   dist_cnt)
CDIR$ ID "@(#) libfi/mpp_reduce/minval_w_s3@.f	92.0	10/08/98 14:37:14"
************************************************************************
*
...
*            the Fortran routine MINVAL_I_JS3@.
*
...
************************************************************************
      ...
      integer result(rext(1),rext(2)), local_min(lmext(1),lmext(2))
...
*
      do j = 1, lmext(2)
          do i = 1, lmext(1)
              local_min(i,j) = HUGE_INT()
          enddo
      enddo
*
      include "minval_w_3@.fh"
*
      return
      end


      subroutine MINVAL_W_SS3@(result,source,dim,mask,maskflg,rext,
     +                   sext,blkcnts,npes,hi1,low1,hi2,low2,lmext,
     +                   dist_cnt)
************************************************************************
*
...
*            mask. This routine handles integer source arrays of rank
*            3. It returns an array result. This routine is called by
*            the Fortran routine MINVAL_I_SS3@.
*
...
************************************************************************
      ...
      real result(rext(1),rext(2)), local_min(lmext(1),lmext(2))
      pointer (res_ptr,local_min)
*
cdir$ serial_only
cdir$ unknown_shared source, mask
