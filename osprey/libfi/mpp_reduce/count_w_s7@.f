...
C
      ...
      subroutine COUNT_W_LS7@(result,source,dim,rext,sext,blkcnts,
     +                      npes,hi1,low1,hi2,low2,hi3,low3,hi4,
     +                      low4,hi5,low5,hi6,low6,lmext,dist_cnt)
CDIR$ ID "@(#) libfi/mpp_reduce/count_w_s7@.f	92.0	10/08/98 14:37:14"
************************************************************************
*
...
*            called by the Fortran routine COUNT_I_LS7@.
*
...
*                      extents of the local_cnt array if called within
...
*                      dimension dim.
*
* Variables: local_cnt - integer array of rank 6: array of local count
*                      of true elements.
...
*                      (blkcnt < npes) or offset into local_cnt when
*                      (blkcnt > npes).
...
************************************************************************
      ...
      integer blkcnts(6), length, npes, mype, dim
     ...
     +               sext(6),sext(7))
      ...
      integer local_cnt(lmext(1),lmext(2),lmext(3),lmext(4),
     +                  lmext(5),lmext(6))
      pointer (res_ptr,local_cnt)
*
cdir$ serial_only
cdir$ unknown_shared source
*
*     Since execution is taking place in a serial region of code,
*     equivalence the local_cnt array to the result array.
*
      res_ptr = loc(result)
*
