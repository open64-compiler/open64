...
C
      ...
      subroutine MAXVAL_I_JS6@(result,source,dim,mask,maskflg,rext,
     +                       sext,blkcnts,dim_bcnt)
CDIR$ ID "@(#) libfi/mpp_reduce/maxval_i_s6@.f	92.0	10/08/98 14:37:14"
************************************************************************
*
...
************************************************************************
      ...
      integer blkcnts(6), npes, mype
      integer maskflg, dim, dim_bcnt(5), dist_cnt
      integer sext(6), loc_sext(6), rext(5), lmext(5)
      logical in_parallel
...
*
      include "setup_6@.fh"
*
      call MAXVAL_W_JS6@(result,source,dim,mask,maskflg,rext,loc_sext,
     +                 dim_bcnt,npes,hi_r1,low_r1,hi_r2,low_r2,hi_r3,
     +                 low_r3,hi_r4,low_r4,hi_r5,low_r5,lmext,dist_cnt)
*
      return
      end


      subroutine MAXVAL_I_SS6@(result,source,dim,mask,maskflg,rext,
     +                       sext,blkcnts,dim_bcnt)
************************************************************************
*
* Purpose:   Find the maximum value of the elements identified by
...
*
      include "setup_6@.fh"
*
      call MAXVAL_W_SS6@(result,source,dim,mask,maskflg,rext,loc_sext,
     +                 dim_bcnt,npes,hi_r1,low_r1,hi_r2,low_r2,hi_r3,
     +                 low_r3,hi_r4,low_r4,hi_r5,low_r5,lmext,dist_cnt)
*
      return
