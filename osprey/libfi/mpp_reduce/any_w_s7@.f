...
C
      ...
      subroutine ANY_W_LS7@(result,source,dim,rext,sext,blkcnts,
     +                      npes,hi1,low1,hi2,low2,hi3,low3,hi4,
     +                      low4,hi5,low5,hi6,low6,lmext,dist_cnt)
CDIR$ ID "@(#) libfi/mpp_reduce/any_w_s7@.f	92.0	10/08/98 14:37:14"
************************************************************************
*
...
*            called by the Fortran routine ANY_I_LS7@.
*
* Arguments: result  - logical array of rank 6.
...
*                      dimension dim.
*
* Variables: local_any - logical array of rank 6: array of local any values.
...
************************************************************************
     ...
     +               sext(6),sext(7))
      logical result(rext(1),rext(2),rext(3),rext(4),rext(5),rext(6))
      logical local_any(lmext(1),lmext(2),lmext(3),lmext(4),
     +                  lmext(5),lmext(6))
      pointer (res_ptr,local_any)
*
cdir$ serial_only
cdir$ unknown_shared source
*
*     Since execution is taking place in a serial region of code,
*     equivalence the local_any array to the result array.
*
      res_ptr = loc(result)
*
...
*
      ...
      do i2 = 1, lmext(2)
      do i1 = 1, lmext(1)
          local_any(i1,i2,i3,i4,i5,i6) = .false.
      enddo
      enddo
      enddo
      enddo
