...
C
      ...
      subroutine MAXVAL_W_JS3@(result,source,dim,mask,maskflg,rext,
     +                   sext,blkcnts,npes,hi1,low1,hi2,low2,lmext,
     +                   dist_cnt)
CDIR$ ID "@(#) libfi/mpp_reduce/maxval_w_s3@.f	92.0	10/08/98 14:37:14"
************************************************************************
*
...
*            the Fortran routine MAXVAL_I_JS3@.
*
...
*                      dimension dim.
*
* Variables: local_max - integer array of rank 2: array of local maximums.
...
************************************************************************
      ...
      integer result(rext(1),rext(2)), local_max(lmext(1),lmext(2))
...
*
      do j = 1, lmext(2)
          do i = 1, lmext(1)
              local_max(i,j) = -HUGE_INT()
          enddo
      enddo
*
      include "maxval_w_3@.fh"
*
      return
      end


      subroutine MAXVAL_W_SS3@(result,source,dim,mask,maskflg,rext,
     +                   sext,blkcnts,npes,hi1,low1,hi2,low2,lmext,
     +                   dist_cnt)
************************************************************************
*
...
*            mask. This routine handles integer source arrays of rank
*            3. It returns an array result. This routine is called by
*            the Fortran routine MAXVAL_I_SS3@.
*
...
************************************************************************
      ...
      real result(rext(1),rext(2)), local_max(lmext(1),lmext(2))
      pointer (res_ptr,local_max)
*
cdir$ serial_only
cdir$ unknown_shared source, mask
