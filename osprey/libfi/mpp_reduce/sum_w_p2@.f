...
C
      ...
      subroutine SUM_W_JP2@(result,source,dim,mask,maskflg,rext,
     +                   sext,blkcnt,npes,hi,low,lmext,dist_cnt)
CDIR$ ID "@(#) libfi/mpp_reduce/sum_w_p2@.f	92.0	10/08/98 14:37:14"
************************************************************************
*
...
*            by the Fortran routine SUM_I_JP2@.
*
...
*                      extent of the local-sum array if called within
...
*                      dimension dim.
*
* Variables: local_sum - integer array of rank 1: array of local sums.
...
************************************************************************
      ...
      integer result(rext), local_sum(lmext)
*
cdir$ parallel_only
...
*
      ...
      subroutine SUM_W_SP2@(result,source,dim,mask,maskflg,rext,
     +                   sext,blkcnt,npes,hi,low,lmext,dist_cnt)
************************************************************************
*
* Purpose:   Compute the sum of the elements identified by mask.
...
*            2. It returns an array result. This routine is called
*            by the Fortran routine SUM_I_SP2@.
*
...
************************************************************************
      ...
      real result(rext), local_sum(lmext)
*
cdir$ parallel_only
cdir$ shared result(:block)
*
