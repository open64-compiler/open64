...
C
      ...
      subroutine PROD_W_JP2@(result,source,dim,mask,maskflg,rext,
     +                   sext,blkcnt,npes,hi,low,lmext,dist_cnt)
CDIR$ ID "@(#) libfi/mpp_reduce/prod_w_p2@.f	92.0	10/08/98 14:37:14"
************************************************************************
*
* Purpose:   Compute the product of the elements identified by mask.
*            This routine handles integer source arrays of rank
*            2. It returns an array result. This routine is called
*            by the Fortran routine PROD_I_JP2@.
*
...
*                      extent of the local product array if called within
...
*                      dimension dim.
*
* Variables: local_prod - integer array of rank 1: array of local products.
...
************************************************************************
      ...
      integer result(rext), local_prod(lmext)
*
...
*
      ...
      subroutine PROD_W_SP2@(result,source,dim,mask,maskflg,rext,
     +                   sext,blkcnt,npes,hi,low,lmext,dist_cnt)
************************************************************************
*
* Purpose:   Compute the product of the elements identified by mask.
...
*            2. It returns an array result. This routine is called
*            by the Fortran routine PROD_I_SP2@.
*
...
************************************************************************
      ...
      real result(rext), local_prod(lmext)
*
cdir$ shared result(:block)
*
*     Initialize the local and global result arrays
