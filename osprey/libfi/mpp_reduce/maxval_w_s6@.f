...
C
      ...
      subroutine MAXVAL_W_JS6@(result,source,dim,mask,maskflg,rext,
     +                   sext,blkcnts,npes,hi1,low1,hi2,low2,hi3,
     +                   low3,hi4,low4,hi5,low5,lmext,dist_cnt)
CDIR$ ID "@(#) libfi/mpp_reduce/maxval_w_s6@.f	92.0	10/08/98 14:37:14"
************************************************************************
*
...
*            the Fortran routine MAXVAL_I_JS6@.
*
...
*                      dimension dim.
*
* Variables: local_max - integer array of rank 5: array of local maximums.
...
*
      ...
      do i2 = 1, lmext(2)
      do i1 = 1, lmext(1)
          local_max(i1,i2,i3,i4,i5) = -HUGE_INT()
      enddo
      enddo
      enddo
      enddo
      enddo
*
      include "maxval_w_6@.fh"
*
      return
      end


      subroutine MAXVAL_W_SS6@(result,source,dim,mask,maskflg,rext,
     +                   sext,blkcnts,npes,hi1,low1,hi2,low2,hi3,
     +                   low3,hi4,low4,hi5,low5,lmext,dist_cnt)
************************************************************************
*
...
*            mask. This routine handles integer source arrays of rank
*            6. It returns an array result. This routine is called by
*            the Fortran routine MAXVAL_I_SS6@.
*
...
*
      ...
      do i2 = 1, lmext(2)
      do i1 = 1, lmext(1)
          local_max(i1,i2,i3,i4,i5) = -HUGE_FLT()
      enddo
      enddo
      enddo
      enddo
