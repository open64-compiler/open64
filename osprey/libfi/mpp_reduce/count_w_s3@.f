...
C
      ...
      subroutine COUNT_W_LS3@(result,source,dim,rext,sext,blkcnts,
     +                      npes,hi1,low1,hi2,low2,lmext,dist_cnt)
CDIR$ ID "@(#) libfi/mpp_reduce/count_w_s3@.f	92.0	10/08/98 14:37:14"
************************************************************************
*
* Purpose:   Count the number of true values in source along
*            dimension dim. This routine handles source arrays of
*            rank 3. It returns an array result. This routine is
*            called by the Fortran routine COUNT_I_LS3@.
*
...
*                      dimension dim.
*
* Variables: local_cnt - integer array of rank 2: array of local count
*                      of true elements.
...
************************************************************************
      ...
      integer result(rext(1),rext(2)), local_cnt(lmext(1),lmext(2))
