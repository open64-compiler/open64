...
C
      ...
      subroutine MAXVAL_W_JP4@(result,source,dim,mask,maskflg,rext,
     +                   sext,blkcnts,npes,hi1,low1,hi2,low2,hi3,
     +                   low3,lmext,dist_cnt)
CDIR$ ID "@(#) libfi/mpp_reduce/maxval_w_p4@.f	92.0	10/08/98 14:37:14"
************************************************************************
*
...
*            the Fortran routine MAXVAL_I_JP4@.
*
...
*                      dimension dim.
*
* Variables: local_max - integer array of rank 3: array of local maximums.
...
************************************************************************
      ...
      integer local_max(lmext(1),lmext(2),lmext(3))
*
cdir$ parallel_only
...
*
cdir$ doshared (k,j,i) on result(i,j,k)
      do k = 1, rext(3)
      do j = 1, rext(2)
      do i = 1, rext(1)
          result(i,j,k) = -HUGE_INT()
      enddo
      enddo
      enddo
      ...
      do j = 1, lmext(2)
      do i = 1, lmext(1)
          local_max(i,j,k) = -HUGE_INT()
      enddo
      enddo
      enddo
*
      include "maxval_w_4@.fh"
*
      return
      end


      subroutine MAXVAL_W_SP4@(result,source,dim,mask,maskflg,rext,
     +                   sext,blkcnts,npes,hi1,low1,hi2,low2,hi3,
     +                   low3,lmext,dist_cnt)
************************************************************************
*
...
*            mask. This routine handles integer source arrays of rank
*            4. It returns an array result. This routine is called by
*            the Fortran routine MAXVAL_I_SP4@.
*
...
************************************************************************
      ...
      real local_max(lmext(1),lmext(2),lmext(3))
*
cdir$ parallel_only
cdir$ shared result(:block,:block,:block)
*
