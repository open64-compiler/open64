...
C
      ...
      subroutine MAXVAL_W_JS5@(result,source,dim,mask,maskflg,rext,
     +                   sext,blkcnts,npes,hi1,low1,hi2,low2,hi3,
     +                   low3,hi4,low4,lmext,dist_cnt)
CDIR$ ID "@(#) libfi/mpp_reduce/maxval_w_s5@.f	92.0	10/08/98 14:37:14"
************************************************************************
*
...
*            the Fortran routine MAXVAL_I_JS5@.
*
...
*                      dimension dim.
*
* Variables: local_max - integer array of rank 4: array of local maximums.
...
************************************************************************
      ...
      integer local_max(lmext(1),lmext(2),lmext(3),lmext(4))
...
*
      ...
      do i2 = 1, lmext(2)
      do i1 = 1, lmext(1)
          local_max(i1,i2,i3,i4) = -HUGE_INT()
      enddo
      enddo
      enddo
      enddo
*
      include "maxval_w_5@.fh"
*
      return
      end


      subroutine MAXVAL_W_SS5@(result,source,dim,mask,maskflg,rext,
     +                   sext,blkcnts,npes,hi1,low1,hi2,low2,hi3,
     +                   low3,hi4,low4,lmext,dist_cnt)
************************************************************************
*
...
*            mask. This routine handles integer source arrays of rank
*            5. It returns an array result. This routine is called by
*            the Fortran routine MAXVAL_I_SS5@.
*
...
************************************************************************
      ...
      real local_max(lmext(1),lmext(2),lmext(3),lmext(4))
      pointer (res_ptr,local_max)
*
cdir$ serial_only
cdir$ unknown_shared source, mask
...
*
      ...
      do i2 = 1, lmext(2)
      do i1 = 1, lmext(1)
          local_max(i1,i2,i3,i4) = -HUGE_FLT()
      enddo
      enddo
      enddo
      enddo
