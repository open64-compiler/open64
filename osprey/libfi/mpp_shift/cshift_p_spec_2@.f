...
C
	...
	subroutine cshift_spec_wd1_p2@(result, source, dim, sext, shftval)
*
CDIR$ ID "@(#) libfi/mpp_shift/cshift_p_spec_2@.f	92.0	"
	...
	if (blkct (source,1,mype) .gt. 0 .AND.
     &	    blkct (source,2,mype) .gt. 0) then
		...
		if (low(1) .gt. 1) then
		    ...
		    pe_low = home (source(sext(1),low(2)))
		endif
		if (hi(1) .lt. sext(1)) then
		    pe_hi = home (source(hi(1)+1,low(2)))
		else
		    pe_hi = home (source(1,low(2)))
		endif
	    else
		if (low(2) .gt. 1) then
		    pe_low = home (source(low(1),low(2)-1))
		else
		    pe_low = home (source(low(1),sext(2)))
		endif
		if (hi(2) .lt. sext(2)) then
		    pe_hi = home (source(low(1),hi(2)+1))
		else
		    pe_hi = home (source(low(1),1))
		endif
	    endif
*
	    call cshift_i_spec_wd1_p2@ (result, source, dim, sext, shftval,
...
*
	...
	subroutine cshift_i_spec_wd1_p2@ (result, source, dim, sext, shftval, 
     &		blksize, pe_low, pe_hi)
*
**************************************************************************
*
...
*
	call cshift_spec_w_wd1_p2@ (result(1,1), source(1,1), dim, blksize,
...
*
	...
	subroutine cshift_spec_w_wd1_p2@ (result, source, dim, sext, shftval,
     &		pe_low, pe_hi)
*
**************************************************************************
*
...
*
	...
	totsx2 = sext(2)
	str1 = 1
	include "cshift_spec_2@.fh"
*
	return
	end


	subroutine cshift_spec_wd2_p2@(result, source, dim, sext, shftval)
*
**************************************************************************
*
* Purpose:	Perform a circular shift of an array in argument SOURCE
...
*
	    call cshift_i_spec_wd2_p2@ (result, source, dim, sext, shftval,
     &		blksize, pe_low, pe_hi)
	endif
*
cdir$	barrier
...
*
	...
	subroutine cshift_i_spec_wd2_p2@ (result, source, dim, sext, shftval, 
     &		blksize, pe_low, pe_hi)
*
**************************************************************************
*
...
cdir$	pe_resident	source, result
*
	call cshift_spec_w_wd2_p2@ (result(1,1), source(1,1), dim, blksize,
     &		shftval, pe_low, pe_hi)
*
	return
	end
	...
	subroutine cshift_spec_w_wd2_p2@ (result, source, dim, sext, shftval,
     &		pe_low, pe_hi)
*
**************************************************************************
*
