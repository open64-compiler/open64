...
/* producer function to add a prologue unat_when descriptor */
__unw_error_t unwind_info_add_prologue_unat_when_info(__unw_info_t *info,
								__uint64_t when) {
	...
	/* construct and add the unat_when descriptor */
	/* format P7 */
	/* 7 6 5 4 3 2 1 0           */ 
	/* 1 1 1 0 1 1 0 0 t(LEB128) */
	encoded[0] = 0xec;
...
/* producer function to add a prologue unat_psprel descriptor */
__unw_error_t unwind_info_add_prologue_unat_psprel_info(__unw_info_t *info,
								__uint64_t offset) {
	__unw_error_t ret = __UNW_OK;
	__unw_format_p7_t *desc;
	__uint64_t rsize, esize;
	...
	/* construct and add the unat_psprel descriptor */
	/* format P7 */
	/* 7 6 5 4 3 2 1 0                */ 
	/* 1 1 1 0 1 1 0 1 pspoff(LEB128) */
	encoded[0] = 0xed;
	if ((esize = __leb128_encode((char *)&encoded[1],
			(__uint64_t)__UNW_ENCODING_SIZE,
			offset)) == 0) {
