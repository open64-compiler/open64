...
bool in_cleanup = FALSE; // TRUE if we are expanding code to be executed during
                         // stack unwinding
...
typedef struct handler_info_t {
  ...
  vector<ST_IDX> *eh_spec; // eh_spec of the containing region to be used while
                           // inside its handler
  LABEL_IDX label_idx;
...
// single variable.
std::stack<HANDLER_INFO>
    handler_stack; // formed from handler_info_stack in Do_Handlers
#else
typedef struct handler_info_t {
  gs_t handler;
  LABEL_IDX label_idx;
...
struct cmp_types : public std::binary_function<const TYPE_FILTER_ENTRY,
                                               const TYPE_FILTER_ENTRY, bool> {
  bool operator()(const TYPE_FILTER_ENTRY &e1, const TYPE_FILTER_ENTRY &e2) {
    return (e1.st < e2.st);
  }
};
...
enum Try_Branch { try_branch, catch_branch, finally_branch };
...
public:
  int Get_Parent(int index) { return try_vector[index].parent; }
  ...
  LABEL_IDX Get_Cmp_Label(int index) { return try_vector[index].cmp_idx[0]; }
  ...
  void Pop_State() { state_stack.pop(); }
  int Get_Current_Try() {
    if (state_stack.empty())
      return -1;
    else {
  ...
  Try_State Get_Current_State() { return state_stack.top(); }
  bool In_Catch_With_Finally() {
    if (state_stack.empty())
      return false;
    if (state_stack.top().branch == catch_branch &&
        gs_tree_code(Get_Indexed_Try(state_stack.top().index).stmt) ==
            GS_TRY_FINALLY_EXPR)
      return true;
    return false;
  }
  void Set_Index_First() { current_index = 0; }
  void Set_Index_Next() { current_index++; }
  bool Index_At_End() { return current_index == try_vector.size(); }
  Try_Info Indexing_TI() { return try_vector[current_index]; }
  int Current_Index() { return current_index; }
  void Clean_Up() {
    try_vector.clear();
  ...
  Try_Info Get_Indexed_Try(int index) { return try_vector[index]; }
  bool State_Empty() { return state_stack.empty(); }
...
static TY_IDX Type_For_Function_Returning_Void(void) {
...
static ST *Function_ST_For_String(const char *s) {
  ST *st = New_ST(GLOBAL_SYMTAB);
  PU_IDX pu_idx;
  PU &pu = New_PU(pu_idx);
  PU_Init(pu, Type_For_Function_Returning_Void(), GLOBAL_SYMTAB + 1);
  ST_Init(st, Save_Str(s), CLASS_FUNC, SCLASS_EXTERN, EXPORT_PREEMPTIBLE,
          TY_IDX(pu_idx));
  return st;
}
...
static inline void Maybe_Emit_Cleanup(INT i, BOOL from_handler) {
  if (!from_handler) {
    gs_t stmt = scope_cleanup_stack[i].stmt;
    if (gs_tree_code(stmt) == GS_CLEANUP_STMT &&
        !scope_cleanup_stack[i].cleanup_eh_only)
...
static void Emit_Cleanup(gs_t cleanup) {
  ...
  if (gs_tree_code(cleanup) == GS_IF_STMT) {
    ...
    WN *test =
        WGEN_Expand_Expr_With_Sequence_Point(gs_if_cond(cleanup), Boolean_type);
...
static void Push_Scope_Cleanup(gs_t t, bool eh_only = false) {
  // Don't push a cleanup without a scope
  if (scope_cleanup_i == -1 && gs_tree_code(t) == GS_CLEANUP_STMT)
    return;
  ...
  if (++scope_cleanup_i == scope_cleanup_max) {
    scope_cleanup_max = ENLARGE(scope_cleanup_max);
    scope_cleanup_stack = (SCOPE_CLEANUP_INFO *)realloc(
        scope_cleanup_stack, scope_cleanup_max * sizeof(SCOPE_CLEANUP_INFO));
  }
  ...
  if (gs_tree_code(t) == GS_CLEANUP_STMT ||
    ...
    New_LABEL(CURRENT_SYMTAB, scope_cleanup_stack[scope_cleanup_i].label_idx);
  else
    scope_cleanup_stack[scope_cleanup_i].label_idx = 0;
  if (gs_tree_code(t) == GS_TRY_BLOCK) {
    New_LABEL(CURRENT_SYMTAB, scope_cleanup_stack[scope_cleanup_i].cmp_idx);
    // try_monitor.Set_Cmp_Label(t, scope_cleanup_stack
    // [scope_cleanup_i].cmp_idx);
  } else
    scope_cleanup_stack[scope_cleanup_i].cmp_idx = 0;
  scope_cleanup_stack[scope_cleanup_i].cleanup_eh_only = eh_only;
  scope_cleanup_stack[scope_cleanup_i].vla.has_alloca = FALSE;
...
#ifdef KEY
void Register_Cleanup(gs_t t) { Push_Scope_Cleanup(t); }
#endif

static void Push_Handler_Info(
    int index, gs_t stmt, std::list<LABEL_IDX> *catch_labels,
    vector<gs_t> *v, // czw 1.18
    vector<SCOPE_CLEANUP_INFO> *scope, vector<TEMP_CLEANUP_INFO> *temp,
    vector<BREAK_CONTINUE_INFO> *break_continue, vector<ST_IDX> *handler_list,
    vector<ST_IDX> *eh_spec, LABEL_IDX label_idx, bool outermost,
    LABEL_IDX cmp_idx[], LABEL_IDX goto_idx) {
 ...
 handler_info_stack [handler_info_i].stmt  = stmt;
 //czw 1.24 handler_info_stack [handler_info_i].catch_labels   = catch_labels;
 //czw 1.23 handler_info_stack [handler_info_i].cleanups = v; handler_info_stack
 [handler_info_i].scope = scope; handler_info_stack
 [handler_info_i].temp_cleanup = temp; handler_info_stack
 [handler_info_i].break_continue = break_continue; handler_info_stack
 [handler_info_i].handler_list = handler_list; handler_info_stack
 [handler_info_i].eh_spec = eh_spec; handler_info_stack
 [handler_info_i].label_idx = label_idx; handler_info_stack
 [handler_info_i].cmp_idx[0] = cmp_idx[0]; handler_info_stack
 [handler_info_i].cmp_idx[1] = cmp_idx[1]; New_LABEL (CURRENT_SYMTAB,
 handler_info_stack [handler_info_i].cleanups_idx); handler_info_stack
 [handler_info_i].goto_idx = goto_idx; handler_info_stack
 [handler_info_i].outermost = outermost;
 */
  Try_Info ti;
...
void Do_Handlers(void) {
  ...
  while (!try_monitor.Index_At_End()) { // czw
    ...
    if (start_handlers) {
      Is_True(LABEL_kind(Label_Table[start_handlers]) == LKIND_BEGIN_HANDLER,
              ("Wrong label kind, expecting handler_begin"));
    ...
    WN *actual_cmp = WN_CreateLabel((ST_IDX)0, ti.cmp_idx[0], 0, NULL);
  ...
  if (key_exceptions)
    FmtAssert(cleanup_list_for_eh.empty(),
...
ST *Get_eh_spec_ST(void) {
  ...
  TY_IDX ty;
  TY_Init(New_TY(ty), eh_spec_size * sizeof(ST_IDX), KIND_ARRAY, MTYPE_UNKNOWN,
          str);
  Set_TY_arb(ty, arb);
...
ST *Get_typeinfo_ST(void) {
  ...
  FLD_Init(fld1, Save_Str("st"), MTYPE_TO_TY_array[MTYPE_U4], 0);
  FLD_HANDLE fld2 = New_FLD();
  FLD_Init(fld2, Save_Str("filter"), MTYPE_TO_TY_array[MTYPE_U4], 4);
  Set_FLD_flags(fld2, FLD_LAST_FIELD);

  TY_IDX struct_ty;
  TY_Init(New_TY(struct_ty), sizeof(TYPE_FILTER_ENTRY), KIND_STRUCT, MTYPE_M,
          Save_Str("__TYPEINFO_ENTRY__"));
...
void Do_EH_Tables(void) {
  ...
  for (int i = 0; i < type_filter_vector.size(); ++i) {
    ...
    if (i == (type_filter_vector.size() - 1)) {
      ...
      INITV_IDX index =
          INITV_next(INITV_next(INITO_val((INITO_IDX)Get_Current_PU().unused)));
      // INITV_Set_VAL resets the next field, so back it up
      // and set it again.
      INITV_IDX bkup = INITV_next(index);
      INITV_Set_VAL(Initv_Table[index], Enter_tcon(Host_To_Targ(MTYPE_U4, id)),
                    1);
  ...
  for (int i = 0; i < eh_spec_func_end.size(); ++i) {
    ...
    if (eh_spec_func_end[i])
      INITV_Set_VAL(Initv_Table[st],
                    Enter_tcon(Host_To_Targ(MTYPE_U4, eh_spec_func_end[i])), 1);
    else
      INITV_Set_ZERO(Initv_Table[st], MTYPE_U4, 1);
...
static void Do_Cleanups_For_EH(void) {
  for (std::list<EH_CLEANUP_ENTRY>::iterator i = cleanup_list_for_eh.begin();
    ...
    WGEN_Stmt_Append(WN_CreateLabel((ST_IDX)0, e.start, 0, NULL), Get_Srcpos());

    for (vector<gs_t>::iterator j = e.cleanups->begin(); j != e.cleanups->end();
         ++j) {
      gs_t cleanup = *j;
...
static void Pop_Scope_And_Do_Cleanups(void) {
    ...
    if (gs_tree_code(t) != GS_CLEANUP_STMT) {
      if (gs_tree_code(t) == GS_BIND_EXPR || gs_tree_code(t) == GS_HANDLER) {
        ...
        if (scope_cleanup_stack[scope_cleanup_i].vla.has_alloca)
          WGEN_Dealloca(
              scope_cleanup_stack[scope_cleanup_i].vla.alloca_st,
              scope_cleanup_stack[scope_cleanup_i].vla.alloca_sts_vector);
        --scope_cleanup_i;
...
#ifdef KEY
void Unregister_Cleanup() { Pop_Scope_And_Do_Cleanups(); }
#endif

static void Push_Scope(gs_t t) {
  if (++scope_i == scope_max) {
    scope_max = ENLARGE(scope_max);
    scope_stack = (gs_t *)realloc(scope_stack, scope_max * sizeof(gs_t));
  }
  scope_stack[scope_i] = t;
}

void Push_Temp_Cleanup(gs_t t, bool is_cleanup
#ifdef KEY
                       ,
                       bool cleanup_eh_only
#endif
  ...
  if (++temp_cleanup_i == temp_cleanup_max) {
    ...
    temp_cleanup_stack = (TEMP_CLEANUP_INFO *)realloc(
        temp_cleanup_stack, temp_cleanup_max * sizeof(TEMP_CLEANUP_INFO));
  }
...
static bool cleanup_matches(gs_t candidate, gs_t target) {
  if (candidate == target)
    return TRUE;
...
void Do_Temp_Cleanups(gs_t t) {
  ...
  while (!cleanup_matches(temp_cleanup_stack[temp_cleanup_i].expr, t)) {
    if (temp_cleanup_stack[temp_cleanup_i].cleanup_eh_only) {
      // We don't want this cleanup to be emitted here -- it is to be executed
...
static void WGEN_Record_Loop_Switch(gs_code_t tree_code) {
  INT32 i;
  Is_True(tree_code == GS_DO_STMT || tree_code == GS_FOR_STMT ||
              tree_code == GS_WHILE_STMT || tree_code == GS_SWITCH_STMT,
  ...
  if (++break_continue_info_i == break_continue_info_max) {
    ...
    break_continue_info_stack = (BREAK_CONTINUE_INFO *)realloc(
        break_continue_info_stack,
        break_continue_info_max * sizeof(BREAK_CONTINUE_INFO));
  }

  break_continue_info_stack[break_continue_info_i].tree_code = tree_code;
  break_continue_info_stack[break_continue_info_i].break_label_idx = 0;
  break_continue_info_stack[break_continue_info_i].continue_label_idx = 0;
  if (scope_cleanup_i == -1)
    break_continue_info_stack[break_continue_info_i].scope = NULL;
  else {
    for (i = scope_cleanup_i;
         gs_tree_code(scope_cleanup_stack[i].stmt) == GS_CLEANUP_STMT; --i)
      ;
    Is_True(
        i != -1 &&
    ...
    break_continue_info_stack[break_continue_info_i].scope =
        scope_cleanup_stack[i].stmt;
  }
...
static void WGEN_Expand_Case(gs_t low, gs_t high
#ifdef KEY
                             ,
  ...
  if (low == NULL) {
    ...
    upper_bound = (high == NULL) ? lower_bound : WGEN_Expand_Expr(high);
    ...
    case_info_stack[case_info_i].case_lower_bound_value =
        (low == NULL) ? 0 : WN_const_val(lower_bound);
    case_info_stack[case_info_i].case_upper_bound_value =
        (high == NULL) ? WN_const_val(lower_bound) : WN_const_val(upper_bound);
...
static void WGEN_Declare_Nonlocal_Label(gs_t label) {
...
static WN *idname_from_regnum(int gcc_reg) {
  if (gcc_reg < 0) {
...
char *remove_plus_modifier(char *s) {
...
BOOL constraint_supported(const char *s) {
  while (*s != 0) {
    if (*s != 'r' && *s != 'f' && *s != 'm' && *s != '+' && *s != ',' &&
        *s != '=' && (*s < '0' || *s > '9')) {
      return FALSE;
...
ST *st_of_new_temp_for_expr(const WN *expr) {
  ...
  ST_Init(retval, Save_Str(temp_name), CLASS_VAR, SCLASS_AUTO, EXPORT_LOCAL,
...
static BOOL constraint_by_address(const char *s) {
#ifndef TARG_X8664
  if (strchr(s, 'm')) {
#else
...
static void update_opnd_num(int *opnd_num_map, char *constraint_string) {
  char *p;
...
static WN *add_offset(WN_OFFSET ofst,
                      WN *address) // not const; some simplification may occur.
{
  return WN_Binary(OPR_ADD, Pointer_Mtype, WN_Intconst(MTYPE_I8, ofst),
                   address);
}

static WN *address_of(const WN *wn) {
  if (WN_operator(wn) == OPR_ILOAD || WN_operator(wn) == OPR_MLOAD) {
    return add_offset(WN_offset(wn), WN_kid0(wn));
  } else if ((WN_operator(wn) == OPR_LDID) &&
             (ST_sclass(WN_st(wn)) != SCLASS_REG)) {
    return WN_Lda(Pointer_Mtype, WN_offset(wn), WN_st(wn), (UINT)0);
  }
...
void Wgen_Expand_Asm_Operands(gs_t string, gs_t outputs, gs_t inputs,
                              gs_t clobbers, int vol, char *filename,
                              int line) {
  ...
  for (tail = outputs; tail; tail = gs_tree_chain(tail)) {
    ...
    constraint_string = const_cast<char *>(
        gs_tree_string_pointer(gs_tree_value(gs_tree_purpose(tail))));
#else
  ...
  for (tail = clobbers; tail; tail = gs_tree_chain(tail)) {
    ...
    if (gcc_reg == -3)
      ...
      if (clobbered_idname) {
        ...
        ST_Init(clobber_st,
                Str_To_Index(Save_Str(clobber_string), Current_Strtab),
                CLASS_NAME, SCLASS_UNKNOWN, EXPORT_LOCAL, (TY_IDX)0);

        clobber_pragma =
            WN_CreateXpragma(WN_PRAGMA_ASM_CLOBBER, ST_st_idx(clobber_st), 1);
        ...
        clobber_pragma = WN_CreatePragma(
            WN_PRAGMA_ASM_CLOBBER, ST_st_idx(clobber_st), (INT32)0, (INT32)0);
    ...
    if (clobber_pragma != NULL)
      WN_INSERT_BlockAfter(clobber_block, WN_last(clobber_block),
                           clobber_pragma);
  }
  ...
  for (tail = inputs; tail; tail = gs_tree_chain(tail)) {
    ...
    if (constraint_by_address(constraint_string)) {
      ...
      if (addr_of_rvalue != NULL) {
        ...
        WN *store_wn =
            WN_Stid(WN_rtype(input_rvalue), (WN_OFFSET)0, temp_st,
                    ...
                    // store.
                    MTYPE_To_TY(WN_rtype(input_rvalue)), input_rvalue);
        WGEN_Stmt_Append(store_wn, Get_Srcpos());
        input_rvalue = WN_Lda(Pointer_Mtype, (WN_OFFSET)0, temp_st, (UINT)0);
      }
    }
...
#endif
  for (tail = outputs; tail; tail = gs_tree_chain(tail), ++opnd_num) {
#ifdef KEY
    // In gcc-3.2, TREE_PURPOSE of tail represents a TREE_LIST node whose
    // first operand gives the string constant.
    constraint_string = const_cast<char *>(
    ...
    if (!constraint_by_address(constraint_string)) {
      ...
      WN *output_rvalue_wn = WGEN_Lhs_Of_Modify_Expr(
          GS_MODIFY_EXPR, gs_tree_value(tail), NULL, plus_modifier,
      ...
      if (plus_modifier) {
        WN_kid(asm_wn, i) = WN_CreateAsm_Input(input_opnd_constraint, opnd_num,
                                               output_rvalue_wn);
        ++i;
      }
      ...
      WN *constraint_pragma = WN_CreatePragma(
          WN_PRAGMA_ASM_CONSTRAINT, (ST_IDX)ST_st_idx(preg_st),
          (INT32)ST_st_idx(constraint_st), asm_neg_preg, opnd_num);

      WN_INSERT_BlockAfter(output_constraint_block,
                           WN_last(output_constraint_block), constraint_pragma);
      --asm_neg_preg;
    }
  }
  ...
  if (label_idx == 0
    ...
    if (!def) {
      if (++undefined_labels_i == undefined_labels_max) {
        ...
        undefined_labels_stack = (LABEL_INFO *)realloc(
            undefined_labels_stack, undefined_labels_max * sizeof(LABEL_INFO));
      }
...
void WGEN_Check_Undefined_Labels(void) {
  INT32 i;
  for (i = undefined_labels_i; i >= 0; --i) {
    ...
    SYMTAB_IDX symtab_idx =
        undefined_labels_stack[undefined_labels_i].symtab_idx;
    //  fprintf (stderr, "WGEN_Check_Undefined_Labels: %d idx = %8x [%d]\n", i,
    //  label_idx, symtab_idx);
...
void WGEN_Stmt_Init(void) {
  ...
  break_continue_info_stack = (BREAK_CONTINUE_INFO *)malloc(
      sizeof(BREAK_CONTINUE_INFO) * break_continue_info_max);
  ...
  scope_cleanup_i = -1;
  scope_cleanup_stack = (SCOPE_CLEANUP_INFO *)malloc(
      sizeof(SCOPE_CLEANUP_INFO) * scope_cleanup_max);

  scope_max = 32;
  scope_i = -1;
  scope_stack = (gs_t *)malloc(sizeof(gs_t) * scope_max);

  temp_cleanup_max = 32;
  temp_cleanup_i = -1;
  temp_cleanup_stack =
      (TEMP_CLEANUP_INFO *)malloc(sizeof(TEMP_CLEANUP_INFO) * temp_cleanup_max);
...
static void Cleanup_To_Scope_From_Handler(gs_t scope) {
  INT32 i = scope_cleanup_i;
  INT32 j = -1;
  Is_True(i != -1,
          ("Cleanup_To_Scope_From_Handler: scope_cleanup_stack empty"));
  while ((i != -1) && (scope_cleanup_stack[i].stmt != scope)) {
    gs_t stmt = scope_cleanup_stack[i].stmt;
    if (gs_tree_code(stmt) == GS_BIND_EXPR || gs_tree_code(stmt) == GS_HANDLER)
      j = i;
    --i;
  }
...
static void Cleanup_To_Scope(gs_t scope) {
  INT32 i = scope_cleanup_i;
  INT32 j = -1;
...
//
static void WGEN_Expand_Break(void) {
  ...
  if (i == -1) {
    FmtAssert(processing_handler && ti.break_continue,
              ("No break/continue info"));
    label_idx =
        (*ti.break_continue)[ti.break_continue->size() - 1].break_label_idx;
...
static void WGEN_Expand_Continue(void) {
  INT32 i = break_continue_info_i;
  ...
  if (processing_handler)
    ti = try_monitor.Indexing_TI(); // czw
  if (i == -1) {
    FmtAssert(processing_handler,
              ("WGEN_Expand_Continue: No break/continue info"));
    scope = (*ti.break_continue)[ti.break_continue->size() - 1].scope;
  } else
    scope = break_continue_info_stack[i].scope;
  WN *wn;
  ...
  if (i != -1) {
    while (break_continue_info_stack[i].tree_code == GS_SWITCH_STMT)
      --i;
    if (i != -1) {
  ...
  if (key_exceptions && processing_handler &&
      !label_idx) { // have not yet found the enclosing loop
    INT32 j = ti.break_continue->size() - 1;
    while ((*ti.break_continue)[j].tree_code == GS_SWITCH_STMT)
      --j;
...
static void WGEN_Expand_Loop(gs_t stmt) {
  ...
  if (!cond) {
...
#endif // KEY
      if (gs_tree_code(cond) == GS_TREE_LIST && gs_tree_value(cond) == NULL) {
    ...
    if (WN_first(cond_block)) {
      loop_test = WN_CreateComma(OPR_COMMA, Boolean_type, MTYPE_V, cond_block,
                                 loop_test);
    } else
  ...
  if (gs_tree_code(stmt) == GS_WHILE_STMT || gs_tree_code(stmt) == GS_FOR_STMT)
    ...
    if (break_continue_info_stack[break_continue_info_i].continue_label_idx) {
      WGEN_Stmt_Append(
          WN_CreateLabel((ST_IDX)0,
                         break_continue_info_stack[break_continue_info_i]
                             .continue_label_idx,
                         0, NULL),
          Get_Srcpos());
    }
...
void WGEN_Expand_Goto(gs_t label) // KEY VERSION
{
  WN *wn;
  ...
  if ((CURRENT_SYMTAB > GLOBAL_SYMTAB + 1) &&
    ...
    if (scope != NULL && scope_cleanup_i != -1) {
      ...
      if (ci != Current_scope_nest.rend()) {
        ...
        while ((i >= 0) && (scope_cleanup_stack[i].stmt != *ci)) {
          ...
          if (lang_java) {
            int index = try_monitor.Find_Finally_Containing_This_Bind(
                scope_cleanup_stack[i].stmt);
            if (index != -1) {
    ...
    if (in_handler && processing_handler && key_exceptions) {
      ...
      Is_True(i != -1,
              ("WGEN_Expand_Goto: scope_cleanup_stack empty inside handler"));
...
static void WGEN_Expand_Computed_Goto(gs_t exp) {
...
static void WGEN_Expand_If(gs_t stmt) {
  WN *if_stmt;
  WN *test;
  WN *then_block;
  WN *else_block;
...
void WGEN_Expand_Label(gs_t label) {
...
// comma_block.
static BOOL comma_is_not_needed(WN *comma_block, WN *wn) {
  ...
  if (sym != WN_st(wn) || Is_Global_Symbol(sym) || // bug 11799
...
void WGEN_Expand_Return(gs_t stmt, gs_t retval) {
  WN *wn = NULL;

  if (retval == NULL ||
      gs_tree_code(gs_tree_type(gs_tree_type(Current_Function_Decl()))) ==
          GS_VOID_TYPE) {
    if (retval)
    ...
    if (lang_java) {
      for (int index = try_monitor.Get_Current_Try(); index != -1;
           index = try_monitor.Get_Parent(index)) {
        gs_t t = try_monitor.Get_Indexed_Try(index).stmt;
    ...
    WN *rhs_wn;
    TY_IDX ret_ty_idx =
        Get_TY(gs_tree_type(gs_tree_type(Current_Function_Decl())));
    ...
    rhs_wn = WGEN_Expand_Expr_With_Sequence_Point(retval, TY_mtype(ret_ty_idx));
    ...
    if (WN_first(cleanup_block)) {
      ...
      if ((gs_tree_code(retval) == GS_TARGET_EXPR ||
...
#ifdef KEY
            // Fix bug in which COMPOUND_EXPR has kid0==OPR_COMMA,
            // kid1==OPR_CSELECT which has side-effects.
        ...
        if (WN_has_side_effects(rhs_wn)) {
          DevWarn("WGEN_Expand_Return: cleanup block and expressson has side "
                  "effects");
#endif
    ...
    if ((!WGEN_Keep_Zero_Length_Structs && TY_mtype(ret_ty_idx) == MTYPE_M &&
      ...
      WN *dest_addr = WN_CreateLdid(
          OPR_LDID, TY_mtype(Ty_Table[tidx]), TY_mtype(Ty_Table[tidx]),
          WN_idname_offset(first_formal), WN_st(first_formal), tidx);
      ...
      WGEN_Set_ST_Addr_Saved(rhs_wn);
      wn = WN_CreateReturn_Val(OPR_RETURN_VAL, WN_rtype(rhs_wn), MTYPE_V,
                               rhs_wn);
    }
  }
  if (wn)
...
void Mark_Scopes_And_Labels(gs_t stmt) {
  if (!stmt)
    return;
...
static void WGEN_Expand_Start_Case(gs_t selector) {
#ifdef KEY
  TYPE_ID index_mtype;
  if (gs_tree_code(selector) == GS_TREE_LIST)
    ...
    index_mtype = Mtype_comparison(TY_mtype(Get_TY(gs_tree_type(selector))));
#else
  TYPE_ID index_mtype =
      Mtype_comparison(TY_mtype(Get_TY(gs_tree_type(gs_tree_value(selector)))));
#endif // KEY
  ...
  if (++switch_info_i == switch_info_max) {
    ...
    switch_info_stack = (SWITCH_INFO *)realloc(
        switch_info_stack, switch_info_max * sizeof(SWITCH_INFO));
  }
...
static void WGEN_Expand_End_Case(void) {
  INT32 i;
  INT32 n;
  WN *switch_wn;
  WN *switch_block;
  ...
  if (break_continue_info_stack[break_continue_info_i].break_label_idx)
    exit_label_idx =
        break_continue_info_stack[break_continue_info_i].break_label_idx;
  else
    New_LABEL(CURRENT_SYMTAB, exit_label_idx);
  if (switch_info_stack[switch_info_i].default_label_idx)
    def_goto =
        WN_CreateGoto(switch_info_stack[switch_info_i].default_label_idx);
  else
    def_goto = WN_CreateGoto(exit_label_idx);
  case_block = WN_CreateBlock();
  for (i = switch_info_stack[switch_info_i].start_case_index; i <= case_info_i;
       i++) {
    INT64 case_value;
  ...
  switch_wn = WN_CreateSwitch(n, switch_info_stack[switch_info_i].index,
                              case_block, def_goto, exit_label_idx);
...
static void WGEN_Expand_Switch(gs_t stmt) {
...
#ifdef KEY
static void WGEN_Expand_Switch_Expr(gs_t switch_expr) {
  ...
  if (gs_tree_operand(switch_expr, 1))
    ...
    Is_True(opnd2, ("WGEN_Expand_Switch_Expr: Both opnd1 and opnd2 of "
    ...
    for (int i = 0; i < gs_tree_vec_length(opnd2); i++) {
      ...
      WGEN_Expand_Case(gs_case_low(case_label), gs_case_high(case_label), label,
                       TRUE);
    }
...
// czw
static void Set_Handler_Labels(gs_t stmt, std::list<LABEL_IDX> *catch_labels) {
  if (gs_cleanup_p(stmt))
    return;
...
INT Current_Handler_Count() {
  ...
  for (int i = scope_cleanup_i; i != -1; --i) {
    ...
    INT result = 0;
    if (gs_tree_code(t) == GS_TRY_BLOCK) {
      for (gs_t handler = gs_try_handlers(t); handler;
...
static ST_IDX Tid_For_Handler(gs_t handler) {
...
void Add_Handler_Info(WN *call_wn, INT i, INT num_handlers) {
  if (temp_cleanup_i != -1) {
    for (int i = temp_cleanup_i; i != -1; --i)
      if (temp_cleanup_stack[i].label_idx != 0) {
        WN_kid(call_wn, i++) = WN_CreateHandlerInfo(
            0, temp_cleanup_stack[temp_cleanup_i].label_idx);
        return;
      }
  }
  ...
  if (gs_tree_code(t) == GS_TRY_BLOCK &&
      gs_tree_code(TRY_HANDLERS(t)) == GS_HANDLER) {
    for (gs_t handler = TRY_HANDLERS(t); handler;
         handler = gs_tree_chain(handler))
      WN_kid(call_wn, i++) = WN_CreateHandlerInfo(Tid_For_Handler(handler),
  ...
  if (gs_tree_code(t) == GS_TRY_CATCH_EXPR) {
    if (gs_tree_code(gs_tree_operand(t, 1)) == GS_CATCH_EXPR)
      WN_kid(call_wn, i++) =
          WN_CreateHandlerInfo(0, HANDLER_LABEL(gs_tree_operand(t, 1)));
    else {
      for (gs_t cmpd = gs_tree_operand(t, 1);
           gs_tree_code(cmpd) == GS_COMPOUND_EXPR;
           cmpd = gs_tree_operand(cmpd, 1)) {
        WN_kid(call_wn, i++) =
            WN_CreateHandlerInfo(0, HANDLER_LABEL(gs_tree_operand(cmpd, 0)));
      }
...
static gs_t Get_typeinfo_var(gs_t t) {
  gs_t ti_var = 0;
...
// Given a type tree, return the typeinfo var for the exception tables
static gs_t Get_type_class(gs_t t) // czw
{
  // assert(NOP_EXPR)
...
static INITV_IDX Create_handler_list() // czw removed the parameter
{
  ...
  if (processing_handler) {
    ...
    if (prev_type_st)
      Set_INITV_next(prev_type_st, next);
    else
      start = next;
  }
  return start;
...
//  }
//
static INITV_IDX lookup_handlers(vector<gs_t> *cleanups) {
  Try_Info ti = try_monitor.Indexing_TI(); // czw
  ...
  for (vector<ST_IDX>::iterator i = h->begin(); i != h->end(); ++i) {
    type_st = New_INITV();
    ST_IDX st = *i;
    INITV_Set_VAL(Initv_Table[type_st], Enter_tcon(Host_To_Targ(MTYPE_U4, st)),
                  1);
...
LABEL_IDX
New_eh_cleanup_entry(gs_t t, gs_t f, vector<gs_t> *v,
                     LABEL_IDX goto_idx) // czw 1.21
{
  EH_CLEANUP_ENTRY e;
...
static void append_eh_filter(INITV_IDX &iv) {
  INITV_IDX tmp = iv;
  while (tmp && INITV_next(tmp))
    tmp = INITV_next(tmp);

  INITV_IDX eh_filter = New_INITV();
  INITV_Set_VAL(Initv_Table[eh_filter],
                Enter_tcon(Host_To_Targ(MTYPE_I4, -current_eh_spec_ofst)), 1);
  if (tmp)
    Set_INITV_next(tmp, eh_filter);
  else
    iv = eh_filter;
}

static void append_catch_all(INITV_IDX &iv) {
  INITV_IDX tmp = iv;
  while (tmp && INITV_next(tmp))
    tmp = INITV_next(tmp);

  INITV_IDX catch_all = New_INITV();
  INITV_Set_VAL(Initv_Table[catch_all], Enter_tcon(Host_To_Targ(MTYPE_U4, 0)),
                1);
  if (tmp)
    Set_INITV_next(tmp, catch_all);
  else
    iv = catch_all;
}
...
static bool optimize_cleanups(vector<gs_t> *current, vector<gs_t> *prev) {
...
lookup_cleanups (INITV_IDX& iv)
{
  ...
  for (scope_index=scope_cleanup_i; scope_index>=0; scope_index--)
  {
        ...
        if (gs_tree_code(t) == GS_CLEANUP_STMT)
                ...
                ||(lang_cplus && (gs_tree_code(t) == GS_TRY_FINALLY_EXPR)))
//czw 1.17 cleanups->push_back (gs_tree_operand(t,1)); else if (gs_tree_code(t)
== GS_TRY_BLOCK
                || (lang_java && (gs_tree_code(t) == GS_TRY_CATCH_EXPR))
                || (lang_java && (gs_tree_code(t) == GS_TRY_FINALLY_EXPR)))
            if (lang_cplus && gs_cleanup_p(t)) cleanups->push_back
(gs_try_handlers(t));	//czw 1.17 else
                {
...
// NOTE: CG expects a zero before eh-spec filter
  ...
  else
  {
        ...
        if (h == 0 && f == 0 && e.tryhandler == 0 && e.finally == 0 &&
!processing_handler &&		//czw 1.21 cleanups->size() !=
e.cleanups->size())
        {
        ...
        if ((h != e.tryhandler || f != e.finally) || // different try block
//czw 1.21 (cleanups->size() != e.cleanups->size())) // # of cleanups doesn't
match return New_eh_cleanup_entry (h, f, cleanups, goto_idx);
...
lookup_cleanups(INITV_IDX &iv) {
  ...
  if (lang_cplus) {
    ...
    for (scope_index = scope_cleanup_i; scope_index >= 0; scope_index--) {
      ...
      if (gs_tree_code(t) == GS_CLEANUP_STMT)
        ...
        if (gs_cleanup_p(t))
          cleanups->push_back(gs_try_handlers(t));
        else
          break;
      if (temp_cleanup && (cleanups->size() == 1)) {
        cleanups->push_back(temp_cleanup);
  ...
  if (lang_java) {
    if (try_monitor.Get_Current_Try() == -1 &&
        !try_monitor.In_Catch_With_Finally()) // czw
    {
      iv = New_INITV();
      INITV_Set_ZERO(Initv_Table[iv], MTYPE_U4, 1);
      return 0;
  ...
  else // no enclosing try block
  {
    if (processing_handler) {
      ...
      if (try_monitor.In_Catch_With_Finally()) // czw
        goto_idx =
            try_monitor.Get_Indexed_Try(try_monitor.Get_Current_State().index)
                .finally_label;
      else
        goto_idx = try_monitor.Indexing_TI().goto_idx;
    } else if (cleanups->empty() && eh_spec_vector.empty()) {
  ...
  if (cleanup_list_for_eh.empty()) {
    ...
    if ((h != e.tryhandler ||
         f != e.finally) || // different try block //czw 1.21
...
Get_handler_list (vector<ST_IDX> *handler_list)
{
  FmtAssert (gs_tree_code(scope_cleanup_stack[scope_cleanup_i+1].stmt) ==
                GS_TRY_BLOCK
                || (lang_java &&
(gs_tree_code(scope_cleanup_stack[scope_cleanup_i+1].stmt) ==      //czw
                GS_TRY_CATCH_EXPR))
                || (lang_java &&
(gs_tree_code(scope_cleanup_stack[scope_cleanup_i+1].stmt) ==      //czw 1.17
                GS_TRY_FINALLY_EXPR))
                , ("EH Error"));
        ...
        if(lang_java)
        {
                if(gs_tree_code(t) == GS_TRY_CATCH_EXPR)
                {
                    ...
                    for (iter.First(); iter.Not_Empty(); iter.Next())
                    {
                        ...
                        if (type)
                            st = ST_st_idx (Get_ST (Get_type_class(type)));
//czw handler_list->push_back (st);
                    }
                }
                else if(gs_tree_code(t) == GS_TRY_FINALLY_EXPR)
                {
                        if(gs_tree_code(gs_tree_operand(t, 0)) ==
GS_TRY_CATCH_EXPR)
                        {
                            CATCH_ITER iter (gs_tree_operand(gs_tree_operand(t,
0), 1)); for (iter.First(); iter.Not_Empty(); iter.Next())
                            {
                                gs_t h = iter.Current();
                                gs_t type = gs_tree_operand(h, 0);
                                ST_IDX st = 0;	// catch-all
                                if (type)
                                    st = ST_st_idx (Get_ST
(Get_type_class(type)));  //czw handler_list->push_back (st);
                            }
                        }
                }
        }
...
static void Get_handler_list(vector<ST_IDX> *handler_list) {
  if (lang_cplus) {
    FmtAssert(gs_tree_code(scope_cleanup_stack[scope_cleanup_i + 1].stmt) ==
                  GS_TRY_BLOCK,
              ("EH Error"));
...
{
  FmtAssert
(gs_tree_code(scope_cleanup_stack[scope_cleanup_i+1].stmt)==GS_TRY_BLOCK
                        || (lang_java &&
(gs_tree_code(scope_cleanup_stack[scope_cleanup_i+1].stmt)==GS_TRY_CATCH_EXPR))
//czw
                        || (lang_java &&
(gs_tree_code(scope_cleanup_stack[scope_cleanup_i+1].stmt)==GS_TRY_FINALLY_EXPR))
//czw  1.17
                ,("EH Processing Error"));
        if(lang_cplus)
        {
...
static bool Get_Cleanup_Info(int index, vector<gs_t> *cleanups,
                             LABEL_IDX *goto_idx) {
  if (lang_cplus) {
    FmtAssert(gs_tree_code(scope_cleanup_stack[scope_cleanup_i + 1].stmt) ==
                  GS_TRY_BLOCK,
    ...
    for (int i = scope_cleanup_i; i >= 0; i--) {
      ...
      if (gs_tree_code(t) == GS_CLEANUP_STMT)
        cleanups->push_back(t);
               ...
               gs_tree_code(t) == GS_TRY_FINALLY_EXPR)
        cleanups->push_back(gs_tree_operand(t, 1));
      else if (gs_tree_code(t) == GS_TRY_BLOCK) { // not the outermost try block
        *goto_idx = scope_cleanup_stack[i].cmp_idx;
        return false;
      }
    }
...
static vector<SCOPE_CLEANUP_INFO> *Get_Scope_Info(void) {
...
static vector<TEMP_CLEANUP_INFO> *Get_Temp_Cleanup_Info(void) {
...
static vector<BREAK_CONTINUE_INFO> *Get_Break_Continue_Info(void) {
...
static void Get_eh_spec(vector<ST_IDX> *in) {
...
static void WGEN_Expand_Try(gs_t stmt) {
  ...
  try_monitor.Set_Cmp_Label(index,
                            scope_cleanup_stack[scope_cleanup_i].cmp_idx);

#ifdef KEY
  vector<SCOPE_CLEANUP_INFO> *scope_cleanup =
      Get_Scope_Info(); // czw annotation:return the low part of
                        // stack_cleanup_info below this TRY_BLOCK
// FIXME: handle temp cleanups for return from handler.
  ...
  if (key_exceptions) {
    ...
    FmtAssert(cleanup_list_for_eh.size() >= handler_count,
  ...
  Push_Handler_Info(index, stmt, 0, cleanups, scope_cleanup, temp_cleanup,
                    break_continue, handler_list, eh_spec_list, end_label_idx,
                    outermost, cmp_idxs, goto_idx);
#else
...
void WGEN_Expand_Try_Catch(gs_t stmt) {
  LABEL_IDX end_label_idx;
  WN *end_label_wn;
...
#ifdef KEY
  vector<SCOPE_CLEANUP_INFO> *scope_cleanup =
      Get_Scope_Info(); // maybe need to remove
// FIXME: handle temp cleanups for return from handler.
#if 0 
  vector<TEMP_CLEANUP_INFO> *temp_cleanup = Get_Temp_Cleanup_Info ();
#else
  ...
  if (key_exceptions) {
    WGEN_Stmt_Pop(wgen_stmk_region_body);
    WN *region_pragmas = WN_CreateBlock();
    // FmtAssert (cleanup_list_for_eh.size() >= handler_count, ("Cleanups cannot
    // be removed here"));  LABEL_IDX cmp_idx =
    // scope_cleanup_stack[scope_cleanup_i+1].cmp_idx;
    if (cleanup_list_for_eh.size() > handler_count) {
      std::list<EH_CLEANUP_ENTRY>::iterator iter = cleanup_list_for_eh.begin();
      for (int incr = 0; incr < handler_count; ++incr)
        ++iter;
...
#ifdef KEY
  Push_Handler_Info(index, stmt, catch_labels, cleanups, scope_cleanup,
                    temp_cleanup, // czw
                    break_continue, handler_list, eh_spec_list, end_label_idx,
                    outermost, cmp_idxs, goto_idx);
#else
...
void WGEN_Expand_Try_Finally(gs_t stmt) // czw 1.17
{
  LABEL_IDX end_label_idx;
  WN *end_label_wn;
...
#ifdef KEY
static int sizeof_eh_spec(gs_t t) {
  int i = 1;
...
static void WGEN_Expand_EH_Spec(gs_t stmt) {
...
//   expand_eh_region_end_allowed (...);
  ...
  if (key_exceptions) {
    ...
    for (; eh_spec; eh_spec = gs_tree_chain(eh_spec)) {
      ST_IDX type_st =
          ST_st_idx(Get_ST(Get_typeinfo_var(gs_tree_value(eh_spec))));
  ...
  //
  gs_t eh_spec_stmt;
  for (eh_spec_stmt = gs_eh_spec_stmts(stmt); eh_spec_stmt != NULL;
       eh_spec_stmt = gs_tree_chain(eh_spec_stmt))
    WGEN_Expand_Stmt(eh_spec_stmt);
#else
  WGEN_Expand_Stmt(EH_SPEC_STMTS(stmt));
#endif
#ifdef KEY
  if (key_exceptions) { // now clear eh_spec_vector, eh_spec_func_end stays.
    if (!initial_size)
      eh_spec_vector.clear();
    else {
...
static WN *Generate_cxa_call_unexpected(void) {
  ST_IDX exc_ptr_param =
...
static void Generate_filter_cmp(int filter, LABEL_IDX goto_idx) {
  ST_IDX filter_param =
      TCON_uval(INITV_tc_val(INITV_next(INITO_val(Get_Current_PU().unused))));
  const TYPE_ID mtype = TARGET_64BIT ? MTYPE_U8 : MTYPE_U4;

  WN *wn_ldid =
      WN_Ldid(mtype, 0, &St_Table[filter_param], MTYPE_TO_TY_array[mtype]);
...
static void WGEN_Expand_Handlers_Or_Cleanup(int index) {
  ...
  if (gs_tree_code(stmt) == GS_TRY_CATCH_EXPR ||
      gs_tree_code(stmt) == GS_TRY_BLOCK)
...
#ifndef KEY
  WGEN_Stmt_Append(WN_CreateLabel((ST_IDX)0, HANDLER_LABEL(t), 0, NULL),
                   Get_Srcpos());
#endif // !KEY

  if (lang_cplus) // czw
  {
    if (gs_tree_code(handlers) == GS_HANDLER ||
        gs_tree_code(handlers) == GS_STATEMENT_LIST) {
        ...
        for (iter.First(); iter.Not_Empty(); iter.Next()) {
          ...
          if (f == type_filter_vector.end()) {
            ...
            if (e.st)
              ...
              WGEN_Stmt_Append(
                  WN_CreateGoto((ST_IDX)NULL, HANDLER_LABEL(t_copy)),
                  Get_Srcpos());
#if 0
...
// TODO: verify and remove it.
	  sort (type_filter_vector.begin(), type_filter_vector.end(), 
        ...
        WGEN_Stmt_Append(WN_CreateLabel((ST_IDX)0, cleanups_idx, 0, NULL),
                         Get_Srcpos());
        // Generate any cleanups that need to be executed before going to the
        // outer scope, which would be a handler in the same PU or a call to
        // _Unwind_Resume
        in_cleanup = TRUE;
        for (vector<gs_t>::iterator j = cleanups->begin(); j != cleanups->end();
             ++j)
      ...
      for (iter.First(); iter.Not_Empty(); iter.Next()) {
        ...
        WGEN_Stmt_Append(
            WN_CreateLabel((ST_IDX)0, HANDLER_LABEL(handlers), 0, NULL),
            Get_Srcpos());
#endif
...
// Fail_FmtAssertion ("Handle it");
  ...
  {
    if (handlers)
      if (gs_tree_code(handlers) == GS_CATCH_EXPR ||
          gs_tree_code(handlers) == GS_COMPOUND_EXPR) // czw 1.18
      {
#ifdef KEY
        ...
        if (key_exceptions) {
          ...
          for (iter.First(); iter.Not_Empty(); iter.Next()) {
            ...
            ST_IDX sym = 0;
            if (type)
              sym = ST_st_idx(Get_ST(Get_type_class(type)));
            TYPE_FILTER_ENTRY e;
            e.st = sym;
            e.filter = 0; // do not compare based on filter
            vector<TYPE_FILTER_ENTRY>::iterator f =
            ...
            if (f == type_filter_vector.end()) {
              ...
              if (e.st)
                ...
                WGEN_Stmt_Append(
                    WN_CreateGoto((ST_IDX)NULL, *catch_labels->begin()),
                    Get_Srcpos());
#if 0
						// we shouldn't need the following sort call
						// TODO: verify and remove it.
          ...
          /*WGEN_Stmt_Append (
                  WN_CreateLabel ((ST_IDX) 0, cleanups_idx, 0, NULL),
          Get_Srcpos());
          // Generate any cleanups that need to be executed before going to the
          outer
          // scope, which would be a handler in the same PU or a call to
          _Unwind_Resume in_cleanup = TRUE; for (vector<gs_t>::iterator
          j=cleanups->begin(); j!=cleanups->end(); ++j) WGEN_One_Stmt_Cleanup
          (gs_cleanup_expr (*j));
          ...
          {
            WGEN_Stmt_Append(WN_CreateLabel((ST_IDX)0, finally_label, 0, NULL),
                             Get_Srcpos());
            try_monitor.Push_State(index, finally_branch);
          ...
          if (outermost) {
            ...
            WGEN_Stmt_Append(WN_CreateGoto((ST_IDX)NULL, goto_idx),
                             Get_Srcpos());
        } // key_exceptions
#endif    // KEY
        // Now, emit the actual exception handler body's.
        for (iter.First(); iter.Not_Empty(); iter.Next()) {
          ...
          // proper label from 'cmp' above
          WGEN_Stmt_Append(
              WN_CreateLabel((ST_IDX)0, *catch_labels->begin(), 0, NULL),
              Get_Srcpos());
          catch_labels->pop_front();
#endif
          // Bug 11006: For a catch-all handler, GNU4 does not have a bind_expr
...
//*************************************************************************
void WGEN_Expand_Stmt(gs_t stmt, WN *target_wn) {
  if (gs_tree_code(stmt) == GS_LABEL_DECL)
    lineno = gs_decl_source_line(stmt);
  else if (gs_tree_code(stmt) != GS_CASE_LABEL_EXPR) {
  ...
  switch (gs_tree_code(stmt)) {
  case GS_ASM_EXPR:
    Wgen_Expand_Asm_Operands(gs_asm_string(stmt), gs_asm_outputs(stmt),
                             gs_asm_inputs(stmt), gs_asm_clobbers(stmt),
                             gs_asm_volatile_p(stmt), gs_expr_filename(stmt),
                             gs_expr_lineno(stmt));
    break;
  ...
  case GS_CLEANUP_STMT:
    ...
    if (!gs_cleanup_eh_only(stmt))
      ...
      Push_Scope_Cleanup(stmt, true /* cleanup_eh_only */);
    // Expand CLEANUP_BODY
...
static void WGEN_fixup_target_expr(gs_t retval) {
  ...
  // parm.
  gs_t ptr_var =
      gs_build_decl(GS_VAR_DECL, gs_build_pointer_type(gs_tree_type(retval)));
...
bool WGEN_has_copy_constructor(gs_t type) {
  if (gs_class_type_p(type) && gs_classtype_copy_constructor(type) != NULL)
    return TRUE;
...
//
void WGEN_Expand_Pragma(gs_t exp) {
#if 0 // wgen TODO
