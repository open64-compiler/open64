...
// TRACE_EXPAND_GS should be called at the top-level in a function which
// processes a GIMPLE node.  The macro is a bit of hackery to indent the lines
// according to the node's depth in the GIMPLE tree with minimal code changes to
// the wgen_XXX files.  It declares a WGEN_TRACE object whose constructor
// increments the static member gimple_depth (used for indenting) and prints out
// a GIMPLE node.  The destructor decrements gimple_depth.  So the number of
// WGEN_TRACE objects = WGEN_Expand recursion depth = indent amount should
// nicely track the current depth in GIMPLE tree depth.
//
// Caveats:
...
#define WN_CreateGotoOuterBlock(args...)                                       \
  WGEN_Trace_wn(WN_CreateGotoOuterBlock(args))
...
#define WN_CreateExcScopeBegin(args...)                                        \
  WGEN_Trace_wn(WN_CreateExcScopeBegin(args))
