...
void Init_Deferred_Function_Stack() {
...
void Push_Deferred_Function(gs_t decl) {
  ...
  // IDENTIFIER_POINTER(DECL_NAME(decl)));  fprintf(stderr,
  // "Push_Deferred_Function 2: %s\n", IDENTIFIER_POINTER(DECL_NAME(decl)));
  if (++deferred_function_i == deferred_function_max) {
    deferred_function_max = 2 * deferred_function_max;
    deferred_function_stack = (gs_t *)realloc(
        deferred_function_stack, deferred_function_max * sizeof(gs_t));
  }

  deferred_function_stack[deferred_function_i] = decl;
}

static gs_t Pop_Deferred_Function(void) {
  gs_t decl;
...
void Init_Deferred_Decl_Init_Stack() {
...
void Push_Deferred_Decl_Init(gs_t decl) {
  if (++deferred_decl_init_i == deferred_decl_init_max) {
    deferred_decl_init_max = 2 * deferred_decl_init_max;
    deferred_decl_init_stack = (gs_t *)realloc(
        deferred_decl_init_stack, deferred_decl_init_max * sizeof(gs_t));
  }

  deferred_decl_init_stack[deferred_decl_init_i] = decl;
} /* Push_Deferred_Decl_Init */

static gs_t Pop_Deferred_Decl_Init(void) {
  gs_t decl;
...
static void Set_Current_Function_Decl(gs_t decl) { curr_func_decl = decl; }

gs_t Current_Function_Decl(void) { return curr_func_decl; }
...
void template_substituted(gs_t orig, gs_t sub, gs_t func_decl) {
  tsubst_var in(orig, sub, func_decl);
  tsubst_map.push_back(in);
}

gs_t get_substituted(gs_t orig, gs_t func_decl) {
...
void push_mp_local_vars(gs_t decl) {
  Is_True(flag_openmp, ("Should not reach here without -mp"));
  mp_local_vars.push(decl);
}

gs_t pop_mp_local_vars(void) {
  Is_True(flag_openmp, ("Should not reach here without -mp"));
  if (mp_local_vars.empty())
    return NULL;
...
void gxx_emits_typeinfos(gs_t t) { emit_typeinfos.push_back(t); }

void defer_decl(gs_t t) { emit_decls.push_back(t); }

void defer_field(gs_t t, FLD_HANDLE fld) {
...
void defer_DST_type(gs_t t, TY_IDX ttidx, TY_IDX idx) {
...
void defer_DST_member_function(gs_t context, gs_t fndecl) {
...
static int WGEN_Expand_Function_Body(gs_t decl) {
  gs_t body;
  ...
  if ( // see if decl should be public
      ...
      gs_decl_inline(decl) && !flag_keep_inline_functions &&
...
extern "C" {
void WGEN_Whirlify_Decl(gs_t decl) {
  gs_code_t code = gs_tree_code(decl);
  switch (code) {
  case GS_FUNCTION_DECL:
    WGEN_Expand_Function_Body(decl);
    // Dump_DST(stdout); // wgen TODO: without this, the DST will has missing
    // subprograms
    break;
...
// adjustment.
static WN *WGEN_Adjust_Thunk(WN *wn, BOOL adjust_this, INT fixed_offset,
                             gs_t virtual_offset) // KEY version
{
  ST *ptr_st = WN_st(wn);
  ...
  if (adjust_this)
    wn =
        WN_Binary(OPR_ADD, ptr_mtype, wn, WN_Intconst(ptr_mtype, fixed_offset));
  if (virtual_offset != 0) {
    ...
    WN *deref = WN_CreateIload(
        OPR_ILOAD, TY_mtype(p_pdiff), TY_mtype(pp_pdiff), 0, p_pdiff, pp_pdiff,
        WN_Tas(TY_mtype(pp_pdiff), pp_pdiff, WN_COPY_Tree(wn)));
    ...
    wn = WN_Binary(OPR_ADD, ptr_mtype, wn,
                   WN_CreateIload(OPR_ILOAD, TY_mtype(pdiff), TY_mtype(p_pdiff),
...
//     return ret;
//     -----------
static void WGEN_Generate_Thunk(gs_t decl) {
#ifdef KEY
  if (expanded_decl(decl) == TRUE)
    return;
  expanded_decl(decl) = TRUE;
  ...
  Is_True(decl != NULL && gs_tree_code(decl) == GS_FUNCTION_DECL &&
  ...
  for (i = 0; i < nargs; i++) {
    ...
    arg_wn = WN_CreateParm(Mtype_comparison(arg_mtype), arg_wn, arg_ty,
                           WN_PARM_BY_VALUE);
  ...
  if (ret_mtype == MTYPE_V) {
    ...
    if (!adjust_this) {
      ...
      BOOL call_returns_ptr =
          gs_tree_code(gs_tree_type(gs_tree_type(thunk_target))) ==
          GS_POINTER_TYPE;
      if (call_returns_ptr) {

        // guard the result adjustment with a check for null-pointer.
        WN *test = WN_NE(ret_mtype, WN_Ldid(ret_mtype, 0, ret_st, ret_type),
...
//#define PRINT
  ...
  */
      ...
      if (body != NULL &&
          ((lang_cplus || lang_java) && !gs_decl_external(decl) ||
           !(lang_cplus || lang_java) &&
               (!c_omit_external || !gs_decl_external(decl) ||
        ...
        // if Enable_WFE_DFE is ON.
          ...
          if (CURRENT_SYMTAB != GLOBAL_SYMTAB ||
              ...
              strncmp(gs_identifier_pointer(gs_decl_name(decl)), "__tcf", 5) ==
                  0)
...
#endif
    gs_t subdecl;
    for (subdecl = gs_cp_namespace_decls(decl); subdecl != NULL;
  ...
  case GS_TEMPLATE_DECL: {
    ...
    if (gs_decl_context(decl) &&
        gs_tree_code(gs_decl_context(decl)) == GS_RECORD_TYPE)
      Get_TY(gs_decl_context(decl));
    gs_t gentemp = gs_most_general_template(decl);
    for (gs_t t = gs_decl_template_specializations(gentemp); t;
         t = gs_tree_chain(t))
    ...
    for (gs_t t = gs_decl_template_instantiations(gentemp); t;
         t = gs_tree_chain(t)) {
      gs_t val = gs_tree_value(t);
      if (gs_tree_code(val) == GS_RECORD_TYPE && !gs_uses_template_parms(val))
        Get_TY(val);
    }
  ...
  case GS_VAR_DECL:
    ...
    // emit the symbol, so this condition may refuse to emit some needed
    // symbols. But since we determine ourselves what symbols we NEED to emit,
    // this condition here should not harm. And this should be able to prevent
    // expansion of some really useless/unreferenced symbols.
    ...
    if (gs_decl_initial(decl) && !gs_decl_external(decl)) {
      ...
      if (gs_tree_code(init) == GS_CONSTRUCTOR) {
        ...
        if (init_type_code != GS_RECORD_TYPE &&
            init_type_code != GS_ARRAY_TYPE && init_type_code != GS_UNION_TYPE
#ifdef KEY
...
static BOOL function_has_varargs(gs_t fndecl) {
...
static void Setup_Entry_For_EH(void) {
  const int lbnd = 0;
  const int hbnd = 3;
  ...
  TY_IDX ty;
  TY_Init(New_TY(ty), (hbnd + 1) * 4, KIND_ARRAY, MTYPE_M,
          str); /// put attention here
  Set_TY_arb(ty, arb);
  ...
  INITV_Set_VAL(Initv_Table[exc_ptr_iv],
                Enter_tcon(Host_To_Targ(MTYPE_U4, ST_st_idx(exc_ptr_st))), 1);
  ...
  INITV_Set_VAL(Initv_Table[filter_iv],
                Enter_tcon(Host_To_Targ(MTYPE_U4, ST_st_idx(filter_st))), 1);
...
static void WGEN_Assemble_Asm(char *asm_string) {
  ...
  ST_Init(asm_st, Str_To_Index(Save_Str(asm_string), Global_Strtab), CLASS_NAME,
          SCLASS_UNKNOWN, EXPORT_LOCAL, (TY_IDX)0);

  Set_ST_asm_function_st(*asm_st);

  WN *func_wn = WN_CreateEntry(0, asm_st, WN_CreateBlock(), NULL, NULL);
...
static WN *WGEN_Start_Function(gs_t fndecl) {
  ...
  static BOOL interface_only =
      (!gs_pragma_implementation(program) || !gs_pragma_interface(program));
  ...
  if (lang_cplus || lang_java) { // czw
    if (interface_only && gs_decl_weak(fndecl) &&
  ...
  if (gs_decl_attributes(fndecl) &&
    ...
    gs_t nd;
    for (nd = gs_decl_attributes(fndecl); nd; nd = gs_tree_chain(nd)) {
      ...
      if (gs_tree_code(attr) == GS_IDENTIFIER_NODE) {
        ...
        else if (strcmp("always_inline", (char *)gs_identifier_pointer(attr)) ==
                 0)
          Set_PU_must_inline(Pu_Table[ST_pu(func_st)]);

        // bug 11754
        else if (strcmp("constructor", (char *)gs_identifier_pointer(attr)) ==
                     0 ||
                 strcmp("destructor", (char *)gs_identifier_pointer(attr)) ==
                     0) {
          ...
          ST_Init(init_st,
                  Save_Str2i(is_ctors ? "__ctors" : "__dtors", "_", ++__ctors),
  ...
  for (pdecl = thunk ? gs_decl_arguments(WGEN_get_final_thunk_target(fndecl))
                     : gs_decl_arguments(fndecl);
       pdecl; pdecl = gs_tree_chain(pdecl)) {
    TY_IDX arg_ty_idx = Get_TY(gs_tree_type(pdecl));
    ST *st;
    if (thunk) {
      st = New_ST();
      ST_Init(st, Save_Str2i(".arg", "", i), CLASS_VAR, SCLASS_FORMAL,
              EXPORT_LOCAL, arg_ty_idx);
    } else {
#ifdef KEY
      ...
      if (passed_type != nominal_type && gs_pointer_type_p(passed_type) &&
  ...
  if (function_has_varargs(fndecl) && !thunk) {
    ...
    for (pdecl = gs_decl_arguments(fndecl); pdecl;
      ...
      ST *arg_st = gs_tree_code(pdecl) == GS_INDIRECT_REF
                       ? Get_ST(gs_tree_operand(pdecl, 0))
                       : Get_ST(pdecl);
#else
  ...
  if (!thunk && gs_decl_global_ctor_p(fndecl)) {
    ...
    if (GLOBAL_INIT_PRIORITY(fndecl) != DEFAULT_INIT_PRIORITY) {
      DevWarn("Discarding ctor priority %d (default %d) at line %d",
              GLOBAL_INIT_PRIORITY(fndecl), DEFAULT_INIT_PRIORITY, lineno);
    }
#endif // !KEY
    ...
    Set_ST_addr_saved(func_st);
    ST *init_st = New_ST(GLOBAL_SYMTAB);
    ST_Init(init_st, Save_Str2i("__ctors", "_", ++__ctors), CLASS_VAR,
            SCLASS_FSTATIC, EXPORT_LOCAL,
            Make_Pointer_Type(ST_pu_type(func_st), FALSE));
    Set_ST_is_initialized(init_st);
    INITO_IDX inito = New_INITO(init_st, initv);
    ST_ATTR_IDX st_attr_idx;
  ...
  if (!thunk && gs_decl_global_dtor_p(fndecl)) {
    ...
    INITV_Init_Symoff(initv, func_st, 0, 1);
    ST *init_st = New_ST(GLOBAL_SYMTAB);
    ST_Init(init_st, Save_Str2i("__dtors", "_", ++__dtors), CLASS_VAR,
            SCLASS_FSTATIC, EXPORT_LOCAL,
            Make_Pointer_Type(ST_pu_type(func_st), FALSE));
    Set_ST_is_initialized(init_st);
    INITO_IDX inito = New_INITO(init_st, initv);
...
private:
  ...
  UINT Traverse_Aggregate_Struct(
      ...
      UINT
          array_elem_offset, // if struct is with an array, then it is the
...
public:
  ...
  AGGINIT(INITO_IDX inito) : _inito(inito) {
    _last_initv = 0;
    _not_root = FALSE;
  }
  ~AGGINIT(void) {}
  ...
  UINT Traverse_Aggregate_Constructor(
      ...
      UINT array_elem_offset, UINT field_id);
...
void AGGINIT::WGEN_Add_Aggregate_Init_Padding(INT size) {
  if (_inito == 0)
    return;
  if (size < 0)
    return; // actually happens from assemble_zeroes
...
void AGGINIT::WGEN_Add_Aggregate_Init_Integer(INT64 val, INT size) {
  if (_inito == 0)
    return;
  INITV_IDX inv = New_INITV();
  TYPE_ID mtype;
  if (size == 1)
    ...
    FmtAssert(FALSE, ("WGEN_Add_Aggregate_Init_Integer unexpected size"));
...
void AGGINIT::WGEN_Add_Init_Block(void) {
  if (_inito == 0)
    return;
...
void AGGINIT::WGEN_Add_Aggregate_Init_Real(gs_t real, INT size) {
  if (_inito == 0)
    return;
  INITV_IDX inv = New_INITV();
  TCON tc;
...
// for i386.
void AGGINIT::WGEN_Add_Aggregate_Init_Complex(gs_t rval, gs_t ival, INT size) {
  if (_inito == 0)
    return;
  INITV_IDX inv = New_INITV();
  TCON rtc;
...
void AGGINIT::WGEN_Add_Aggregate_Init_String(char *s, INT size) {
  if (_inito == 0)
    return;
  INITV_IDX inv = New_INITV();
...
void AGGINIT::WGEN_Add_Aggregate_Init_Symbol(ST *st, WN_OFFSET offset) {
  if (_inito == 0)
    return;
  INITV_IDX inv = New_INITV();
...
#ifdef TARG_IA64
void AGGINIT::Add_Aggregate_Init_Symiplt(ST *st, WN_OFFSET offset) {
  if (_inito == 0)
    return;
  INITV_IDX inv = New_INITV();
...
void AGGINIT::WGEN_Add_Aggregate_Init_Label(LABEL_IDX lab) {
...
void AGGINIT::WGEN_Add_Aggregate_Init_Address(gs_t init) {
  ST *st;
  ...
  case GS_STRING_CST: {
    TCON tcon = Host_To_Targ_String(
        MTYPE_STRING, const_cast<char *>(gs_tree_string_pointer(init)),
        gs_tree_string_length(init));
    ST *const_st = New_Const_Sym(Enter_tcon(tcon), Get_TY(gs_tree_type(init)));
  ...
  case GS_PLUS_EXPR:
    if (gs_tree_code(gs_tree_operand(init, 0)) == GS_ADDR_EXPR &&
      ...
      WGEN_Add_Aggregate_Init_Symbol(
          Get_ST(addr_kid), gs_get_integer_value(gs_tree_operand(init, 1)));
    } else {
  ...
  default: {
    ...
    if (WN_operator(wn) != OPR_ARRAY)
      ...
      if (const_indices) {
        ...
        FmtAssert(FALSE, ("cannot resolve to constant address in array"));
    }
    WN_Delete(init_wn);
  } break;
  }
...
#ifdef KEY // bug 10501
void AGGINIT::WGEN_Add_Aggregate_Init_Vector(gs_t init_list) {
  gs_t init;
  UINT vec_size;
  INT i;
  ...
  for (i = 0; init; init = gs_tree_chain(init), i++) {
    gs_t value = gs_tree_value(init);
#ifdef Is_True_On
    gs_t unit_type = gs_tree_type(value);
    Is_True(
        gs_tree_code(gs_type_size(unit_type)) == GS_INTEGER_CST,
        ("WGEN_Add_Aggregate_Init_Vector: Vector of variable-sized units?"));
    Is_True(esize ==
                gs_get_integer_value(gs_type_size(unit_type)) / BITSPERBYTE,
            ("WGEN_Add_Aggregate_Init_Vector: variable-sized elements"));
    Is_True(
        code == gs_tree_code(value),
    ...
    default:
      Fail_FmtAssertion(
          "WGEN_Add_Aggregate_Init_Vector: unexpected value of vector element");
    }
  }
  ...
  for (; i < nunits; i++) {
    ...
    case GS_REAL_CST: // mimick WGEN_Add_Aggregate_Init_Real()
    {
      ...
      default: // size 12 and 16 should not arise
        Fail_FmtAssertion("WGEN_Add_Aggregate_Init_Vector unexpected size %d",
                          esize);
        break;
      }
      INITV_Set_VAL(Initv_Table[inv], Enter_tcon(tc), 1);
...
/*
static BOOL Has_Non_Constant_Init_Value(gs_t init) {
  if (init == NULL) {
    return FALSE;
  }
  switch (gs_tree_code(init)) {
  ...
  case GS_NOP_EXPR:
    if (gs_tree_code(gs_tree_operand(init, 0)) == GS_ADDR_EXPR &&
        gs_tree_code(gs_tree_operand(gs_tree_operand(init, 0), 0)) ==
            GS_STRING_CST)
      return FALSE;
    else
      return TRUE;
...
static BOOL Use_Static_Init_For_Aggregate(ST *st, gs_t init) {
#ifdef KEY // bug 10507
  if (CURRENT_SYMTAB == GLOBAL_SYMTAB)
    return TRUE;
#endif
...
void AGGINIT::Add_Initv_For_Tree(gs_t val, UINT size) {
  switch (gs_tree_code(val)) {
  case GS_INTEGER_CST:
    WGEN_Add_Aggregate_Init_Integer(gs_get_integer_value(val), size);
    break;
  case GS_REAL_CST:
  ...
  case GS_NOP_EXPR:
    ...
    if (gs_tree_code(kid) == GS_ADDR_EXPR &&
      ...
      break;
    } else if (gs_tree_code(kid) == GS_INTEGER_CST) {
      WGEN_Add_Aggregate_Init_Integer(gs_get_integer_value(kid), size);
      break;
    }
    // fallthru
  default: {
    ...
    if (gs_tree_code(val) == GS_FDESC_EXPR && WN_operator(init_wn) == OPR_LDA) {
      Add_Aggregate_Init_Symiplt(WN_st(init_wn), WN_offset(init_wn));
      WN_DELETE_Tree(init_wn);
      break;
    }
#endif
    if (WN_operator(init_wn) == OPR_LDA_LABEL) {
      gs_t label_decl = (gs_tree_code(gs_tree_operand(val, 0)) == GS_ADDR_EXPR)
                            ? gs_tree_operand(gs_tree_operand(val, 0), 0)
                            : gs_tree_operand(val, 0);
      LABEL_IDX label_idx = WGEN_Get_LABEL(label_decl, FALSE);
      WGEN_Add_Aggregate_Init_Label(label_idx);
    } else
...
void AGGINIT::Add_Bitfield_Initv_For_Tree(gs_t val, FLD_HANDLE fld,
                                          INT &bytes) {
    ...
    if (ib.Nth_byte(bytes_out - 1) != 0) { // merge and change _last_initv
      if (INITV_kind(_last_initv) == INITVKIND_VAL) {
        ...
        if (INITV_kind(_last_initv) == INITVKIND_ONE)
          ...
          INITV_Init_Integer(_last_initv, MTYPE_I1, ib.Nth_byte(bytes_out - 1));
        }
      }
    }
  }
...
static void Gen_Assign_Of_Init_Val(ST *st, gs_t init, UINT offset,
                                   UINT array_elem_offset, TY_IDX ty,
                                   BOOL is_bit_field, UINT field_id,
                                   FLD_HANDLE fld, INT &bytes) {
#ifdef KEY
  ...
  if (TY_return_in_mem(ty) && gs_tree_code(init) == GS_CALL_EXPR) {
  ...
  if (gs_tree_code(init) == GS_STRING_CST && TY_kind(ty) == KIND_ARRAY) {
    ...
    WN *load_wn =
        WN_CreateMload(0, ptr_ty, init_wn,
#ifdef KEY // bug 3188
    ...
    WGEN_Stmt_Append(
        WN_CreateMstore(offset, ptr_ty, load_wn, addr_wn,
#ifdef KEY // bug 3188
                        WN_Intconst(MTYPE_I4, gs_tree_string_length(init))),
#else
                        WN_Intconst(MTYPE_I4, size)),
    ...
    if (size - gs_tree_string_length(init)) {
      ...
      WGEN_Stmt_Append(
          WN_CreateMstore(
              offset + gs_tree_string_length(init), ptr_ty, load_wn, addr_wn,
    ...
    WN *wn = WN_Stid(mtype, ST_ofst(st) + offset, st, ty, init_wn, field_id);
    WGEN_Stmt_Append(wn, Get_Srcpos());
    if (!is_bit_field)
      bytes += TY_size(ty);
    else {
...
void WGEN_Process_Initialization(gs_t exp) {
  INT emitted_bytes = 0;
  ...
  Gen_Assign_Of_Init_Val(st, init, 0, 0, ST_type(st), FALSE, 0, FLD_HANDLE(),
                         emitted_bytes);
}
...
void AGGINIT::Traverse_Aggregate_Pad(ST *st, BOOL gen_initv, UINT pad,
                                     UINT current_offset) {
  if (gen_initv) {
    ...
    WGEN_Stmt_Append(
        WN_CreateMstore(current_offset, mstore_ty, zero_wn, addr_wn, pad_wn),
        Get_Srcpos());
  }
...
void Traverse_Aggregate_Vector_Const(
    ST *st,         // symbol being initialized
    gs_t init_list, // list of initializers for units in vector
    BOOL gen_initv, // TRUE if initializing with INITV, FALSE for statements
    UINT current_offset) // offset from start of symbol for current vector
...
UINT AGGINIT::Traverse_Aggregate_Constructor(
    ST *st,         // symbol being initialized
    gs_t init_list, // list of initilaizers for this aggregate
#ifdef KEY
    gs_t struct_type, // type of top level struct
  ...
  if (TY_kind(ty) == KIND_STRUCT) {
    ...
    field_id = Traverse_Aggregate_Struct(st, init_list,
                                         ...
                                         struct_type,
#endif
                                         type, gen_initv, current_offset,
                                         array_elem_offset, field_id);
  }

  else if (TY_kind(ty) == KIND_ARRAY) {

    Traverse_Aggregate_Array(st, init_list, type, gen_initv,
                             current_offset); // czw
  }

#ifdef KEY // bug 9550
  else if (TY_kind(ty) == KIND_SCALAR && MTYPE_is_vector(TY_mtype(ty))) {

    Traverse_Aggregate_Vector(st, init_list, TY_mtype(ty), gen_initv,
                              current_offset);
  }
#endif
...
#ifdef KEY
  if (gen_initv &&
      _last_initv == 0) // for empty list; set to reserved value (bug 961)
...
void AGGINIT::Add_Init_For_WHIRL(WN *init_wn, UINT size, INT64 ofst) {
...
void AGGINIT::Add_Inito_For_Tree(gs_t init, ST *st) {
  gs_t kid;
  gs_code_t code;
  ...
  case GS_COMPLEX_CST:
    _inito = New_INITO(st);
    _not_root = FALSE;
    WGEN_Add_Aggregate_Init_Complex(
        gs_tree_realpart(init), gs_tree_imagpart(init), TY_size(ST_type(st)));
    return;
  case GS_STRING_CST:
    _inito = New_INITO(st);
    _not_root = FALSE;
  ...
  case GS_ADDR_EXPR:
    ...
    if (code == GS_VAR_DECL || code == GS_FUNCTION_DECL ||
        code == GS_STRING_CST || code == GS_LABEL_DECL ||
  ...
  case GS_PLUS_EXPR:
    ...
    if (gs_tree_code(kid) == GS_ADDR_EXPR) {
      ...
      if (INITV_kind(_last_initv) == INITVKIND_SYMOFF &&
        ...
        Set_INITV_ofst(_last_initv, gs_get_integer_value(kid));
        return;
      }
    }
    break;
  ...
  _not_root = FALSE;
  if (WN_operator(init_wn) == OPR_LDA_LABEL) {
    gs_t label_decl = (gs_tree_code(gs_tree_operand(init, 0)) == GS_ADDR_EXPR)
                          ? gs_tree_operand(gs_tree_operand(init, 0), 0)
                          : gs_tree_operand(init, 0);
    LABEL_IDX label_idx = WGEN_Get_LABEL(label_decl, FALSE);
    WGEN_Add_Aggregate_Init_Label(label_idx);
  } else
    Add_Init_For_WHIRL(init_wn, TY_size(ST_type(st)), 0);
  WN_DELETE_Tree(init_wn);
}
...
extern ST *WGEN_Generate_Temp_For_Initialized_Aggregate(gs_t init, char *name) {
  ...
  ST_Init(temp, Save_Str2(name, ".init"), CLASS_VAR, SCLASS_PSTATIC,
          EXPORT_LOCAL, ty_idx);
  AGGINIT agginit;
  gs_code_t code = gs_tree_code(init);
  if (code == GS_CONSTRUCTOR && !Use_Static_Init_For_Aggregate(temp, init)) {
    ...
    if (TY_size(ty) == 0 ||
        (TY_kind(ty) == KIND_ARRAY && !ARB_const_ubnd(TY_arb(ty)) &&
      ...
      if (TY_kind(ty) == KIND_ARRAY) {
        Set_ARB_const_ubnd(TY_arb(ty));
        Set_ARB_ubnd_val(TY_arb(ty), (TY_size(ty) / TY_size(TY_etype(ty))) - 1);
      }
    }
...
extern void WGEN_Initialize_Decl(gs_t decl) {
  ...
  ST *st = Get_ST(decl);
  gs_t init = gs_decl_initial(decl);
  if (gs_tree_code(init) == GS_VAR_DECL && gs_decl_context(init) &&
  ...
  if (gs_tree_static(decl) || gs_decl_context(decl) == NULL) {
    ...
    if (gs_tree_code(init) == GS_CONSTRUCTOR) {
      ...
      if (Use_Static_Init_For_Aggregate(st, init)) {
        ...
        ST *copy =
            WGEN_Generate_Temp_For_Initialized_Aggregate(init, ST_name(st));
        WN *init_wn =
            WN_CreateLdid(OPR_LDID, MTYPE_M, MTYPE_M, 0, copy, ST_type(copy));
        WGEN_Stmt_Append(WN_CreateStid(OPR_STID, MTYPE_V, MTYPE_M, 0, st,
                                       ST_type(st), init_wn),
                         Get_Srcpos());
      } else {
      ...
      INT emitted_bytes;
      Gen_Assign_Of_Init_Val(st, init, 0 /*offset*/, 0 /*array_elem_offset*/,
                             ST_type(st), FALSE, 0 /*field_id*/, FLD_HANDLE(),
                             emitted_bytes);
    }
  }
}
...
ST *WGEN_Get_Return_Address_ST(int level) {
...
ST *WGEN_Alloca_ST(gs_t decl) {
  ST *st = Create_ST_For_Tree(decl);
  ST *alloca_st = New_ST(CURRENT_SYMTAB);
  ST_Init(alloca_st, Save_Str(ST_name(st)), CLASS_VAR, SCLASS_AUTO,
          EXPORT_LOCAL, Make_Pointer_Type(ST_type(st), FALSE));
...
void WGEN_Dealloca(ST *alloca_st, vector<ST *> *vars) {
  int nkids = vars->size();
...
void WGEN_Resolve_Duplicate_Decls(gs_t olddecl, gs_t newdecl) {
  ST *st = DECL_ST(olddecl);
  gs_t newtype = gs_tree_type(newdecl);
  gs_t newsize = gs_type_size(newtype);
  TY_IDX ty = ST_type(st);

  if (gs_tree_static(olddecl) == FALSE && gs_tree_static(newdecl) == TRUE &&
      gs_tree_public(olddecl) == TRUE && gs_tree_public(newdecl) == FALSE) {
    Set_ST_sclass(st, SCLASS_FSTATIC);
    Set_ST_export(st, EXPORT_LOCAL);
  }

  if (newsize && gs_tree_code(newsize) == GS_INTEGER_CST &&
...
extern "C" void WGEN_Weak_Finish() {
...
void WGEN_Process_Class_Decl(gs_t decl) {
// fprintf(stderr, "CLASS_DECL: %s\n", IDENTIFIER_POINTER(gs_decl_name(decl)));
  ...
  gs_t field;
  for (field = get_first_real_or_virtual_field(decl); field != NULL;
       field = next_real_field(decl, field)) {
    if (gs_tree_code(field) == GS_TYPE_DECL) {
      gs_t field_type = gs_tree_type(field);
      if (field_type && gs_tree_code(field_type) == GS_RECORD_TYPE &&
          field_type != decl) {
  ...
  while (method != NULL) {
    if (gs_tree_code(method) == GS_FUNCTION_DECL) {
      ...
      if (body != NULL && !gs_decl_external(method) && !gs_decl_weak(method) &&
          !gs_decl_inline(method) && DECL_ST(method) == NULL &&
...
void WGEN_Process_Type_Decl(gs_t decl) {
#if 0 // wgen TODO
...
void WGEN_Process_Template_Decl(gs_t decl) {
...
bool decl_is_needed_vtt(gs_t decl) {
  bool needed = false;

  // Assume all VTTs are needed.
  if (gs_decl_name(decl) && gs_identifier_pointer(gs_decl_name(decl)) &&
...
void WGEN_Process_Var_Decl(gs_t decl) {
  ...
  if (gs_tree_public(decl) &&
    ...
    if (!gs_decl_weak(decl) || decl_is_needed_vtable(decl)
#ifdef KEY
...
void WGEN_Process_Function_Decl(gs_t decl) {
  // fprintf(stderr, "FUNCTION_DECL: %s\n",
  // IDENTIFIER_POINTER(gs_decl_name(decl)));
  gs_t body;
  ST *st;
  body = gs_decl_saved_tree(decl);
  if (body != NULL && !gs_decl_external(decl) && !gs_decl_artificial(decl) &&
      !gs_decl_inline(decl) && DECL_ST(decl) == NULL &&
...
void WGEN_Process_Decl(gs_t decl) {
  switch (gs_tree_code(decl)) {
...
void WGEN_Process_Namespace_Decl(gs_t namespace_decl) {
#if 0 // wgen TODO
...
extern "C" void WGEN_Expand_Emitted_Decl() // czw
{
  gs_t list;
        ...
        if (Debug_Level > 0) {
          ...
          if (context && gs_tree_code(context) == GS_RECORD_TYPE) {
            ...
            if (gs_identifier_ctor_or_dtor_p(gs_decl_name(decl)))
              ...
              skip = gs_identifier_ctor_or_dtor_p(gs_decl_name(target_func));
            }
            if (!skip) {
...
extern "C" void WGEN_Expand_Top_Level_Decl(gs_t top_level_decl) {
  int error_count, sorry_count;
#ifdef KEY
  gs_t old_namespace_decl = curr_namespace_decl;
...
#ifdef KEY
extern "C" void WGEN_Expand_Defers() {
  {
    int i;
    ...
    for (i = 0; i < emit_decls.size(); i++) {
      ...
      if (code == GS_VAR_DECL)
        WGEN_Expand_Decl(decl, TRUE);
      else if (code == GS_RECORD_TYPE || code == GS_UNION_TYPE)
        Get_TY(decl);
      else
...
static inline gs_t WGEN_get_thunk_target(gs_t decl) {
...
// forward.
static inline gs_t WGEN_get_final_thunk_target(gs_t decl) // KEY
{
...
static void WGEN_Handle_Named_Return_Value(gs_t fn) {
  ...
  FmtAssert(
      gs_tree_code(named_ret_obj) == GS_VAR_DECL,
      ("WGEN_Handle_Named_Return_Value: named return object not a VAR_DECL"));
  FmtAssert(
      actual_ret_obj && gs_tree_code(actual_ret_obj) == GS_RESULT_DECL,
  ...
  if (gs_decl_initial(actual_ret_obj)) {
    ...
    named_ret_obj_initializer =
        gs_build_target_expr(gs_tree_type(actual_ret_obj), actual_ret_obj,
                             gs_decl_initial(actual_ret_obj), NULL);
    gs_t flags = gs_operand(named_ret_obj_initializer, GS_FLAGS);
    _gs_bv(flags, GS_TREE_SIDE_EFFECTS, 1);
  }

  gs_t ptr_var = gs_build_decl(
      GS_RESULT_DECL, gs_build_pointer_type(gs_tree_type(gs_tree_type(fn))));
