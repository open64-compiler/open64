...
extern WN *WGEN_Expand_Expr(gs_t exp, bool need_result = TRUE,
                            TY_IDX nop_ty_idx = 0, TY_IDX component_ty_idx = 0,
                            INT64 component_offset = 0, UINT32 field_id = 0,
 ...
 */
extern WN *WGEN_Lhs_Of_Modify_Expr(
    gs_code_t assign_code, gs_t lhs,
    /* if lhs contains CALL, promote the CALL and pass its return value by
       lhs_retval */
    /* only used in f()=g()=h(); so far */
    WN *lhs_retval, bool need_result, TY_IDX component_ty_idx,
    INT64 component_offset, UINT32 field_id, bool is_bit_field, WN *rhs_wn,
    PREG_NUM rhs_preg_num, bool is_realpart, bool is_imagpart);
