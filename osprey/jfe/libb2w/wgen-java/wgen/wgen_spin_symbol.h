 ...
 * or create a new one.
 */
inline TY_IDX Get_TY(gs_t type_tree) {
#ifdef KEY
    ...
    if (gs_tree_code(type_tree) == GS_RECORD_TYPE ||
      ...
      if (elt_fld.Is_Null() && TY_content_seen(idx) == 0)
        ...
        return idx;
             ...
             (TY_is_incomplete(idx) || (TY_kind(idx) == KIND_POINTER &&
 ...
 */
inline ST *Get_ST(gs_t decl_tree) {
  ST *st = DECL_ST(decl_tree);

  /* If the st is null, we try to create it, so that we can handle its section
   * name. See OSP_133
   */
  if (st == NULL)
  ...
  if (st != NULL) {
    if (gs_tree_code(decl_tree) == GS_VAR_DECL &&
        ST_sclass(st) == SCLASS_EXTERN && !ST_is_weak_symbol(st) &&
        !gs_decl_external(decl_tree) && !gs_decl_initial(decl_tree)
#if 1 // wgen, bug 10324
    ...
    if (gs_tree_code(decl_tree) == GS_VAR_DECL &&
        (ST_sclass(st) == SCLASS_EXTERN || ST_sclass(st) == SCLASS_FSTATIC ||
         ST_sclass(st) == SCLASS_COMMON || ST_sclass(st) == SCLASS_UGLOBAL ||
         ST_sclass(st) == SCLASS_DGLOBAL) &&
        !ST_is_weak_symbol(st) && !gs_decl_external(decl_tree) &&
        gs_tree_static(decl_tree) && gs_decl_section_name(decl_tree)) {
      ...
      if (st_attr_idx) {
        STR_IDX str_index =
            ST_ATTR_section_name(St_Attr_Table(ST_IDX_level(idx), st_attr_idx));
        ...
        if (strcmp(Index_To_Str(str_index),
          ...
          Set_ST_ATTR_section_name(
              St_Attr_Table(ST_IDX_level(idx), st_attr_idx),
              Save_Str(
                  gs_tree_string_pointer(gs_decl_section_name(decl_tree))));
      }
      // bug fix for OSP_136
       ...
       * attribute, create one for current tree node */
        ...
        ST_ATTR_Init(
            st_attr, idx, ST_ATTR_SECTION_NAME,
  ...
  if ((CURRENT_SYMTAB > GLOBAL_SYMTAB + 1) &&
      ...
      (ST_level(st) < CURRENT_SYMTAB) && (ST_level(st) > GLOBAL_SYMTAB)) {
