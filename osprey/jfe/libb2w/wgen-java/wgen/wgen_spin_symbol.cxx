...
static ST *get_duplicate_st(gs_t decl) {
  int count = duplicate_of.count(decl);
...
gs_t get_first_real_or_virtual_field(gs_t type_tree) {
  ...
  if ((lang_cplus || lang_java) && gs_type_fields(type_tree) &&
      gs_type_vfield(type_tree)) // czw
...
gs_t get_first_real_field(gs_t type_tree) {
  ...
  if (field == gs_type_vfield(type_tree)) {
    Is_True((lang_cplus || lang_java),
            ("get_first_real_field: TYPE_VFIELD used for C")); // czw
...
gs_t next_real_field(gs_t type_tree, gs_t field) {
  ...
  if (field == gs_type_vfield(type_tree)) {
    Is_True((lang_cplus || lang_java),
            ("next_real_field: TYPE_VFIELD used for C")); // czw
...
size_t Type_Size_Without_Vbases(gs_t type_tree) {
  ...
  for (field = get_first_real_or_virtual_field(type_tree); field;
  ...
  return gs_get_integer_value(gs_decl_field_offset(last_field_decl)) +
         gs_get_integer_value(gs_decl_field_bit_offset(last_field_decl)) /
             BITSPERBYTE +
...
bool lookup_attribute(char *attr_name, gs_t attr_list) {
  gs_t nd;
...
extern TY_IDX Create_TY_For_Tree(gs_t type_tree, TY_IDX idx) {
  ...
  if (gs_type_name(type_tree) && idx == 0 &&
...
#else
      DST_INFO_IDX dst = Create_DST_type_For_Tree(type_tree, idx, orig_idx);
  ...
  if (type_size == NULL) {
    ...
    FmtAssert(gs_tree_code(type_tree) == GS_ARRAY_TYPE ||
              ...
              ("Create_TY_For_Tree: type_size NULL for non ARRAY/RECORD/VOID, "
               "type is %d",
...
#ifdef KEY // bug 3045
      tsize = (gs_get_integer_value(type_size) + BITSPERBYTE - 1) / BITSPERBYTE;
#else
...
#endif /* KEY */
    default:
      FmtAssert(FALSE, ("Get_TY unexpected size %d", tsize));
    }
  ...
  case GS_COMPLEX_TYPE:
    ...
    case 2:
    case 4:
      ErrMsg(EC_Unsupported_Type, "Complex integer");
    case 8:
      mtype = MTYPE_C4;
      break;
    case 16:
  ...
  case GS_REFERENCE_TYPE:
    ...
    Set_TY_align(idx, align);
    break;
  case GS_ARRAY_TYPE: { // new scope for local vars
#ifdef KEY              /* bug 8346 */
    ...
    if (gs_tree_code(gs_type_size(gs_tree_type(type_tree))) == GS_INTEGER_CST) {
      ...
      Set_ARB_stride_val(
          arb, gs_get_integer_value(gs_type_size(gs_tree_type(type_tree))) /
                   BITSPERBYTE);
    }
#ifdef KEY /* bug 8346 */
    else if (!expanding_function_definition && processing_function_prototype) {
      Set_ARB_const_stride(arb);
      // dummy stride val 4
      ...
      if (WN_opcode(swn) == OPC_U4I4CVT || WN_opcode(swn) == OPC_U8I8CVT) {
        swn = WN_kid0(swn);
      }
#ifdef KEY
      ...
      if (WN_operator(swn) != OPR_LDID) {
        TY_IDX ty_idx = Get_TY(gs_tree_type(type_size));
      ...
      if (!gs_type_max_value(gs_type_domain(type_tree))) {
        ...
        Set_ARB_ubnd_val(arb, 0xffffffff);
          ...
          if (gs_tree_code(gs_type_max_value(gs_type_domain(type_tree))) ==
              GS_INTEGER_CST) {
        Set_ARB_const_ubnd(arb);
        Set_ARB_ubnd_val(arb, gs_get_integer_value(gs_type_max_value(
                                  gs_type_domain(type_tree))));
      }
#ifdef KEY /* bug 8346 */
        ...
        WN *uwn =
            WGEN_Expand_Expr(gs_type_max_value(gs_type_domain(type_tree)));
        if (WN_opcode(uwn) == OPC_U4I4CVT || WN_opcode(uwn) == OPC_U8I8CVT) {
          uwn = WN_kid0(uwn);
        }
        ST *st;
        TY_IDX ty_idx;
        ...
        if (WN_operator(uwn) != OPR_LDID) {
          ty_idx = Get_TY(
              gs_tree_type(gs_type_max_value(gs_type_domain(type_tree))));
    ...
    if (variable_size) {
#ifdef KEY /* bug 8346 */
      if (!expanding_function_definition && processing_function_prototype) {
        Set_TY_is_incomplete(idx);
      } else
#endif
      {
  ...
  break;
  case GS_RECORD_TYPE:
  case GS_UNION_TYPE: { // new scope for local vars
    ...
    if (tsize != 0 &&
        ...
        gs_classtype_as_base(type_tree) && gs_classtype_size(type_tree) &&
  ...
  case GS_FUNCTION_TYPE: { // new scope for local vars
    ...
    for (arg = gs_type_arg_types(type_tree); arg; arg = gs_tree_chain(arg)) {
    ...
    for (num_args = 0, arg = gs_type_arg_types(type_tree); arg;
         num_args++, arg = gs_tree_chain(arg)) {
      arg_ty_idx = Get_TY(gs_tree_value(arg));
      Is_True(!TY_is_incomplete(arg_ty_idx) || TY_is_incomplete(idx),
      ...
      if (!WGEN_Keep_Zero_Length_Structs && TY_mtype(arg_ty_idx) == MTYPE_M &&
        ...
        DevWarn(
            "zero length struct encountered in function prototype at line %d",
            lineno);
      } else
  ...
  if (Debug_Level >= 2) {
    ...
    if (gs_tree_code(type_tree) != GS_RECORD_TYPE &&
        ...
        !(TY_kind(idx) == KIND_POINTER && TY_is_incomplete(TY_pointed(idx)))) {
...
ST *Create_ST_For_Tree(gs_t decl_node) {
  ...
  case GS_FUNCTION_DECL: {
    if (Enable_WFE_DFE) {
      ...
      /*
                ...
                if (DECL_TINFO_FN_P(decl_node))
                  Push_Deferred_Function (decl_node);
      */
      else if (body != NULL && !gs_decl_external(decl_node) &&
    ...
    if (gs_decl_assembler_name(decl_node) == NULL)
      ...
      p = gs_identifier_pointer(gs_decl_assembler_name(decl_node));
  ...
  case GS_VAR_DECL: {
      ...
      if (gs_decl_context(decl_node) == 0 ||
        ...
        if (gs_tree_public(decl_node)) {
          ...
          if (gs_decl_external(decl_node) ||
            ...
            sclass = SCLASS_EXTERN;
          else if (gs_decl_initial(decl_node))
        ...
        if (section_name && !strncmp(gs_tree_string_pointer(section_name),
                                     ".gnu.linkonce.", 14)) {
          if (!strncmp(gs_tree_string_pointer(section_name), ".gnu.linkonce.b.",
                       16)) {
            sclass = SCLASS_UGLOBAL;
            level = GLOBAL_SYMTAB;
            eclass = EXPORT_PREEMPTIBLE;
        ...
        // Bug 8652: If GNU marks it as COMMON, we should the same.
                 ...
                 gs_decl_common(decl_node) && gs_tree_public(decl_node)) {
          sclass = SCLASS_COMMON;
          level = GLOBAL_SYMTAB;
          eclass = EXPORT_PREEMPTIBLE;
        }
          ...
          if (gs_tree_static(decl_node)) {
...
#ifdef KEY // bug 12668
                &&
                !(gs_decl_initial(decl_node) && !gs_decl_external(decl_node) &&
            ...
            sclass = SCLASS_AUTO;
            level = DECL_SYMTAB_IDX(decl_node) ? DECL_SYMTAB_IDX(decl_node)
                                               : CURRENT_SYMTAB;
          }
          eclass = EXPORT_LOCAL;
        }
    ...
    if (TY_mtype(ty_idx) == MTYPE_M && Aggregate_Alignment > 0 &&
    ...
    if (gs_tree_code(decl_node) == GS_VAR_DECL &&
        gs_decl_threadprivate(decl_node))
    ...
    if (gs_decl_size_unit(decl_node) &&
      ...
      ST_Init(alloca_st, Save_Str(name), CLASS_VAR, SCLASS_AUTO, EXPORT_LOCAL,
  ...
  if (gs_tree_code(decl_node) == GS_VAR_DECL &&
      gs_decl_asmreg(decl_node) != 0) {
    extern PREG_NUM
        Map_Reg_To_Preg[]; // defined in common/com/arch/config_targ.cxx
    int reg = gs_decl_asmreg(decl_node) - 1;
    PREG_NUM preg = Map_Reg_To_Preg[reg];
    FmtAssert(preg >= 0, ("mapping register %d to preg failed\n", reg));
    TY_IDX ty_idx = ST_type(st);
    Set_TY_is_volatile(ty_idx);
  ...
  if (gs_tree_code(decl_node) == GS_VAR_DECL && gs_decl_context(decl_node) &&
      gs_tree_code(gs_decl_context(decl_node)) == GS_RECORD_TYPE)
    Get_TY(gs_decl_context(decl_node));

  if (Enable_WFE_DFE) {
    if (gs_tree_code(decl_node) == GS_VAR_DECL && level == GLOBAL_SYMTAB &&
        !gs_decl_external(decl_node) && gs_decl_initial(decl_node)) {
      Push_Deferred_Function(decl_node);
    }
  }
  ...
  if (lang_cplus && gs_decl_section_name(decl_node)) {
    ...
    ST_ATTR &st_attr = New_ST_ATTR(level, st_attr_idx);
    ST_ATTR_Init(
        st_attr, ST_st_idx(st), ST_ATTR_SECTION_NAME,
  ...
  if (gs_tree_code(decl_node) == GS_FUNCTION_DECL) {
    ...
    gs_t type_attr;
    for (type_attr = type_attr_list; type_attr;
         type_attr = gs_tree_chain(type_attr)) {

      if (gs_tree_purpose(type_attr) != NULL &&
          gs_code(gs_tree_purpose(type_attr)) == GS_IDENTIFIER_NODE) {
        const char *attr_name =
            gs_tree_string_pointer(gs_tree_purpose(type_attr));
      ...
      (gs_tree_code(t) == GS_PARM_DECL ||
       (gs_tree_code(t) == GS_VAR_DECL &&
        (ST_sclass(st) == SCLASS_AUTO ||
         (!pstatic_as_global && ST_sclass(st) == SCLASS_PSTATIC))))) {
...
DST_INFO_IDX &DECL_DST_IDX(gs_t t) {
  hash_map<gs_t, DST_INFO_IDX, ptrhash>::iterator it = decl_idx_map.find(t);
...
DST_INFO_IDX &TYPE_DST_IDX(gs_t t) {
  hash_map<gs_t, DST_INFO_IDX, ptrhash>::iterator it = type_idx_map.find(t);
