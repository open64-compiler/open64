...
#ifdef KEY
static DST_accessibility get_dwarf_access(gs_t t) {
...
static UINT Get_Dir_Dst_Info(char *name) {
  ...
  for (found = dir_dst_list.begin(); found != dir_dst_list.end(); ++found) {
...
static UINT Get_File_Dst_Info(char *name, UINT dir) {
  ...
  std::vector<std::pair<char *, UINT> >::iterator found;
  // assume linear search is okay cause list will be small?
  for (found = file_dst_list.begin(); found != file_dst_list.end(); ++found) {
    if (strcmp((*found).first, name) == 0) {
      return (*found).second;
    }
  }
...
static void DST_enter_file(char *file_name, UINT dir) {
  UINT64 file_size = 0;
  UINT64 fmod_time = 0;
  struct stat fstat;
  ...
  DST_mk_file_name(file_name, dir, file_size, fmod_time);
}
...
// the DECL dst record.
//
static void DST_enter_static_data_mem(gs_t parent_tree, DST_INFO_IDX parent_idx,
                                      TY_IDX parent_ty_idx, gs_t field) {
  /* cannot be a bit field */
  int isbit = 0;
  DST_INFO_IDX field_idx = DST_INVALID_INIT;

  char *linkage_name = gs_identifier_pointer(gs_decl_assembler_name(field));
...
void
#endif
DST_enter_member_function(gs_t parent_tree, DST_INFO_IDX parent_idx,
                          TY_IDX parent_ty_idx, gs_t fndecl) {
  USRCPOS src;
  USRCPOS_srcpos(src) = Get_Srcpos();
...
#else
  char *basename = gs_identifier_opname_p(gs_decl_name(fndecl))
                       ? 0
                       : gs_identifier_pointer(gs_decl_name(fndecl));
#endif
  char *linkage_name = gs_identifier_pointer(gs_decl_assembler_name(fndecl));
#ifdef KEY
  // Bug 3846
  if (gs_identifier_opname_p(gs_decl_name(fndecl)) &&
      gs_identifier_typename_p(gs_decl_name(fndecl))) {
    basename =
        cplus_demangle(linkage_name, DMGL_PARAMS | DMGL_ANSI | DMGL_TYPES);
    if (basename) {
  ...
  DST_virtuality virtuality = gs_decl_pure_virtual_p(fndecl)
  ...
  if (is_prototyped) {
    ...
    if (!parms) {
      ...
      DST_enter_param_vars(fndecl, dst, parms, is_abstract_root,
...
static void DST_enter_normal_field(gs_t parent_tree, DST_INFO_IDX parent_idx,
                                   TY_IDX parent_ty_idx, gs_t field) {
  char isbit = 0;
...
#else
  // Bug 1271
  INT fld_offset_bytes =
      gs_get_integer_value(gs_decl_field_offset(field)) + bitoff / BITSPERBYTE;
#endif
  ...
  if (isbit == 0) {
  //  currentcontainer = -1 ;                    // invalidate bitfield
  //  calculation
#ifndef KEY
    field_idx = DST_mk_member(src, field_name,
                              fidx,             // field type
...
//***********************************************************
    ...
    if (Target_Byte_Sex != BIG_ENDIAN) {
      into_cont_off =
          tsize * BITSPERBYTE - into_cont_off; // reset current offset
      into_cont_off -=
          gs_get_integer_value(gs_decl_size(field)); // start at MSB
    }
#endif
...
static void DST_enter_struct_union_members(gs_t parent_tree,
  ...
  for (; field; field = gs_tree_chain(field)) {
    if (gs_tree_code(field) == GS_FIELD_DECL) {
      DST_enter_normal_field(parent_tree, parent_idx, parent_ty_idx, field);
    } else if (gs_tree_code(field) == GS_VAR_DECL) {
      // Here create static class data mem decls
      // These cannot be definitions.
      DST_enter_static_data_mem(parent_tree, parent_idx, parent_ty_idx, field);
  ...
  if (context != NULL) // bug 10483
    if (gs_tree_code(context) != GS_TYPE_DECL || !gs_decl_context(context) ||
  ...
  for (; methods != NULL; methods = gs_tree_chain(methods)) {
    ...
    // in particular, operator= is there.  We do want to omit the __base_ctor
    // stuff though
#ifndef KEY
      ...
      if (gs_identifier_ctor_or_dtor_p(gs_decl_name(methods))) {
        ...
        DST_enter_member_function(parent_tree, parent_idx, parent_ty_idx,
                                  methods);
      }
    }
  }
...
// and enter it.
static DST_INFO_IDX DST_enter_struct_union(gs_t type_tree, TY_IDX ttidx,
                                           TY_IDX idx, INT tsize) {
#ifdef KEY
  ...
  if (DST_IS_NULL(dst_idx)) {
    ...
    if ((type_binfo = gs_type_binfo(type_tree)) != NULL &&
      ...
      for (list = basetypes; gs_code(list) != EMPTY;
        ...
        DST_INFO_IDX bcidx = Create_DST_type_For_Tree(basetype, itx, idx);

#ifndef KEY
        // There is no way to pass in DW_ACCESS_* here.
        // That is am omission.  FIX
        DST_INFO_IDX inhx = DST_mk_inheritance(src, bcidx, virtuality, offset);
#else
        DST_INFO_IDX inhx = DST_mk_inheritance(
            src, bcidx, virtuality,
            // Bug 1737 - handle virtual base classes
            virtuality
                ? // bug 11555: gs_tree_int_cst_low -> gs_get_integer_value
...
// We have a enum.
// and enter it.
static DST_INFO_IDX DST_enter_enum(gs_t type_tree, TY_IDX ttidx, TY_IDX idx,
                                   INT tsize) {
  DST_INFO_IDX dst_idx = TYPE_DST_IDX(type_tree);
  ...
  DST_INFO_IDX current_scope_idx = DST_get_context(gs_type_context(type_tree));
  ...
  if (DST_IS_NULL(dst_idx)) {
    ...
    t_dst_idx = DST_mk_enumeration_type(src, name1,
                                        e_tsize,               // Type size.
                                        DST_INVALID_IDX,       // Not inlined.
    ...
    if (tsize == 8) {
      ...
      // ???
      DST_CONST_VALUE_form(enumerator) = DST_FORM_DATA4;
      DevWarn("Unexpected type size  %d in enumerator", (int)tsize);
    }
    ...
    for (; enum_entry; enum_entry = gs_tree_chain(enum_entry)) {
      ...
      USRCPOS_clear(src);
#endif // KEY
      char *name2 = gs_identifier_pointer(gs_tree_purpose(enum_entry));
      ...
      DST_INFO_IDX ed = DST_mk_enumerator(src, name2, enumerator);
 ...
 *===================================================
 */
static DST_INFO_IDX DST_create_basetype(TY_IDX ty) {
  TYPE_ID bt;
  DST_INFO_IDX i;
  ...
  i = DST_mk_basetype(ate_types[bt].name, ate_types[bt].encoding,
                      ate_types[bt].size);
...
// We have a subrange
// enter it.
static DST_INFO_IDX DST_enter_subrange_type(ARB_HANDLE ar) {
  DST_INFO_IDX i;
  DST_cval_ref lb, ub;
  ...
  if (const_lb)
    ...
    lb.ref = DST_mk_variable(src, // srcpos
                             ST_name(var_st), type, 0,
                             (void *)(INTPTR)ST_st_idx(var_st), DST_INVALID_IDX,
                             FALSE, // is_declaration
  ...
  if (const_ub)
    ...
    type = DST_create_basetype(ST_type(var_st));
    ub.ref = DST_mk_variable(src, // srcpos
                             ST_name(var_st), type, 0,
                             (void *)(INTPTR)ST_st_idx(var_st), DST_INVALID_IDX,
                             FALSE, // is_declaration
                             ST_sclass(var_st) == SCLASS_AUTO,
  ...
  i = DST_mk_subrange_type(const_lb, lb, const_ub, ub);
...
// We have an array
// enter it.
static DST_INFO_IDX DST_enter_array_type(gs_t type_tree, TY_IDX ttidx,
                                         TY_IDX idx, INT tsize) {
  DST_INFO_IDX dst_idx = TYPE_DST_IDX(type_tree);
  ...
  if (DST_IS_NULL(dst_idx)) {
    ...
    DST_INFO_IDX inner_dst = Create_DST_type_For_Tree(elt_tree, itx, idx);
...
// bug 11554: use is_typedef to check whether or not it is a typedef type
  ...
  DST_INFO_IDX mdst = Create_DST_type_For_Tree(member_type, midx, orig_idx);
  ...
  DST_INFO_IDX container_dst =
      Create_DST_type_For_Tree(base_type, container_idx, orig_idx);
...
Create_DST_type_For_Tree(gs_t type_tree, TY_IDX ttidx, TY_IDX idx,
  ...
  if (type_size == NULL) {
    ...
    // incomplete structs have 0 size
    FmtAssert(
        gs_tree_code(type_tree) == GS_ARRAY_TYPE ||
            gs_tree_code(type_tree) == GS_UNION_TYPE ||
            gs_tree_code(type_tree) == GS_RECORD_TYPE ||
  ...
  if (!ignoreconst && gs_type_readonly(type_tree)) {
    ...
    if (DST_IS_NULL(dst_idx)) {
      ...
      DST_INFO_IDX unqual_dst =
          Create_DST_type_For_Tree(type_tree, itx, idx, true, false);
  ...
  if (!ignorevolatile && gs_type_volatile(type_tree)) {
    ...
    if (DST_IS_NULL(dst_idx)) {
      TY_IDX itx = TYPE_TY_IDX(type_tree);
      DST_INFO_IDX unqual_dst =
          Create_DST_type_For_Tree(type_tree, itx, idx, true, true);
  ...
  // enter base type
    ...
    // the same.
    if (tsize == 1 &&
        (strcmp(name1, "char") == 0 || strcmp(name1, "unsigned char") == 0)) {
  ...
  case GS_CHAR_TYPE: { // modified for java char type 16bits 2008-1-16 ykq
    if (lang_java) {
      if (gs_decl_unsigned(type_tree)) {
        encoding = DW_ATE_unsigned;
      } else {
...
#endif
      dst_idx = DST_enter_enum(type_tree, ttidx, idx, tsize);
  ...
  //============
  common_basetypes : {
    FmtAssert(name1 != 0, ("name of base type empty, cannot make DST entry!"));

    std::string names(name1);
    DST_Type_Map::iterator p = basetypes.find(names);
    if (p != basetypes.end()) {
...
#endif
      {
        dst_idx = DST_mk_basetype(name1, encoding, tsize);
  ...
  case GS_REFERENCE_TYPE: {
    ...
    if (DST_IS_NULL(dst_idx)) {
      ...
      DST_INFO_IDX inner_dst = Create_DST_type_For_Tree(ttree, itx, idx);

      // not created yet, so create
      dst_idx = DST_mk_reference_type(inner_dst,    // type ptd to
...
#endif
    {
      ...
      if (DST_IS_NULL(dst_idx)) {
        ...
        if (gs_tree_code(ttree) == GS_OFFSET_TYPE) {
          ...
          dst_idx = DST_mk_pointer_type(inner_dst,    // type ptd to
                                        DW_ADDR_none, // no address class
                                        tsize);
        }
...
#endif
      dst_idx = DST_enter_struct_union(type_tree, ttidx, idx, tsize);
  } break;
  case GS_METHOD_TYPE: {
    // DevWarn ("Encountered METHOD_TYPE at line %d", lineno);
  } break;

  case GS_FUNCTION_TYPE: { // new scope for local vars
#if 0
		tree arg;
		INT32 num_args;
		TY &ty = New_TY (idx);
  ...
  case GS_VECTOR_TYPE: {
    ...
    type_tree = gs_tree_type(
        gs_type_fields(gs_type_debug_representation_type(type_tree)));
...
extern DST_INFO_IDX Create_DST_decl_For_Tree(gs_t decl, ST *var_st) {
  ...
  if (gs_tree_code(decl) == GS_VAR_DECL &&
      (!gs_tree_static(decl) && !gs_decl_common(decl))) {
    return cur_idx;
  }
#endif // !KEY
...
static DST_INFO_IDX DST_Create_type(ST *typ_decl, gs_t decl) {
  ...
  DST_INFO_IDX current_scope_idx = DST_get_context(gs_decl_context(decl));
...
static DST_INFO_IDX DST_Create_Parmvar(ST *var_st, gs_t param) {
  USRCPOS src;
  // For now, the source location appears bogus
  // (or at least odd) for files other than the base
  // file, so lets leave it out. Temporarily.
...
static DST_INFO_IDX DST_find_class_member(char *linkage_name_in,
  ...
  for (; field; field = gs_tree_chain(field)) {
    if (gs_tree_code(field) == GS_VAR_DECL) {
      ...
      char *linkage_name = gs_identifier_pointer(gs_decl_assembler_name(field));
      if (linkage_name && (strcmp(linkage_name, linkage_name_in) == 0)) {
...
static DST_INFO_IDX DST_Create_var(ST *var_st, gs_t decl) {
...
#else
    char *linkage_name = gs_identifier_pointer(gs_decl_assembler_name(decl));
#endif // KEY

    if (context && gs_tree_code(context) == GS_RECORD_TYPE) {
      /*look for  static data member decl*/
      class_var_idx = DST_find_class_member(linkage_name, context);
    ...
    for (; field; field = gs_tree_chain(field)) {
      ...
      if (gs_tree_code(field) == GS_FIELD_DECL ||
        ...
        dst = DST_mk_variable(src, // srcpos
                              field_name, TYPE_DST_IDX(gs_tree_type(field)),
                              0, // offset (fortran uses non zero )
                              (void *)(INTPTR)ST_st_idx(
                                  var_st), // underlying type here, not typedef.
                              DST_INVALID_IDX, // abstract origin
  ...
  // if(linkage_name && linkage_name[0] != '\0' && field_name &&
  // strcmp(linkage_name,field_name)) {
...
//
static void DST_enter_param_vars(gs_t fndecl, DST_INFO_IDX parent_idx,
                                 gs_t parameter_list, int is_abstract_root,
                                 int is_declaration_only) {
  USRCPOS src;
  USRCPOS_srcpos(src) = Get_Srcpos();
  ...
  for (; pdecl; pdecl = gs_tree_chain(pdecl)) {
    ...
    if (gs_tree_code(pdecl) == GS_INDIRECT_REF) {
      ...
      if (!is_declaration_only) {
        st = Get_ST(gs_tree_operand(
            pdecl, 0)); // As a side effect,
                        // parameters and the types are set in ST
                        // and dst
      ...
      ST_IDX loc = (is_abstract_root || is_declaration_only) ? ST_IDX_ZERO
                                                             : ST_st_idx(st);
      ...
      param_idx = DST_mk_formal_parameter(
          ...
          is_artificial,
          is_declaration_only); // bug 1735: this is totally bogus but is needed
                                // because the backend be/cg/cgdrawf.cxx tries
                                // to put a location attribute into the tag if
...
DST_Create_Subprogram(ST *func_st, gs_t fndecl) {
  ...
  DST_INFO_IDX current_scope_idx =
      fndecl ? (DST_get_context(gs_decl_context(fndecl))) : comp_unit_idx;
  ...
  gs_t context = NULL;
  if (fndecl)
    context = gs_decl_context(fndecl);
#else
  gs_t context = gs_decl_context(fndecl);
#endif
  if (context && gs_tree_code(context) == GS_RECORD_TYPE) {
    /*look for  static data member decl*/
    class_func_idx = DST_find_class_member(linkage_name, context);
  ...
  if (fndecl) {
    ...
    if (!parms) {
      ...
      DST_enter_param_vars(fndecl, dst, parms,
                           /* abstract_root = */ 0,
                           /* declaration only = */ 0);
    }
  }
...
DST_INFO_IDX
DST_Get_Comp_Unit(void) { return comp_unit_idx; }

//***********************************************************************
// Look in "lexical.h" at push_input_stack() to find out about directories
// and search paths.
//***********************************************************************
void DST_build(int num_copts, /* Number of options passed to fec(c) */
               char *copts[]) /* The array of option strings passed to fec(c) */
{
  ...
  {
    comp_unit_idx = DST_mk_compile_unit(
        Last_Pathname_Component(Src_File_Name), current_host_dir, comp_info,
        lang_cplus ? DW_LANG_C_plus_plus
                   : (lang_java ? DW_LANG_Java : DW_LANG_C89), // czw
...
void WGEN_Macro_End_File(void) { DST_mk_macr_end_file(); }
#endif
