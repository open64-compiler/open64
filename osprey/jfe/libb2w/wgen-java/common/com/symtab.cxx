...
Invalid_Pointer_Map_Entry(const TY_IDX_POINTER_MAP::value_type &entry) {
  ...
  if (TY_kind(ty) != KIND_POINTER || TY_pointed(ty) != pointee ||
...
static inline TY_IDX Find_Ty_Pointer(TY_POINTEE_KEY key) {
...
make_ptr_type(TY_IDX ty_idx, BOOL f90_pointer) {
  ...
  TY &ty = New_TY(ty_ptr);
  TY_Init(ty, Pointer_Size, KIND_POINTER, Pointer_Mtype, Save_Str("anon_ptr."));
...
// create the new type if we didn't find it.
  ...
  TY_Init(function_ty, (UINT64)0, KIND_FUNCTION, MTYPE_UNKNOWN, Save_Str(buf));
...
struct matches_pu_ty_and_name {
  ...
  BOOL operator()(const ST *const st) {
    return (ST_pu_type(st) == ty && strcmp(ST_name(st), name) == 0);
  }
};

static void INTRINSIC_LIST_add(ST *st) { intrinsic_list.push_back(st); }

ST *INTRINSIC_LIST_lookup(TY_IDX ty, char *function_name) {
  vector<ST *>::iterator result =
      std::find_if(intrinsic_list.begin(), intrinsic_list.end(),
...
ST *Gen_Intrinsic_Function(TY_IDX ty, char *function_name) {
...
//----------------------------------------------------------------------
inline size_t TY_struct_hash(const TY &ty) {
...
static BOOL Push_Equiv_Visit_Order(UINT32 ty_index1, UINT32 ty_index2,
                                   BOOL &are_equiv) {
...
BOOL FLD_are_equivalent(FLD_HANDLE fld1, FLD_HANDLE fld2, UINT32 flags) {
  ...
  if (FLD_ofst(fld1) != FLD_ofst(fld2) || FLD_bsize(fld1) != FLD_bsize(fld2) ||
      FLD_bofst(fld1) != FLD_bofst(fld2) ||
      FLD_flags(fld1) != FLD_flags(fld2) || FLD_st(fld1) != FLD_st(fld2))
    return FALSE;
...
// equivalent
static BOOL Struct_are_equivalent(FLD_HANDLE fld_id1, FLD_HANDLE fld_id2,
                                  UINT32 flags) {
  if (fld_id1 == fld_id2)
    return TRUE;
...
BOOL TYLIST_are_equivalent(TYLIST_IDX tylist_id1, TYLIST_IDX tylist_id2,
                           UINT32 flags) {
...
BOOL TY_are_equivalent(TY_IDX ty_id1, TY_IDX ty_id2, UINT32 flags) {
  ...
  if (Push_Equiv_Visit_Order(ty_index1, ty_index2, are_equiv)) {
    ...
    if (TY_size(ty1) != TY_size(ty2) || TY_kind(ty1) != TY_kind(ty2) ||
        TY_mtype(ty1) != TY_mtype(ty2) || TY_flags(ty1) != TY_flags(ty2)) {
      ...
      case KIND_FUNCTION:
        ...
        are_equiv =
            ((ty1.Pu_flags() == ty2.Pu_flags()) &&
             (TYLIST_are_equivalent(TY_tylist(ty1), TY_tylist(ty2), flags)));
        break;
...
static inline TY_IDX TY_is_unique_op(const TY_IDX ty_idx,
...
PREG_NUM
Create_Preg_explicit(TYPE_ID mtype, const char *name, SCOPE *scope_tab,
                     SYMTAB_IDX level) {
  PREG_IDX preg_idx;
  PREG_IDX preg_idx2;
...
void ST::Print(FILE *f, BOOL verbose) const {
  const char *name_str =
      (sym_class == CLASS_CONST) ? "<constant>" : &Str_Table[u1.name_idx];
  ...
  if (sym_class == CLASS_FUNC && verbose) {
    ...
    if (ty_idx != 0 && TY_tylist(ty) != 0) {
      ...
      if (Pu_Table[u2.pu].src_lang & PU_C_LANG)
        fprintf(f, "C  ");
      if (Pu_Table[u2.pu].src_lang & PU_CXX_LANG)
        fprintf(f, "C++  ");
      if (Pu_Table[u2.pu].src_lang & PU_JAVA_LANG)
        fprintf(f, "JAVA  ");
      if (Pu_Table[u2.pu].src_lang & PU_F77_LANG)
        fprintf(f, "F77  ");
      if (Pu_Table[u2.pu].src_lang & PU_F90_LANG)
        fprintf(f, "F90  ");
      ...
      if (flags & PU_IS_PURE)
        fprintf(f, " pure");
      if (flags & PU_NO_SIDE_EFFECTS)
        fprintf(f, " no_side_effects");
      if (flags & PU_IS_INLINE_FUNCTION)
        fprintf(f, " inline");
      if (flags & PU_NO_INLINE)
        fprintf(f, " no_inline");
      if (flags & PU_MUST_INLINE)
        fprintf(f, " must_inline");
      if (flags & PU_NO_DELETE)
        fprintf(f, " no_delete");
      if (flags & PU_HAS_EXC_SCOPES)
        fprintf(f, " exc_scopes");
      if (flags & PU_IS_NESTED_FUNC)
        fprintf(f, " nested_func");
      if (flags & PU_HAS_NON_MANGLED_CALL)
        fprintf(f, " non_mangled_call");
      if (flags & PU_ARGS_ALIASED)
        fprintf(f, " args_aliased");
      if (flags & PU_NEEDS_FILL_ALIGN_LOWERING)
        fprintf(f, " fill_align");
      if (flags & PU_NEEDS_T9)
        fprintf(f, " t9");
      if (flags & PU_HAS_VERY_HIGH_WHIRL)
        fprintf(f, " very_high_whirl");
      if (flags & PU_HAS_ALTENTRY)
        fprintf(f, " altentry");
      if (flags & PU_RECURSIVE)
        fprintf(f, " recursive");
      if (flags & PU_IS_MAINPU)
        fprintf(f, " main");
      if (flags & PU_UPLEVEL)
        fprintf(f, " uplevel");
      if (flags & PU_MP_NEEDS_LNO)
        fprintf(f, " mp_needs_lno");
      if (flags & PU_HAS_ALLOCA)
        fprintf(f, " alloca");
      if (flags & PU_IN_ELF_SECTION)
        fprintf(f, " in_elf_section");
      if (flags & PU_HAS_MP)
        fprintf(f, " has_mp");
      if (flags & PU_MP)
        fprintf(f, " mp");
      if (flags & PU_HAS_NAMELIST)
        fprintf(f, " namelist");
      if (flags & PU_HAS_RETURN_ADDRESS)
        fprintf(f, " return_address");
      if (flags & PU_HAS_REGION)
        fprintf(f, " has_region");
      if (flags & PU_HAS_INLINES)
        fprintf(f, " has_inlines");
      if (flags & PU_CALLS_SETJMP)
        fprintf(f, " calls_setjmp");
      if (flags & PU_CALLS_LONGJMP)
        fprintf(f, " calls_longjmp");
      if (flags & PU_IPA_ADDR_ANALYSIS)
        fprintf(f, " ipa_addr");
      if (flags & PU_SMART_ADDR_ANALYSIS)
        fprintf(f, " smart_addr");
      if (flags & PU_HAS_GLOBAL_PRAGMAS)
        fprintf(f, " global_pragmas");
      if (flags & PU_HAS_USER_ALLOCA)
        fprintf(f, " user_alloca");
      if (flags & PU_HAS_UNKNOWN_CONTROL_FLOW)
        fprintf(f, " unknown_control_flow");
      if (flags & PU_IS_THUNK)
        fprintf(f, " thunk");
#ifdef KEY
      if (flags & PU_NEEDS_MANUAL_UNWINDING)
        fprintf(f, " needs_manual_unwinding");
      if (flags & PU_IS_EXTERN_INLINE)
        fprintf(f, " extern_inline");
      if (flags & PU_IS_MARKED_INLINE)
        fprintf(f, " inline_keyword");
#endif
#ifdef TARG_X8664
      if (flags & PU_FF2C_ABI)
        fprintf(f, " ff2c_abi");
#endif
      if (TY_return_to_param(ty_idx))
        fprintf(f, " return_to_param");
      if (TY_is_varargs(ty_idx))
        fprintf(f, " varargs");
      if (TY_has_prototype(ty_idx))
        fprintf(f, " prototype");
      fprintf(f, "\n");
    }
  }
  ...
  if (verbose) {
    ...
    if (base_idx != 0) {
      ...
      fprintf(f, "\t\tAddress: %lld(%s<%d,%d>)  ", offset,
              ST_class(base_st) == CLASS_CONST ? "" : ST_name(base_idx),
              ST_IDX_level(base_idx), ST_IDX_index(base_idx));
    }
    ...
    if (flags) {
      if (flags & ST_IS_WEAK_SYMBOL)
        fprintf(f, " weak");
      if (flags & ST_IS_SPLIT_COMMON)
        fprintf(f, " split_common");
      if (flags & ST_IS_NOT_USED)
        fprintf(f, " not_used");
      if (flags & ST_IS_INITIALIZED)
        fprintf(f, " initialized");
      if (flags & ST_IS_RETURN_VAR)
        fprintf(f, " return_var");
      if (flags & ST_IS_VALUE_PARM)
        fprintf(f, " value_parm");
      if (flags & ST_PROMOTE_PARM)
        fprintf(f, " promote_parm");
      if (flags & ST_KEEP_NAME_W2F)
        fprintf(f, " keep_name_w2f");
      if (flags & ST_IS_DATAPOOL)
        fprintf(f, " datapool");
      if (flags & ST_IS_RESHAPED)
        fprintf(f, " reshaped");
      if (flags & ST_EMIT_SYMBOL)
        fprintf(f, " emit_symbol");
      if (flags & ST_HAS_NESTED_REF)
        fprintf(f, " has_nested_ref");
      if (flags & ST_INIT_VALUE_ZERO)
        fprintf(f, " init_value_zero");
      if (flags & ST_GPREL)
        fprintf(f, " gprel");
      if (flags & ST_NOT_GPREL)
        fprintf(f, " not_gprel");
      if (flags & ST_IS_NAMELIST)
        fprintf(f, " namelist");
      if (flags & ST_IS_F90_TARGET)
        fprintf(f, " f90_target");
      if (flags & ST_DECLARED_STATIC)
        fprintf(f, " static");
      if (flags & ST_IS_EQUIVALENCED)
        fprintf(f, " equivalenced");
      if (flags & ST_IS_FILL_ALIGN)
        fprintf(f, " fill_align");
      if (flags & ST_IS_OPTIONAL_ARGUMENT)
        fprintf(f, " optional");
      if (flags & ST_PT_TO_UNIQUE_MEM)
        fprintf(f, " pt_to_unique_mem");
      if (flags & ST_IS_TEMP_VAR)
        fprintf(f, " temp");
      if (flags & ST_IS_CONST_VAR)
        fprintf(f, " const");
      if (flags & ST_ADDR_SAVED)
        fprintf(f, " addr_saved");
      if (flags & ST_ADDR_PASSED)
        fprintf(f, " addr_passed");
...
void FLD::Print(FILE *f) const {
  ...
  fprintf(f, "\n\t\tfl:0x%04x", flags);
  if (flags) {
    if (flags & FLD_LAST_FIELD)
      fprintf(f, " last_field");
    if (flags & FLD_EQUIVALENCE)
      fprintf(f, " equivalence");
    if (flags & FLD_BEGIN_UNION)
      fprintf(f, " begin_union");
    if (flags & FLD_END_UNION)
      fprintf(f, " end_union");
    if (flags & FLD_BEGIN_MAP)
      fprintf(f, " begin_map");
    if (flags & FLD_END_MAP)
      fprintf(f, " end_map");
...
void TY::Print(FILE *f) const {
  fprintf(f, "%-14s:", name_idx ? &Str_Table[name_idx] : "(anon)");

  fprintf(f, " (f: 0x%04x", flags);
  if (flags) {
    if (flags & TY_IS_CHARACTER)
      fprintf(f, " character");
    if (flags & TY_IS_LOGICAL)
      fprintf(f, " logical");
    if (flags & TY_IS_UNION)
      fprintf(f, " union");
    if (flags & TY_IS_PACKED)
      fprintf(f, " packed");
    if (flags & TY_PTR_AS_ARRAY)
      fprintf(f, " ptr_as_array");
    if (flags & TY_ANONYMOUS)
      fprintf(f, " anonymous");
    if (flags & TY_SPLIT)
      fprintf(f, " split");
    if (flags & TY_IS_F90_POINTER)
      fprintf(f, " f90_pointer");
    if (flags & TY_NOT_IN_UNION)
      fprintf(f, " not_in_union");
    if (flags & TY_NO_ANSI_ALIAS)
      fprintf(f, " no_ansi_alias");
    if (flags & TY_IS_NON_POD)
      fprintf(f, " non_pod");
#ifdef KEY
    if (flags & TY_RETURN_IN_MEM)
      fprintf(f, " return_in_mem");
    if (flags & TY_CONTENT_SEEN)
      fprintf(f, " content_seen");
    if (flags & TY_IS_INCOMPLETE)
      fprintf(f, " incomplete");
#endif
  }
  fprintf(f, ")");

  fprintf(f, " size %lld %s: ", size, (mtype != 0) ? Mtype_Name(mtype) : "");
...
void LABEL::Print(FILE *f) const {
  ...
  if (flags & LABEL_ADDR_SAVED)
    fprintf(f, " addr_saved");
  if (flags & LABEL_ADDR_PASSED)
    fprintf(f, " addr_passed");
  fprintf(f, "\n");
} // LABEL::Print
...
void FILE_INFO::Print(FILE *f) const {
  fprintf(f, "gp_group: %d, flags: 0x%08x", gp_group, flags);
  if (flags) {
    if (flags & FI_IPA)
      fputs(" IPA-generated", f);
    if (flags & FI_NEEDS_LNO)
      fputs(" needs_LNO", f);
    if (flags & FI_HAS_INLINES)
      fputs(" has_inlines", f);
    if (flags & FI_HAS_MP)
      fputs(" has_mp", f);
  }
...
template <class T> struct print_op {
  FILE *fid;
...
template <class T>
inline void print_op<T>::operator()(UINT idx, T *entry) const {
  fprintf(fid, "[%d]: ", idx);
  entry->Print(fid);
}

// specialization for printing TCONs
template <> inline void print_op<TCON>::operator()(UINT idx, TCON *c) const {
  fprintf(fid, "[%d] %s: %s\n", idx, MTYPE_name(TCON_ty(*c)),
          Targ_Print(NULL, *c));
...
void Print_local_symtab(FILE *f, const SCOPE &scope) {
  ...
  fprintf(f,
          ...
          " inito %d, st_attr %d\n%s\n",
          DBar, ST_name(scope.st), PU_lexical_level(scope.st),
          scope.st_tab->Size() - 1, scope.label_tab->Size() - 1,
          scope.preg_tab->Size() - 1, scope.inito_tab->Size() - 1,
          scope.st_attr_tab->Size() - 1, DBar);
...
void dump_st_idx(ST_IDX st) { dump_st(st); }
...
void dump_ty_idx(TY_IDX ty_idx) { dump_ty(ty_idx); }

void dump_ty(const TY &ty) { ty.Print(stdout); }

void dump_label(LABEL_IDX idx) { Label_Table[idx].Print(stdout); }

void dump_inito(INITO_IDX idx) { Inito_Table[idx].Print(stdout); }

void dump_st_attr(ST_ATTR_IDX idx) { St_Attr_Table[idx].Print(stdout); }

#ifndef KEY
static
#endif // !KEY
    ST *
    Gen_Temp_Named_Symbol(TY_IDX ty, const char *rootname, ST_CLASS sym_class,
...
ST *Gen_Temp_Symbol(TY_IDX ty,            // type of the desired symbol
...
ST *Gen_Read_Only_Symbol(TY_IDX ty, const char *rootname) {
...
Promoted_Parm_Type(const ST *formal_parm) {
  ...
  if (!ST_promote_parm(formal_parm))
    return ptype;
...
void New_Scope(SYMTAB_IDX level, MEM_POOL *pool, BOOL reserve_index_zero) {
  if (level >= Max_scope) {
    ...
    Max_scope *= 2;
    Scope_tab =
        (SCOPE *)MEM_POOL_Realloc(Malloc_Mem_Pool, Scope_tab, size, size * 2);
  }
  ...
  Scope_tab[level].Init(st_tab, label_tab, preg_tab, inito_tab, st_attr_tab,
                        pool);
...
static void Setup_Preg_Pointers() {
  for (TYPE_ID i = MTYPE_FIRST; i <= MTYPE_LAST; ++i) {
    ...
    MTYPE_To_PREG(i) =
        MTYPE_signed(i) ? MTYPE_To_PREG(MTYPE_I4) : MTYPE_To_PREG(MTYPE_U4);
  }
...
static void Create_All_Preg_Symbols() {
  for (TYPE_ID i = MTYPE_FIRST; i <= MTYPE_LAST; ++i) {
    ...
    if (MTYPE_byte_size(i) < 4) {
      ...
      if (i != MTYPE_B && !MTYPE_is_integral(i))
        continue;
#else
    ...
    ST_Init(st, Save_Str2(".preg_", MTYPE_name(i)), CLASS_PREG, SCLASS_REG,
            EXPORT_LOCAL, MTYPE_To_TY(i));
    MTYPE_To_PREG(i) = st;
  }
  ...
  if (Return_Val_Preg == NULL) {
    ...
    ST *st = New_ST(GLOBAL_SYMTAB);
    ST_Init(st, Save_Str(".preg_return_val"), CLASS_PREG, SCLASS_REG,
            EXPORT_LOCAL, 0);
    Return_Val_Preg = st;
  }
 ...
 * ====================================================================
 */
static inline STR_IDX Gen_predef_type_name(const char *basename) {
...
static void Create_Special_Global_Symbols() {
  ...
  for (TYPE_ID i = MTYPE_FIRST; i <= MTYPE_LAST; ++i) {
    ...
    if (i == MTYPE_A4 || i == MTYPE_A8) {
      ...
      Is_True(i > MTYPE_V, ("Create_Special_Global_Symbols() assumes MTYPE_V < "
...
static inline BOOL Predefined_Void_Type(TY_IDX ty_idx) {
  const TY &ty = Ty_Table[ty_idx];
...
void Initialize_Special_Global_Symbols() {
  ...
  for (UINT32 i = MTYPE_FIRST; i < last; ++i) {
    ...
    TYPE_ID mtype = TY_mtype(ty);
    if (MTYPE_byte_size(mtype) != TY_size(ty) || TY_flags(ty) != 0)
      continue;
    switch (mtype) {
    case MTYPE_V:
    ...
    case MTYPE_A4:
    case MTYPE_A8:
      if (TY_kind(ty) != KIND_POINTER || !Predefined_Void_Type(TY_pointed(ty)))
        continue;
      break;
...
void Initialize_Symbol_Tables(BOOL reserve_index_zero) {
  ...
  Scope_tab =
      (SCOPE *)MEM_POOL_Alloc(Malloc_Mem_Pool, Max_scope * sizeof(SCOPE));
