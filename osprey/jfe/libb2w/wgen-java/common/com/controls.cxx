...
#define Reset_CI_user_specified_impl(ci)                                       \
  ((ci)->flags &= ~CI_USER_SPECIFIED_IMPL)
...
#define Reset_CI_user_specified_expl(ci)                                       \
  ((ci)->flags &= ~CI_USER_SPECIFIED_EXPL)
...
CONTROL_INFO Aflag_Tbl[] = {
    ...
    {"targ", CONTROL_TARG, N | C, TARG_FIRST_DEF, TARG_SECOND_DEF,
     (INTPS)&Possible_Targets},
...
static O_GR_EXP o_group_expansion[] = {
    /*0*/ {"no-opt", "callmod=0,constp=0,copyp=0,domain=1,flow=0,fcm=0,"
                     "alias=0,mopt=0,reg=0,sched=0,unroll=0,whole=0"},
    /*1*/
    {"local-opt", "callmod=0,constp=0,copyp=0,domain=1,flow=0,fcm=0,"
                  "alias=1,mopt=1,reg=0,sched=0,unroll=0,whole=0"},
    /*2*/
    {"global-opt", "callmod=1,constp=2,copyp=2,domain=1,flow=1,fcm=1,"
                   "alias=3,mopt=3,reg=1,sched=1,unroll=0,whole=0"},
    /*3*/
    {"swp-opt", "callmod=1,constp=2,copyp=2,domain=1,flow=1,fcm=1,"
                "alias=3,mopt=3,reg=1,sched=1,unroll=0,whole=0"},
};
...
static BOOL same_name_lists(STR_LIST *a, STR_LIST *b) {
  STR_LIST *p;
  INT16 ac, bc;
  for (ac = 0, p = a; p; p = p->next)
    ac++;
  for (bc = 0, p = b; p; p = p->next)
    bc++;
  if (ac != bc)
    return FALSE;
  while (a) {
...
static INT store_ctrl(char *name, STR_LIST *name_list, INT flags) {
  CONTROL_INFO *a;
  BOOL ok_int;
  INT32 int_val;
      ...
      if (name[0] == cgi->name[0]) {
        ...
        if (CGI_is_int_type(cgi)) {
          ...
          if (v < cgi->min_val || v > cgi->max_val) {
            Report_Error(EC_Group_Range, v, cgi->name, cgi->min_val,
                         cgi->max_val);
            return ERRORS_FOUND;
          }
        } else {
    ...
    if (a->name && strcmp(name, a->name) == 0) {
      ...
      if (CI_is_nlist_type(a)) {
        ...
        if (v < CI_int(a, min_val) || v > CI_int(a, max_val)) {
          Report_Error(EC_Ctrl_Range, v, a->name, CI_int(a, min_val),
                       CI_int(a, max_val));
          return ERRORS_FOUND;
        }
        if (v != a->cur_val) {
...
void Init_Controls_Tbl(void) {
  ...
  for (INT i = CONTROL_FIRST; i < CONTROL_LAST; i++) {
    ...
    if (a->name == NULL)
      break;
    Is_True(a->index == i, ("Aflag_Tbl index mismatch: i=%1d a->index=%1d(%s)",
                            i, a->index, a->name));
    if (CI_is_int_type(a)) {
      if (a->max_val <= a->min_val) {
        Is_True(a->max_val == -1 && a->min_val == 1,
                ("inconsistent min_val and max_val of %s: %1d %1d", a->name,
                 a->min_val, a->max_val));
      } else {
        Is_True(a->first_def >= a->min_val && a->first_def <= a->max_val,
                ("inconsistent first_def(%1d) of %s: %1d..%1d", a->first_def,
                 a->name, a->min_val, a->max_val));
        Is_True(a->sec_def >= a->min_val && a->sec_def <= a->max_val,
                ("inconsistent sec_def(%1d) of %s: %1d..%1d", a->first_def,
                 a->name, a->min_val, a->max_val));
      }
    } else {
...
void Pop_Once_Line_Controls(void) { Pop_Controls(CI_SCOPE_LINE); }
