...
#ifdef _KEEP_RCS_ID
/*REFERENCED*/
static char *config_opt_rcs_id =
    "$Source: common/com/SCCS/s.config_opt.cxx $ $Revision: 1.35 $";
#endif /* _KEEP_RCS_ID */
...
BOOL OPT_IPA_addr_analysis =
    TRUE; /* enable the use of IPA addr analysis result */
BOOL Delay_U64_Lowering =
    TRUE; /* Delay unsigned 64-bit lowering to after wopt */
...
UINT32 Div_Exe_Counter =
    40000;                 /* A number that can avoid regression on facerec. */
...
BOOL OPT_Enable_Simp_Fold =
    TRUE; /* enables new float/complex/str/intrinsic foldings */
...
static OPTION_DESC Options_OPT[] = {
    {OVK_BOOL, OV_VISIBLE, TRUE, "warning", "warn", 0, 0, 0, &Show_OPT_Warnings,
     NULL, "Control interpretation of possible variable aliasing"},

#ifdef KEY
    {OVK_BOOL, OV_INTERNAL, FALSE, "asm_memory", "", 0, 0, 0, &Asm_Memory, NULL,
     "Assumes each asm has memory flag specified even if it is not there"},
    {OVK_BOOL, OV_INTERNAL, FALSE, "align_unsafe", "", 0, 0, 0, &Align_Unsafe,
     NULL, "Generate aligned load/store even though it may be unsafe"},
#endif

    {OVK_LIST, OV_VISIBLE, TRUE, "alias", "alia", 0, 0, 0, &Alias_Option, NULL,
     "Control interpretation of possible variable aliasing"},

    {OVK_INT32, OV_SHY, TRUE, "align_instructions", "align_i", 16, 0, 1024,
     ALIGN_INSTS, NULL,
     "Align subprogram entries and loops by given byte count"},

    {OVK_BOOL, OV_INTERNAL, TRUE, "align_object", "align_o", 0, 0, 0,
     &Align_Object, NULL,
     "Allow realignment of objects to improve memory accesses"},

    {OVK_BOOL, OV_INTERNAL, TRUE, "align_padding", "align_p", 0, 0, 0,
     &Align_Padding, NULL,
     "Allow padding of objects to improve memory alignment"},

    {OVK_INT32, OV_INTERNAL, TRUE, "bblength", "bb", DEF_BBLENGTH, MIN_BBLENGTH,
     MAX_BBLENGTH, &Split_BB_Length, NULL,
     "Restrict BB length by splitting longer BBs"},

    {OVK_BOOL, OV_VISIBLE, TRUE, "cis", "cis", 0, 0, 0, &CIS_Allowed, &CIS_Set,
    ...
    {OVK_BOOL, OV_VISIBLE, TRUE, "cray_ivdep", "cray_ivdep", 0, 0, 0,
     &Cray_Ivdep, NULL,
     "IVDEP pragma/directive break parallelism-inhibiting dependencies"},

    {OVK_BOOL, OV_INTERNAL, TRUE, "cvt", "cvt", 0, 0, 0, &Enable_CVT_Opt,
     &Enable_CVT_Opt_Set, "Optimize conversion operators"},

    {OVK_BOOL, OV_VISIBLE, TRUE, "div_split", "div_split", 0, 0, 0,
     &Div_Split_Allowed, &Div_Split_Set,
     "Allow splitting of a/b into a*recip(b)"},

#ifdef KEY
    {OVK_UINT32, OV_INTERNAL, TRUE, "div_exe_counter", "", 0, 0, UINT32_MAX,
     &Div_Exe_Counter, NULL,
     "Restrict div/rem/mod optimization via value profiling"},
    {OVK_UINT32, OV_INTERNAL, TRUE, "div_exe_ratio", "", 0, 0, 100,
     &Div_Exe_Ratio, NULL,
     "Restrict div/rem/mod optimization via value profiling"},
    {OVK_UINT32, OV_INTERNAL, TRUE, "div_exe_candidates", "", 0, 0, 10,
     &Div_Exe_Candidates, NULL,
     "Restrict div/rem/mod optimization via value profiling"},
    {OVK_UINT32, OV_INTERNAL, TRUE, "mpy_exe_counter", "", 0, 0, UINT32_MAX,
     &Mpy_Exe_Counter, NULL, "Restrict mpy optimization via value profiling"},
    {OVK_UINT32, OV_INTERNAL, TRUE, "mpy_exe_ratio", "", 0, 0, 100,
     &Mpy_Exe_Ratio, NULL, "Restrict mpy optimization via value profiling"},
    {OVK_BOOL, OV_INTERNAL, TRUE, "simp_debug", "simp_debug", 0, 0, 0,
     &OPT_Enable_Simp_Fold, NULL, "New foldings in simplifier"},
    ...
    {OVK_BOOL, OV_VISIBLE, TRUE, "fast_stdlib", "", 0, 0, 0, &OPT_Fast_Stdlib,
     NULL, "Use fast versions of some standard library functions"},

    {OVK_BOOL, OV_INTERNAL, TRUE, "mp_barrier_opt", "", 0, 0, 0,
     &OPT_MP_Barrier_Opt, NULL,
     "Optimize generation of barrier calls for OpenMP"},

    {OVK_BOOL, OV_INTERNAL, TRUE, "icall_instr", "", 0, 0, 0, &OPT_Icall_Instr,
     NULL, "perform profiling of indirect calls to get their called functions"},

    {OVK_BOOL, OV_INTERNAL, TRUE, "int_value_instr", "", 0, 0, 0,
     &OPT_Int_Value_Instr, NULL, "perform profiling of integer values"},

    {OVK_BOOL, OV_INTERNAL, TRUE, "fp_value_instr", "", 0, 0, 0,
     &OPT_FP_Value_Instr, NULL, "perform profiling of floating-point values"},

    {OVK_BOOL, OV_INTERNAL, TRUE, "ffast_math", "", 0, 0, 0, &OPT_Ffast_Math,
     &OPT_Ffast_Math_Set,
     "Determines conformance to IEEE-754 arithmetic rules"},

    {OVK_BOOL, OV_INTERNAL, TRUE, "funsafe_math_optimizations", "", 0, 0, 0,
     &OPT_Funsafe_Math_Optimizations, &OPT_Funsafe_Math_Optimizations_Set,
     "Determines conformance to IEEE-754 arithmetic rules"},
    {OVK_BOOL, OV_INTERNAL, TRUE, "float_via_int", "", 0, 0, 0,
     &OPT_Float_Via_Int, NULL,
     "perform floating-point memory copies using integer registers"},

    {OVK_UINT32, OV_VISIBLE, TRUE, "malloc_algorithm", "malloc_alg", 0, 0, 1,
     &OPT_Malloc_Alg, NULL, "Use alternate malloc algorithm"},
#endif

    {OVK_BOOL, OV_INTERNAL, TRUE, "early_mp", "early_mp", 0, 0, 0,
     &Early_MP_Processing, NULL, "Lower before LNO"},

    {OVK_BOOL, OV_VISIBLE, TRUE, "fast_bit_intrinsics", "fast_bit", 0, 0, 0,
     &Fast_Bit_Allowed, &Fast_Bit_Set,
     "Don't check bit count range for Fortran bit intrinsics"},

    {OVK_BOOL, OV_VISIBLE, TRUE, "fast_complex", "fast_co", 0, 0, 0,
     &Fast_Complex_Allowed, &Fast_Complex_Set,
     "Use fast algorithms with limited domains for complex norm and divide"},

    {OVK_BOOL, OV_VISIBLE, TRUE, "fast_exp", "fast_ex", 0, 0, 0,
     &Fast_Exp_Allowed, &Fast_Exp_Set,
     "Use multiplication and square root for exp() where faster"},

    {OVK_BOOL, OV_VISIBLE, TRUE, "fast_io", "fast_io", 0, 0, 0,
     &Fast_IO_Allowed, &Fast_IO_Set,
     "Inline some C I/O routines if __INLINE_INTRINSICS is defined"},

    {OVK_BOOL, OV_VISIBLE, TRUE, "fast_nint", "fast_nint", 0, 0, 0,
     &Fast_NINT_Allowed, &Fast_NINT_Set,
     "Use IEEE rounding instead of Fortran rounding for NINT intrinsics"},

#ifdef TARG_X8664
    {OVK_BOOL, OV_VISIBLE, TRUE, "fast_anint", "fast_anint", 0, 0, 0,
     &Fast_ANINT_Allowed, &Fast_ANINT_Set,
     "Use IEEE rounding instead of Fortran rounding for ANINT intrinsics"},

    {OVK_BOOL, OV_VISIBLE, TRUE, "vcast_complex", "vcast_complex", 0, 0, 0,
     &Vcast_Complex, &Vcast_Complex_Set,
     "Convert MTYPE_C8 to MTYPE_V16C8 in the lowerer"},
#endif

    {OVK_BOOL, OV_VISIBLE, TRUE, "fast_sqrt", "fast_sq", 0, 0, 0,
     &Fast_Sqrt_Allowed, &Fast_Sqrt_Set,
     "May use x*rsqrt(x) for sqrt(x) on machines where faster"},

    {OVK_BOOL, OV_VISIBLE, TRUE, "fast_trunc", "fast_trunc", 0, 0, 0,
     &Fast_trunc_Allowed, &Fast_trunc_Set,
     "Inline NINT and related intrinsics with limited-domain algorithm"},

    {OVK_BOOL, OV_SHY, TRUE, "fold_aggressive", "fold_ag", 0, 0, 0,
     &Enable_Cfold_Aggressive, &Cfold_Aggr_Set,
     "Allow aggressive expression folding optimizations"},

    {OVK_BOOL, OV_SHY, TRUE, "fold_intrinsics", "fold_i", 0, 0, 0,
     &Enable_Cfold_Intrinsics, &Cfold_Intrinsics_Set,
     "Allow expression folding of Fortran intrinsic calls"},

    {OVK_BOOL, OV_VISIBLE, TRUE, "fold_reassociate", "fold_r", 0, 0, 0,
     &Enable_Cfold_Reassociate, &Cfold_Reassoc_Set,
     "Allow optimizations which reassociate floating point operators"},

    {OVK_BOOL, OV_VISIBLE, TRUE, "fold_unsafe_relops", "fold_unsafe_relops", 0,
     0, 0, &Simp_Unsafe_Relops, &Simp_Unsafe_Relops_Set,
     "Allow relational operator folding which may cause integer overflow"},

    {OVK_BOOL, OV_VISIBLE, TRUE, "fold_unsigned_relops", "fold_unsigned", 0, 0,
     0, &Simp_Fold_Unsigned_Relops, &Simp_Fold_Unsigned_Relops_Set,
     "Allow relop folding which may cause unsigned integer overflow"},

    {OVK_BOOL, OV_VISIBLE, TRUE, "got_call_conversion", "got_call", 0, 0, 0,
     &Enable_GOT_Call_Conversion, &Enable_GOT_Call_overridden,
     "Allow function address loads to be moved out of loops"},

    {OVK_BOOL, OV_INTERNAL, TRUE, "goto_conversion", "goto", 0, 0, 0,
     &WOPT_Enable_Goto, NULL,
     "Enable conversion of GOTO to more structured constructs"},

    {OVK_INT32, OV_VISIBLE, TRUE, "IEEE_arithmetic", "IEEE_a", 1, 1, 3,
     &IEEE_Arithmetic, &IEEE_Arith_Set,
     "Level of conformance to IEEE-754 arithmetic rules"},

    {OVK_BOOL, OV_SHY, TRUE, "IEEE_comparisons", "IEEE_c", 0, 0, 0,
     &Force_IEEE_Comparisons, NULL,
     "Force conforming operations on IEEE-754 NaN and Inf values"},

    {OVK_BOOL, OV_VISIBLE, TRUE, "IEEE_NaN_Inf", "IEEE_N", 0, 0, 0,
     &Force_IEEE_Comparisons, NULL,
     "Force conforming operations on IEEE-754 NaN and Inf values"},

    {OVK_BOOL, OV_SHY, TRUE, "implied_do_io_opt", NULL, 1, 0, 0,
     &Implied_Do_Io_Opt, NULL,
     "Optimize implied DO I/O to minimize calls in Fortran"},

    {OVK_BOOL, OV_VISIBLE, TRUE, "inline_intrinsics", "inline_intr", 0, 0, 0,
     &Inline_Intrinsics_Allowed, &Inline_Intrinsics_Set,
     "Allow inlining of Fortran intrinsic functions"},

    {OVK_BOOL, OV_SHY, TRUE, "ldx", "ldx", 0, 0, 0, &Indexed_Loads_Allowed,
     NULL, "Allow generation of indexed load/store operations"},

    {OVK_BOOL, OV_VISIBLE, TRUE, "liberal_ivdep", "liberal_ivdep", 0, 0, 0,
     &Liberal_Ivdep, NULL,
     "IVDEP pragmas/directives break all dependencies within an array"},

    {OVK_BOOL, OV_VISIBLE, TRUE, "Inhibit_EH_opt", "Inhibit_EH_opt", 0, 0, 0,
     &Inhibit_EH_opt, NULL, "Don't remove EH regions without calls"},

    {OVK_BOOL, OV_INTERNAL, TRUE, "nary", "nary", 0, 0, 0, &Enable_NaryExpr,
     &Enable_NaryExpr_Set, "Allow N-ary tree height reduction of MADDs"},

    {OVK_NAME, OV_SHY, FALSE, "Ofast", "Ofast", 0, 0, 0, &Ofast, NULL,
     "Tailor options for performance on current target"},

    {OVK_INT32, OV_VISIBLE, FALSE, "Olimit", "Ol", DEFAULT_OLIMIT, 0,
     MAX_OLIMIT, &Olimit, &Olimit_Set,
     "Limit size of subprograms which will be optimized"},

    {OVK_BOOL, OV_INTERNAL, FALSE, "Olimit_opt", "Olimit_o", 0, 0, 0,
     &Olimit_opt, &Olimit_opt_Set, "Use regions if Olimit exceeded"},

    {OVK_BOOL, OV_VISIBLE, TRUE, "pad_common", "pad", 0, 0, 0, &OPT_Pad_Common,
     NULL, "Force padding of COMMON blocks to improve cache behavior"},

#ifdef KEY
    {OVK_BOOL, OV_INTERNAL, FALSE, "profile_arcs", "", 0, 0, 0, &profile_arcs,
     NULL, "arc profiling used by GCC"},
#endif

    {OVK_BOOL, OV_INTERNAL, TRUE, "ptr_opt", "ptr_o", 0, 0, 0, &Ptr_Opt_Allowed,
     NULL, "Allow treatment of pointers as arrays when possible in C"},

    {OVK_BOOL, OV_INTERNAL, FALSE, "rail", "rail", 0, 0, 0,
     &Regions_Around_Inner_Loops, NULL, "Insert regions around inner loops"},

    {OVK_BOOL, OV_INTERNAL, FALSE, "rbi", "rbi", 0, 0, 0, &Region_Boundary_Info,
     NULL, "Create region boundary information"},

    {OVK_BOOL, OV_VISIBLE, TRUE, "recip", "recip", 0, 0, 0, &Recip_Allowed,
     &Recip_Set, "Allow use of recip instruction"},

    {OVK_LIST, OV_SHY, FALSE, "region_skip_equal", "region_skip_e", 0, 0, 4096,
     &Region_Skip, NULL, "Skip optimization of this region"},

    {OVK_LIST, OV_SHY, FALSE, "region_skip_before", "region_skip_b", 0, 0, 4096,
     &Region_Skip, NULL, "Skip optimization of regions before this one"},

    {OVK_LIST, OV_SHY, FALSE, "region_skip_after", "region_skip_a", 0, 0, 4096,
     &Region_Skip, NULL, "Skip optimization of regions after this one"},

    {OVK_BOOL, OV_VISIBLE, TRUE, "reorg_common", "reorg", 0, 0, 0,
     &OPT_Reorg_Common, &OPT_Reorg_Common_Set,
     "Allow splitting of COMMON blocks to improve cache behavior"},

    {OVK_BOOL, OV_VISIBLE, FALSE, "procedure_reorder", "procedure_reorder", 0,
     0, 0, &Section_For_Each_Function, NULL,
     "Place each function in its own .text section"},

    {OVK_INT32, OV_VISIBLE, TRUE, "roundoff", "ro", 2, 0, 3, &Roundoff_Level,
     &Roundoff_Set,
     "Level of acceptable departure from source roundoff semantics"},

#ifdef TARG_X8664
    {OVK_UINT32, OV_VISIBLE, TRUE, "rsqrt", "rsqrt", 0, 0, 2, &Rsqrt_Allowed,
     &Rsqrt_Set, "How to use rsqrt instructions"},
#else
    {OVK_BOOL, OV_VISIBLE, TRUE, "rsqrt", "rsqrt", 0, 0, 0, &Rsqrt_Allowed,
     &Rsqrt_Set, "Allow use of rsqrt instruction"},
#endif

    {OVK_BOOL, OV_VISIBLE, TRUE, "shared_memory", "shared_mem", 0, 0, 0,
     &OPT_shared_memory, NULL, "Assume use of shared memory"},

    {OVK_LIST, OV_SHY, FALSE, "skip_equal", "skip_e", 0, 0, 4096, &Opt_Skip,
     NULL, "Skip optimization of this subprogram"},

    {OVK_LIST, OV_SHY, FALSE, "skip_before", "skip_b", 0, 0, 4096, &Opt_Skip,
     NULL, "Skip optimization of subprograms before this one"},

    {OVK_LIST, OV_SHY, FALSE, "skip_after", "skip_a", 0, 0, 4096, &Opt_Skip,
     NULL, "Skip optimization of subprograms after this one"},

    {OVK_BOOL, OV_VISIBLE, FALSE, "space", "sp", 0, 0, 0, &OPT_Space, NULL,
     "Bias optimizations to minimize code space"},

    {OVK_BOOL, OV_INTERNAL, TRUE, "speculate", "", 0, 0, 0,
     &OPT_Lower_Speculate, NULL, "Allow speculation for CAND/COR operators"},

    {OVK_BOOL, OV_INTERNAL, TRUE, "speculative_null_ptr_deref", "", 0, 0, 0,
     &GCM_Eager_Null_Ptr_Deref, &GCM_Eager_Null_Ptr_Deref_Set,
     "Allow speculation of loads above NULL pointer tests"},

    {OVK_BOOL, OV_INTERNAL, TRUE, "speculative_ptr_deref", "", 0, 0, 0,
     &GCM_Speculative_Ptr_Deref, &GCM_Speculative_Ptr_Deref_Set,
     "Allow speculative loads of memory locations that differ by a small "
     "offset from some referenced memory location"},

    {OVK_BOOL, OV_VISIBLE, TRUE, "swp", "swp", 0, 0, 0, &Enable_SWP,
     &Enable_SWP_overridden, "Enable software pipelining"},

    {OVK_BOOL, OV_INTERNAL, FALSE, "transform_to_memlib", "transform", 0, 0, 0,
     &OPT_Lower_To_Memlib, NULL,
     "Allow loop or struct memory copy/set to be library calls"},

    {OVK_INT32, OV_INTERNAL, TRUE, "threshold_to_memlib", "", 256, 0, INT32_MAX,
     &OPT_Threshold_To_Memlib, NULL,
     "Threshold to transform loop copy/set to be memory library calls"},

    {OVK_INT32, OV_INTERNAL, TRUE, "memlib_limit", "memlib_limit", INT32_MAX, 0,
     INT32_MAX, &OPT_Enable_Lower_To_Memlib_Limit, NULL},

    {OVK_BOOL, OV_INTERNAL, TRUE, "treeheight", "", 0, 0, 0,
     &OPT_Lower_Treeheight, &OPT_Lower_Treeheight_Set,
     "Allow tree height reduction"},

    {OVK_BOOL, OV_INTERNAL, TRUE, "inline_divide", "", 0, 0, 0,
     &OPT_Inline_Divide, &OPT_Inline_Divide_Set,
     "Inline divide and remainder operations if possible"},

    {OVK_BOOL, OV_VISIBLE, TRUE, "unroll_analysis", "unroll_analysis", 0, 0, 0,
     &OPT_Unroll_Analysis, &OPT_Unroll_Analysis_Set,
     "Analyze inner loop requirements before unrolling"},

    {OVK_INT32, OV_VISIBLE, TRUE, "unroll_size", "unroll_s", 0, 0, INT32_MAX,
     &OPT_unroll_size, &OPT_unroll_size_overridden,
     "Maximum size of loops to be unrolled"},

    {OVK_INT32, OV_VISIBLE, TRUE, "unroll_times_max", "unroll", 0, 0, INT32_MAX,
     &OPT_unroll_times, &OPT_unroll_times_overridden,
     "Maximum number of times to unroll loops"},

    {OVK_BOOL, OV_INTERNAL, TRUE, "wn_simplify", "wn_simp", 0, 0, 0,
     &Enable_WN_Simp, &Enable_WN_Simp_Set, "Enable simplifier"},
#ifdef KEY
    {OVK_INT32, OV_INTERNAL, TRUE, "simp_limit", "", INT32_MAX, 0, INT32_MAX,
     &Enable_WN_Simp_Expr_Limit, NULL},

    {OVK_LIST, OV_SHY, FALSE, "goto_skip_equal", "goto_skip_e", 0, 0, 4096,
     &Goto_Skip, NULL, "Skip goto conversion of this subprogram"},

    {OVK_LIST, OV_SHY, FALSE, "goto_skip_before", "goto_skip_b", 0, 0, 4096,
     &Goto_Skip, NULL, "Skip goto conversion of subprograms before this one"},

    {OVK_LIST, OV_SHY, FALSE, "goto_skip_after", "goto_skip_a", 0, 0, 4096,
     &Goto_Skip, NULL, "Skip goto conversion of subprograms after this one"},
#endif
    ...
    {OVK_BOOL, OV_VISIBLE, TRUE, "mem_intrinsics", "", 0, 0, 0,
     &CG_mem_intrinsics, NULL,
     "Enable inline expansion of memory intrinsics (bzero, blkclr, bcopy, "
     "memset, memcpy, memmove)"},
    {OVK_INT32, OV_VISIBLE, TRUE, "memmove_count", "memmove", 16, 0, INT32_MAX,
     &CG_memmove_inst_count, &CG_memmove_inst_count_overridden,
     "Maximum size of inline expansion of memory intrinsics"},
    {OVK_BOOL, OV_VISIBLE, TRUE, "bcopy_cannot_overlap", "", 0, 0, 0,
     &CG_bcopy_cannot_overlap, NULL,
     "Assume that source and target of bcopy cannot overlap"},
    {OVK_BOOL, OV_VISIBLE, TRUE, "memcpy_cannot_overlap", "", 0, 0, 0,
     &CG_memcpy_cannot_overlap, NULL,
     "Assume that source and target of memcpy cannot overlap"},
    {OVK_BOOL, OV_VISIBLE, TRUE, "memmove_cannot_overlap", "", 0, 0, 0,
     &CG_memmove_cannot_overlap, NULL,
     "Assume that source and target of memmove cannot overlap"},
    {OVK_BOOL, OV_VISIBLE, TRUE, "memmove_nonconst", "", 0, 0, 0,
     &CG_memmove_nonconst, NULL,
     "Enable inline expansion of memory intrinsics (bzero, blkclr, bcopy, "
     "memset, memcpy, memmove) whose size is not a constant"},

    {OVK_BOOL, OV_INTERNAL, TRUE, "ipa_addr_analysis", "ipa_addr", 0, 0, 0,
     &OPT_IPA_addr_analysis, NULL,
     "Enable the use of IPA address analysis result in the backend"},

    {OVK_BOOL, OV_INTERNAL, TRUE, "recompute_addr_flags", "", 0, 0, 0,
     &OPT_recompute_addr_flags, NULL,
     "Recompute address flags in the backend (for debugging)"},

    {OVK_BOOL, OV_VISIBLE, TRUE, "instrumentation", "instr", 0, 0, 0,
     &Instrumentation_Enabled, NULL,
     "Allow adding instrumentation instructions for feedback profiling"},

    {OVK_INT32, OV_VISIBLE, TRUE, "fb_phase", "", 0, 0, 5,
     &Instrumentation_Phase_Num, NULL,
     "Phases in the compiler where instrumentation or feedback  needs to be "
     "done"},

    {OVK_INT32, OV_VISIBLE, TRUE, "fb_type", "", 0, 0, 14,
     &Instrumentation_Type_Num, NULL,
     "Types in the compiler where instrumentation needs to be done"},

    {OVK_UINT32, OV_INTERNAL, TRUE, "instrument_action", "", 0, 0, UINT32_MAX,
     &Instrumentation_Actions, NULL,
     "Phases in the compiler where instrumentation needs to be done"},

    {OVK_BOOL, OV_INTERNAL, FALSE, "instr_unique_output", "", 0, 0, 0,
     &Instrumentation_Unique_Output, NULL,
     "Always create a unique name for the profile data file"},

    {OVK_LIST, OV_VISIBLE, TRUE, "feedback", "feed", 0, 0, 0, &Feedback_Option,
     NULL, "Phases in the compiler where feedback needs to be done"},

    {OVK_BOOL, OV_INTERNAL, TRUE, "early_intrinsics", "", 0, 0, 0,
     &Inline_Intrinsics_Early, NULL, "Lower intrinsics early"},

    {OVK_BOOL, OV_INTERNAL, TRUE, "delay_u64_lowering", "delay_u64", 0, 0, 0,
     &Delay_U64_Lowering, NULL, "Delay unsigned 64-bit lowering to after wopt"},

    {OVK_BOOL, OV_INTERNAL, TRUE, "extract_bits", "extr", 0, 0, 0,
     &Enable_extract_bits, NULL, "Enable use of extract opcode"},

    {OVK_BOOL, OV_INTERNAL, TRUE, "compose_bits", "", 0, 0, 0,
     &Enable_compose_bits, NULL, "Enable use of compose opcode"},

    {OVK_BOOL, OV_VISIBLE, FALSE, "ansi_setjmp", "ansi_setjmp", 0, 0, 0,
     &LANG_Ansi_Setjmp_On, &LANG_Ansi_Setjmp_Set,
     "C/C++: enable optimization of functions with calls to setjmp"},

    {OVK_BOOL, OV_SHY, FALSE, "Olegacy", "Olegacy", 0, 0, 0, &Olegacy, NULL,
     "default options for performance on shared and alias"},

#ifdef __linux__
    {OVK_BOOL, OV_INTERNAL, TRUE, "wfe_dfe", "wfe_dfe", 0, 0, 0,
     &Enable_WFE_DFE, NULL, "Enable dead function elimination in the frontend"},
#endif /* __linux__ */

    /* Obsolete options: */

    {OVK_OBSOLETE, OV_INTERNAL, FALSE, "global_limit", NULL, 0, 0, 0, NULL,
     NULL, ""},

    {OVK_OBSOLETE, OV_INTERNAL, FALSE, "feopt", NULL, 0, 0, 0, NULL, NULL,
     "Enable special optimizations in front ends"},

    {OVK_OBSOLETE, OV_VISIBLE, FALSE, "fold_arith_limit", NULL, 0, 0, 0, NULL,
     NULL, "Limit size of subexpressions to be folded"},

    {OVK_OBSOLETE, OV_INTERNAL, FALSE, "fold_float", NULL, 0, 0, 0, NULL, NULL,
     "Allow expression folding optimizations of floating point"},

    {OVK_OBSOLETE, OV_INTERNAL, FALSE, "fprop_limit", NULL, 0, 0, INT32_MAX,
     NULL, NULL, ""},
