...
void get_license(char *feature_name, int soft, int tiers)
  ...
  */
    ...
    if (conf == NULL) {
      ...
      if ((conf->users == 0) && (conf->lc_vendor_def != NULL) &&
        ...
        if ((numCPU >= systemNumCPU)) {
          ...
          err = TRUE;
          fprintf(stderr,
                  " This product's license is for %d CPU's; Your system has %d "
                  "CPU's\n",
                  numCPU, systemNumCPU);
          fprintf(stderr, "\n");
        }
  ...
  if (err) {
   ...
   fprintf(stderr,"    Otherwise:\n");
    ...
    fprintf(stderr, " Graphics support customer then contact your local "
                    "support provider. \n");
    fprintf(stderr, "\n");
#if 0
