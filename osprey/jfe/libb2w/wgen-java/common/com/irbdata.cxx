...
inline void add_initv(INITV_IDX ninv, INITO_IDX ino, INITV_IDX inv) {
  if (inv != 0)
...
void INITV_Init_Integer(INITV_IDX inv, TYPE_ID mtype, INT64 val,
                        UINT16 repeat) {
  if (val == 0)
...
#ifdef TARG_IA64
void INITV_Init_Symiplt(INITV_IDX inv, ST *st, INT64 ofst, UINT16 repeat) {
...
void INITV_Init_Symdiff(INITV_IDX inv, LABEL_IDX lab1, ST *st2, BOOL halfword,
                        UINT16 repeat) {
  INITV_Set_SYMDIFF(Initv_Table[inv], repeat, lab1, ST_st_idx(st2), halfword);
}
...
Irb_Init_Symdiff(INITO_IDX ino, INITV_IDX inv, mUINT16 repeat, LABEL_IDX lab1,
  ...
  add_initv(idx, ino, inv);
  INITV_Set_SYMDIFF(initv, repeat, lab1, ST_st_idx(st2), halfword);
  return idx;
}
...
static INITV_IDX Irb_Init_predefined_integer(INITO_IDX ino, INITV_IDX inv,
                                             INT32 repeat, TYPE_ID mtype,
                                             INT32 val) {
  INITV_IDX idx;
  INITV &initv = Initv_Table.New_entry(idx);
...
Irb_Init_Complex(INT size, double real, double imag, INT32 repeat,
                 INITO_IDX ino, INITV_IDX inv) {
  TCON tc = Host_To_Targ_Complex(size == 8 ? MTYPE_C4 : MTYPE_C8, real, imag);
  return Irb_Init_Val(ino, inv, repeat, Enter_tcon(tc));
}
...
Irb_Init_Complex_4(INT size, float real, float imag, INT32 repeat,
                   INITO_IDX ino, INITV_IDX inv) {
  TCON tc = Host_To_Targ_Complex_4(size == 8 ? MTYPE_C4 : MTYPE_C8, real, imag);
  return Irb_Init_Val(ino, inv, repeat, Enter_tcon(tc));
}

INITV_IDX
Irb_Init_Complex_Quad(INT size, QUAD_TYPE real, QUAD_TYPE imag, INT32 repeat,
                      INITO_IDX ino, INITV_IDX inv) {
...
void Print_INITV(const INITV &initv) {
  ...
  case INITVKIND_VAL:
    repeat = INITV_repeat2(initv);
    fprintf(TFile, " VAL: %s", Targ_Print(NULL, Tcon_Table[INITV_tc(initv)]));
    break;

  case INITVKIND_SYMOFF:
    repeat = INITV_repeat1(initv);
    fprintf(TFile, " SYMOFF: %s(0x%x)+%d(0x%x)",
            ST_class(INITV_st(initv)) == CLASS_CONST ? "<constant>"
                                                     : ST_name(INITV_st(initv)),
            INITV_st(initv), INITV_ofst(initv), INITV_ofst(initv));
    break;
#ifdef TARG_IA64
...
void Print_INITV_idx(const INITV_IDX inv) { Print_INITV(Initv_Table[inv]); }

void Print_INITVs(INITV_IDX idx) { For_all_initv(idx, Print_INITV); }
...
void Print_INITO(const INITO &ino) { ino.Print(TFile); }
...
extern UINT Get_INITO_Size(INITO_IDX ino) {
