...
static const char rcs_id[] =
    "$Source: common/com/SCCS/s.em_dwarf.cxx $ $Revision: 1.21 $";
#endif
...
static void identity_function(Dwarf_Unsigned symidx, Dwarf_Unsigned *elf_symidx,
                              Dwarf_Unsigned *elf_symoff) {
  *elf_symidx = symidx;
  *elf_symoff = 0;
}

static Dwarf_Unsigned another_identity_function(Elf64_Word x) { return x; }

static symbol_index_recorder Record_Symidx;

static int /* TODO: should be size_t */
setup_new_section_for_dwarf(char *name, int is_64bit, Dwarf_Unsigned type,
                            Dwarf_Unsigned flags, Dwarf_Unsigned link,
                            Dwarf_Unsigned info, Dwarf_Unsigned *name_idx,
                            int *error) {
  pSCNINFO newscn;
  BOOL is_reloc;
  INT scnalign;
  ...
  if (num_dwarf_scns >= MAX_DWARF_SECTIONS) {
    fprintf(stderr,
            "ERROR: setup_new_section_for_dwarf: too many dwarf sections\n");
    exit(-1);
  }
...
pSCNINFO Em_Dwarf_Find_Dwarf_Scn(size_t scndx) {
  ...
  for (i = 0; i < num_dwarf_scns; i++) {
    if (Em_Get_Section_Index(dwarf_scn[i]) == scndx)
      return dwarf_scn[i];
  }
  return NULL;
}
...
void Em_Dwarf_Add_File(UINT16 file_idx, char *name, UINT16 incl_idx,
                       Dwarf_Unsigned modification_time,
                       Dwarf_Unsigned file_size) {
  Dwarf_Unsigned dwarf_idx;

  Increase_File_Table_Size(file_idx);
  dwarf_idx = dwarf_add_file_decl(dw_dbg, name, incl_table[incl_idx].dwarf_idx,
                                  modification_time, file_size, &dw_error);
...
Dwarf_P_Debug Em_Dwarf_Begin(BOOL is_64bit, BOOL dwarf_trace,
...
#define EXT_OP(v) (DW_CFA_extended | v)
  ...
  dw_dbg = dwarf_producer_init_b(flags, setup_new_section_for_dwarf, 0, 0,
                                 &dw_error);
  ...
  if (dst_lang == DW_LANG_C_plus_plus) /* may have eh info */ // czw
  {
    ...
    if (Gen_PIC_Call_Shared || Gen_PIC_Shared) {
      ...
      ST_Init(pic_personality_st, name, CLASS_VAR, SCLASS_DGLOBAL,
              EXPORT_HIDDEN, MTYPE_TO_TY_array[MTYPE_U8]);
      ...
      ST_ATTR_Init(st_attr, ST_st_idx(pic_personality_st), ST_ATTR_SECTION_NAME,
                   Save_Str(".gnu.linkonce.d.DW.ref.__gxx_personality_v0"));

      ST *personality_st = New_ST(GLOBAL_SYMTAB);
      ST_Init(personality_st, Save_Str("__gxx_personality_v0"), CLASS_VAR,
              SCLASS_EXTERN, EXPORT_PREEMPTIBLE, MTYPE_TO_TY_array[MTYPE_U8]);
  ...
  } else if (dst_lang == DW_LANG_Java) /* may have eh info */ // czw
  {
    ...
    if (Gen_PIC_Call_Shared || Gen_PIC_Shared) {
      ...
      ST_ATTR &st_attr = New_ST_ATTR(GLOBAL_SYMTAB, st_attr_idx);
      ST_ATTR_Init(
          st_attr, ST_st_idx(pic_personality_st), ST_ATTR_SECTION_NAME,
          Save_Str(".gnu.linkonce.d.DW.ref.__gcj_personality_v0")); // czw

      ST *personality_st = New_ST(GLOBAL_SYMTAB);
      ST_Init(personality_st, Save_Str("__gcj_personality_v0"), CLASS_VAR,
              SCLASS_EXTERN, EXPORT_PREEMPTIBLE,
              MTYPE_TO_TY_array[MTYPE_U8]); // czw
      INITV_IDX iv = New_INITV();
      INITV_Init_Symoff(iv, personality_st, 0, 1);
      New_INITO(ST_st_idx(pic_personality_st), iv);
      // bug 8315: With -ipa, if this file is not symtab.s (actually
  ...
  cie_index =
      dwf_add_frame_cie(dw_dbg, cie_augmenter, code_alignent_factor,
                        data_alignment_factor, return_reg,
                        0, // no personality
                        (Gen_PIC_Call_Shared || Gen_PIC_Shared), is_64bit,
                        cie_init_bytes, cie_init_byte_len, &dw_error);

#ifdef KEY
  // Generate a CIE for .eh_frame only if it is C++ or if -DEBUG:eh_frame=on
  if ((dst_lang == DW_LANG_C_plus_plus) || (dst_lang == DW_LANG_Java) ||
      DEBUG_Emit_Ehframe) // czw
    eh_cie_index = dwf_add_ehframe_cie(
        dw_dbg, augmenter, 1, data_alignment_factor,
        // In common/com/em_dwarf.cxx, we have
        // target description only for x86_64. So, we need to
        // specify what is the right column for x86.
        Is_Target_64bit() ? DW_FRAME_RA_COL : 0x8, personality,
        (Gen_PIC_Call_Shared || Gen_PIC_Shared), is_64bit,
#ifdef TARG_X8664
...
extern "C" Dwarf_P_Debug Em_Dwarf_Begin(BOOL is_64bit, BOOL dwarf_trace,
                                        DST_language dst_lang) // czw
{
...
static inline int find_indexed_section(int i, const Dwarf_Signed scn) {
...
static int advance_to_next_stream_buffer(const Dwarf_Signed scndx) {
  ...
  current_scn = find_indexed_section(current_scn + 1, scndx);
...
void Em_Dwarf_Write_Scns(Cg_Dwarf_Sym_To_Elfsym_Ofst translate_dwarf_sym) {
  ...
  index_to_buffer_map = (index_to_buffer_map_entry *)malloc(
      section_count * sizeof(index_to_buffer_map_entry));
  ...
  result = dwarf_get_relocation_info_count(dw_dbg, &relocation_section_count,
                                           &reloc_buffer_version, &dw_error);
  if (result != DW_DLV_OK) {
  ...
  for (i = 0; i < relocation_section_count; i++) {
    ...
    result = dwarf_get_relocation_info(dw_dbg, &scndx, &link_scn, &entry_count,
                                       &reloc_buf, &dw_error);
    if (result != DW_DLV_OK) {
    ...
    buffer = Em_Dwarf_Symbolic_Relocs_To_Elf(
        get_current_buffer, get_current_bufsize, advance_to_next_stream_buffer,
        link_scn, reloc_buf, entry_count,
        // Cheap hack to see if
        // this is a 64-bit compile:
        data_alignment_factor == -8,
        (is_debug_line ? identity_function : translate_dwarf_sym), &bufsize);
    cursection = Em_Dwarf_Find_Dwarf_Scn(scndx);
    if (cursection != NULL) {
  ...
  for (i = 0; i < section_count; ++i) {
    Em_Add_Bytes_To_Scn(index_to_buffer_map[i].cursection,
                        ...
                        index_to_buffer_map[i].bufsize, 1);
  }
...
extern "C" void Em_Dwarf_Write_Scns(void) { Em_Dwarf_Write_Scns(NULL); }
...
void Em_Dwarf_End(void) { dwarf_producer_finish(dw_dbg, &dw_error); }
...
void Em_Dwarf_Add_Line_Entry(INT code_address, SRCPOS srcpos) {
  ...
  dwarf_add_line_entry(dw_dbg, file_table[USRCPOS_filenum(usrcpos)].dwarf_idx,
                       code_address, USRCPOS_linenum(usrcpos),
                       USRCPOS_column(usrcpos), USRCPOS_stmt_begin(usrcpos),
                       USRCPOS_bb_begin(usrcpos), &dw_error);
}
...
void Em_Dwarf_Start_Text_Lines(pSCNINFO scninfo, INT start_offset) {
  ...
  dwarf_lne_set_address(dw_dbg, start_offset, Em_Create_Section_Symbol(scninfo),
                        &dw_error);
}
...
void Em_Dwarf_Start_Text_Region_Semi_Symbolic(
    pSCNINFO scninfo, INT start_offset, Dwarf_Unsigned first_label,
...
void Em_Dwarf_End_Text_Arange_Symbolic(Dwarf_Unsigned last_label,
  ...
  dwarf_add_arange_b(dw_dbg, Offset_From_Text_Start_Label, 0 /* dummy length */,
                     Text_Start_Label, last_label, offset_from_last_label,
                     &dw_error);
}
...
void Em_Dwarf_End_Text_Region_Semi_Symbolic(pSCNINFO scninfo, INT end_offset,
                                            Dwarf_Unsigned last_label,
                                            Dwarf_Addr offset_from_last_label) {
  Em_Dwarf_End_Text_Arange_Symbolic(last_label, offset_from_last_label);
  Em_Dwarf_End_Text_Lines(scninfo, end_offset);
}

void Em_Dwarf_Process_PU(Dwarf_Unsigned begin_label, Dwarf_Unsigned end_label,
                         INT32 begin_offset, INT32 end_offset,
                         Dwarf_P_Die PU_die, Dwarf_P_Fde fde,
#ifdef TARG_X8664
                         Dwarf_P_Fde eh_fde,
#endif
                         Elf64_Word eh_symindex, INT eh_offset) {
  /* setup the low_pc and the high_pc attributes. */
  dwarf_add_AT_targ_address_b(dw_dbg, PU_die, DW_AT_low_pc, begin_offset,
                              (Dwarf_Unsigned)begin_label, &dw_error);
  dwarf_add_AT_targ_address_b(dw_dbg, PU_die, DW_AT_high_pc, end_offset,
                              (Dwarf_Unsigned)end_label, &dw_error);

  if (fde == NULL)
    return;
  /* emit the debug_frame information for this procedure. */
  if (eh_offset == DW_DLX_NO_EH_OFFSET) /* no exception handler */
    dwarf_add_frame_fde_b(dw_dbg, fde, PU_die, cie_index, begin_offset,
                          0 /* dummy code length */,
                          (Dwarf_Unsigned)begin_label,
                          (Dwarf_Unsigned)end_label, end_offset, &dw_error);
  else
    dwarf_add_frame_info_b(
        dw_dbg, fde, PU_die, cie_index, begin_offset, 0 /* dummy code length */,
        (Dwarf_Unsigned)begin_label, (Dwarf_Unsigned)end_label, end_offset,
  ...
  if (eh_offset == DW_DLX_NO_EH_OFFSET) /* no exception handler */
    dwf_add_ehframe_fde_b(dw_dbg, eh_fde, PU_die, eh_cie_index, begin_offset,
                          0 /* dummy code length */,
                          (Dwarf_Unsigned)begin_label,
                          (Dwarf_Unsigned)end_label, end_offset, &dw_error);
  else
    dwf_add_ehframe_info_b(dw_dbg, eh_fde, PU_die, eh_cie_index, begin_offset,
                           0 /* dummy code length */,
                           (Dwarf_Unsigned)begin_label,
                           (Dwarf_Unsigned)end_label, end_offset, eh_offset,
                           eh_symindex, &dw_error);
#endif
}
#ifdef TARG_X8664
void Em_Dwarf_Add_PU_Entries(Dwarf_Unsigned begin_label,
                             Dwarf_Unsigned end_label, INT32 begin_offset,
                             INT32 end_offset, Dwarf_P_Die PU_die,
                             Dwarf_P_Fde fde) {
  dwarf_add_AT_targ_address_b(dw_dbg, PU_die, DW_AT_low_pc, begin_offset,
                              (Dwarf_Unsigned)begin_label, &dw_error);
  dwarf_add_AT_targ_address_b(dw_dbg, PU_die, DW_AT_high_pc, end_offset,
  ...
  /* emit the debug_frame information for this procedure. */
  dwarf_add_frame_fde_b(dw_dbg, fde, PU_die, cie_index, begin_offset,
                        0 /* dummy code length */, (Dwarf_Unsigned)begin_label,
                        (Dwarf_Unsigned)end_label, end_offset, &dw_error);
}
#endif
