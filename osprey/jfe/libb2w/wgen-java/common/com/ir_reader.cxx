...
#define OPCODE_has_alias_info(opc)                                             \
  (OPCODE_is_load(opc) || OPCODE_is_store(opc) ||                              \
   OPCODE_operator(opc) == OPR_PARM)

#define IR_dump_alias_info(opc)                                                \
  (IR_alias_map != WN_MAP_UNDEFINED && OPCODE_has_alias_info(opc))
...
/* If prefix is true, set the dump order to prefix order instead of postfix
 * order */
...
#ifndef MONGOOSE_BE
extern void IR_reader_finish(void) { MEM_POOL_Pop(&MEM_local_pool); }
...
extern void IR_close(void) { fclose(ir_file); }
...
extern BOOL IR_open_output(char *filename) {
  if (filename == NULL)
    ir_ofile = stdout;
  else if ((ir_ofile = fopen(filename, "w+")) == NULL) {
...
#define ir_chk_kids(m, n)                                                      \
  {                                                                            \
    if (m != n)                                                                \
      ir_error("wrong number of kids");                                        \
  }
  ...
  for (idx = DST_get_file_names(); !DST_IS_NULL(idx);
    ...
    if (count >= file_table_size) {
      ...
      if (file_table == NULL)
        ...
        file_table =
            (file_info *)realloc(file_table, new_size * sizeof(file_info));
      if (file_table == NULL)
...
static BOOL ir_insert_hash(char *s, IR_OPCODE_TABLE *opcode_entry) {
  ...
  for (p = s; *p != '\0'; p++)
    sum =
        (sum + sum + 3377) ^ *p; /* better than sum+= *p for our keyword set */
                                 // if (sum < 0) sum = -sum;
  sum %= HASH_LEN;
...
static inline void enter_opcode_table(char *name, OPC_EXTENDED opc, INT opr) {
...
extern "C" BOOL LnoDependenceEdge(WN *, WN *, mUINT16 *, DIRECTION *, BOOL *,
                                  BOOL *);
#endif

/*
 * little routine to print TY's and their attributes, since otherwise they
 * aren't visible
 */
static void ir_put_ty(TY_IDX ty) {
  fprintf(ir_ofile, " T<%d,%s,%d", TY_id(ty), TY_name(ty), TY_align(ty));
...
static void ir_put_wn(WN *wn, INT indent) {
  ...
  if (OPCODE_has_offset(opcode)) {
    ...
    fprintf(ir_ofile, " %d %d", WN_loop_trip_est(wn), WN_loop_depth(wn));
  }
  ...
  if (follow_st && OPCODE_has_sym(opcode) && OPCODE_has_offset(opcode) &&
    ...
    if (Preg_Is_Dedicated(WN_offset(wn))) {
      if (Preg_Offset_Is_Int(WN_offset(wn))) {
        ...
        fprintf(ir_ofile, " # $f%d", WN_offset(wn) - Float_Preg_Min_Offset);
      }
#ifdef TARG_X8664
      else if (Preg_Offset_Is_X87(WN_offset(wn))) {
        fprintf(ir_ofile, " # $st%d", WN_offset(wn) - X87_Preg_Min_Offset);
      }
#endif
    } else {
      ...
      if ((WN_offset(wn) - Last_Dedicated_Preg_Offset) <
        ...
        fprintf(ir_ofile, " # <Invalid PREG Table index (%d)>", WN_offset(wn));
    }
  }
  ...
  if (OPCODE_operator(opcode) == OPR_PARM) {
    ...
    if (flag & WN_PARM_BY_REFERENCE)
      fprintf(ir_ofile, " by_reference ");
    if (flag & WN_PARM_BY_VALUE)
      fprintf(ir_ofile, " by_value ");
    if (flag & WN_PARM_OUT)
      fprintf(ir_ofile, " out ");
    if (flag & WN_PARM_DUMMY)
      fprintf(ir_ofile, " dummy ");
    if (flag & WN_PARM_READ_ONLY)
      fprintf(ir_ofile, " read_only ");
    if (flag & WN_PARM_PASSED_NOT_SAVED)
      fprintf(ir_ofile, "passed_not_saved ");
    if (flag & WN_PARM_NOT_EXPOSED_USE)
      fprintf(ir_ofile, " not_euse ");
    if (flag & WN_PARM_IS_KILLED)
      fprintf(ir_ofile, " killed ");
  }

  if (IR_dump_map_info) {
    fprintf(ir_ofile, " # <id %d:%d>", OPCODE_mapcat(opcode), WN_map_id(wn));
  ...
  if (IR_freq_map != WN_MAP_UNDEFINED &&
      (OPCODE_is_scf(WN_opcode(wn)) || OPCODE_is_stmt(WN_opcode(wn)))) {
    USRCPOS srcpos;
    USRCPOS_srcpos(srcpos) = WN_Get_Linenum(wn);

    fprintf(ir_ofile, " {freq: %d, ln: %d, col: %d}",
            WN_MAP32_Get(IR_freq_map, wn), USRCPOS_linenum(srcpos),
            USRCPOS_column(srcpos));
  }
  if (Current_Map_Tab != NULL && WN_MAP32_Get(WN_MAP_ALIAS_CLASS, wn) != 0) {
    fprintf(ir_ofile, " {class %d}", WN_MAP32_Get(WN_MAP_ALIAS_CLASS, wn));
  }
  fprintf(ir_ofile, "\n");
}
...
static void ir_put_expr(WN *wn, INT indent) {
  ...
  for (i = 0; i < WN_kid_count(wn); i++) {
    ...
    if (wn2) {
      ...
      else if (op == OPC_BLOCK && ((WN_operator(wn) == OPR_RCOMMA && i == 1) ||
...
static void fdump_DUMPDEP(FILE *f, DUMPDEPp head, struct ALIAS_MANAGER *alias) {
  ...
  fprintf(f, "\nDUMP DEPENDENCY DUMP: == SAME_LOCATION, != NOT_ALIASED, ? "
             "POSSIBLY ALIASED\n");
  ...
  for (p = head; p; p = DUMPDEP_next(p)) {
    ...
    if (Valid_alias(alias, node)) {
      ...
      for (q = head; q; q = DUMPDEP_next(q)) {
        ...
        if (Valid_alias(alias, wn) && SAME_LOCATION == Aliased(alias, node, wn))
      ...
      for (q = head; q; q = DUMPDEP_next(q)) {
        WN *wn = DUMPDEP_node(q);
        if (Valid_alias(alias, wn) && NOT_ALIASED == Aliased(alias, node, wn))
          fprintf(f, "%d,", DUMPDEP_id(q));
      }
    ...
    for (q = head; q; q = DUMPDEP_next(q)) {
      ...
      if (LnoDependenceEdge(node, DUMPDEP_node(q), &dist, &dir, &is_must,
                            &ok)) {
        fprintf(f, "<%d: %d, %s", DUMPDEP_id(q), dist, is_must ? "MUST " : "");
...
extern void dump_tree(WN *wn) { fdump_tree(stdout, wn); }
...
extern void fdump_tree_with_alias(FILE *f, const WN *wn, WN_MAP map,
...
extern void enable_tree_freq_display(void) { IR_freq_map = WN_MAP_FEEDBACK; }

extern void disable_tree_freq_display(void) { IR_freq_map = WN_MAP_UNDEFINED; }
  ...
  if (dump_parent_before_children) {
    ...
    while (tree_iter != WN_TREE_ITER<PRE_ORDER, WN *>()) {
      ...
      if (OPCODE_is_expression(WN_opcode(wn2)) ||
        ...
        Is_True(parent_wn && ((WN_operator(parent_wn) == OPR_COMMA) ||
...
extern void WN_TREE_dump_tree(WN *wn) { WN_TREE_fdump_tree(stdout, wn); }
