...
static inline void *Symtab_Alloc(size_t bytes, BOOL is_global) {
  if (is_global) {
...
static DST_IDX DST_CHECK_IDX(DST_IDX i) {
  Is_True(
      ((i.byte_idx >= 0 &&
        i.byte_idx <
            current_DST->dst_blocks[DST_CHECK_BLOCK_IDX(i.block_idx)].size)) ||
          DST_IS_FOREIGN_OBJ(i),
      ("Illegal DST idx"));
  return i;
}
...
extern void DST_Init(char *start, INT32 num_blocks) {
  INT i;
  block_header *blocks;
  ...
  if (start == NULL) {
    ...
    blocks = (block_header *)Symtab_Alloc(
        NUM_BLOCK_HEADERS * sizeof(block_header), TRUE);
  } else {
...
static block_header *new_block(DST_BLOCK_KIND kind, INT32 size) {
  ...
  if (++last_block >= max_block) {
    blocks = TYPE_MEM_POOL_REALLOC_N(block_header, MEM_src_pool_ptr, blocks,
   ...
   */
  blocks[last_block].offset = (char *)Symtab_Alloc(allocsize, TRUE);
#else
  blocks[last_block].offset = (char *)Symtab_Alloc(
      allocsize, (kind == DST_local_scope_block) ? FALSE : TRUE);
#endif
...
extern void DST_begin_block(DST_BLOCK_KIND block_kind) {
  DST_BLOCK_IDX *blklist;
  current_DST = (DST_Type *)Current_DST;
  blklist = &current_DST->block_list[0];
...
extern DST_IDX DST_allocate(INT32 size, INT32 align) {
  DST_IDX new_idx;
  INT32 total_size;
 ...
 */
extern void DST_return_to_block(DST_IDX idx) {
 ...
 * written to file.
 */
extern char *DST_idx_to_string(DST_IDX idx) {
  block_header *block;
  current_DST = (DST_Type *)Current_DST;
 ...
 */
extern DST_IDX DST_get_include_dirs(void) {
  current_DST = (DST_Type *)Current_DST;
 ...
 */
extern DST_IDX DST_get_file_names(void) {
  current_DST = (DST_Type *)Current_DST;
 ...
 */
extern DST_IDX DST_get_macro_info(void) {
  current_DST = (DST_Type *)Current_DST;
 ...
 */
extern DST_IDX DST_get_compile_unit(void) {
  current_DST = (DST_Type *)Current_DST;
