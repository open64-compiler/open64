 ...
 * DST_DIR_INFO.
 */
static void DST_enter_mk(DST_PRODUCER_STATE new_state, DST_IDX last_idx) {
...
#define DST_check_info_idx(required_tag, idx)                                  \
  DST_ASSERT((DST_IS_FOREIGN_OBJ(idx)) ||                                      \
                 (DST_INFO_tag(DST_INFO_IDX_TO_PTR(idx)) == required_tag),     \
...
static void DST_set_last_file_name(void) {
  DST_IDX idx;
  num_file_names = 0;
  for (idx = DST_get_file_names(); !DST_IS_NULL(idx);
       idx = DST_FILE_NAME_next(DST_FILE_IDX_TO_PTR(idx))) {
    num_file_names++;
    last_file_name = idx;
  }
}

static void DST_set_last_include_dir(void) {
  DST_IDX idx;
  num_incl_dirs = 0;
  for (idx = DST_get_include_dirs(); !DST_IS_NULL(idx);
       idx = DST_INCLUDE_DIR_next(DST_DIR_IDX_TO_PTR(idx))) {
    num_incl_dirs++;
...
mUINT16 DST_number_of_include_dirs(void) { return num_incl_dirs; }
 ...
 */
DST_IDX
DST_mk_file_name(char *file_name, mUINT16 incl_dir, UINT64 size, UINT64 modt) {
...
mUINT16 DST_number_of_files(void) { return num_file_names; }

#ifdef KEY
mUINT16 DST_number_of_macros(void) { return num_macros; }
#endif
...
BOOL DST_is_locked_to_file_scope_mem(void) { return file_scope_locks > 0; }

/* Initiates the fields of a new DST_INFO record.
 */
static DST_INFO_IDX DST_init_info(DST_INFO_IDX info_idx, DST_DW_tag tag,
                                  DST_flag flag, DST_ATTR_IDX attrs) {
  DST_INFO *info_ptr;
 ...
 */
DST_INFO_IDX
DST_mk_compile_unit(char *src_path, char *comp_dir, char *comp_info,
                    DST_language language, DST_identifier_case id_case) {
...
DST_mk_inlined_subroutine(void *low_pc,  /* ptr to front-end label */
  ...
  return DST_init_info(info_idx, DW_TAG_inlined_subroutine, flag, attr_idx);
}

#if defined(_SUPPORT_IPA) || defined(_STANDALONE_INLINER)
/* These are not needed with _LEGO_CLONER */

void DST_label_add_name(DST_LABEL *attr, char *label_name) {
  DST_LABEL_name(attr) = DST_mk_name(label_name);
}

void DST_lexical_block_add_name(DST_LEXICAL_BLOCK *attr, char *block_name) {
 ...
 */
static mUINT16 DST_get_ordinal_num(DST_DIR_IDX dir_idx, char *dir_name) {
...
static char *DST_get_dirname(mUINT16 ordinal) {
...
char *DST_get_file(mUINT16 ordinal, UINT64 *file_size, UINT64 *fmod_time,
                   char **dirname) {
 ...
 * index
 */
static mUINT16 DST_get_file_id(char *filename, char *dirname, UINT64 file_size,
                               UINT64 fmod_time) {
  DST_FILE_IDX f_idx = DST_get_file_names();
  DST_FILE_NAME *f_ptr;
  mUINT16 num = 0;
  ...
  while (!DST_IS_NULL(f_idx)) {
    ...
    if ((DST_FILE_NAME_size(f_ptr) == file_size) &&
        ...
        (strcmp(filename, DST_STR_IDX_TO_PTR(DST_FILE_NAME_name(f_ptr))) ==
         0) &&
...
/* add the file into the file directory, return the ordinal num of the
   new file
 */
static mUINT16 DST_enter_file(char *filename, char *dirname, UINT64 file_size,
                              UINT64 fmod_time) {
  DST_mk_file_name(filename,
                   DST_get_ordinal_num(DST_get_include_dirs(), dirname),
                   file_size, fmod_time);

  return (num_file_names);
}

mUINT16 DST_get_cross_inlined_file_id(
...
DST_mk_cross_inlined_subroutine(
    ...
    mUINT16
        *file_index,  /* returns the file_index of entry created for filename */
    UINT64 file_size, /* File size (bytes) */
    UINT64 fmod_time, /* Last file mod time */
                         ...
                         DST_INFO_IDX spec) /* decl in class */
{
  ...
  return DST_init_info(info_idx, DW_TAG_subprogram, DST_flag_memdef, attr_idx);
}
 ...
 */
DST_INFO_IDX
DST_mk_subprogram(USRCPOS decl, char *name, DST_INFO_IDX type,
                  DST_INFO_IDX origin, void *subpr, /* front-end routine */
                  DST_inline inlin, DST_virtuality virtuality,
                  DST_vtable_elem_location vtable_elem_location,
                  BOOL is_declaration, BOOL is_prototyped,
#ifdef KEY
                  BOOL is_artificial,
#endif
                  BOOL is_external) {
  ...
  DST_INFO_IDX t = DST_init_info(info_idx, DW_TAG_subprogram, flag, attr_idx);
  return t;
}
 ...
 */
void DST_subprogram_concrete_to_abstract(DST_INFO_IDX subprogram) {
  DST_INFO *info;
  DST_ATTR_IDX attr_idx;
  DST_SUBPROGRAM *attr;
  DST_flag flag;
  ...
  DST_ASSERT((DST_INFO_tag(info) == DW_TAG_subprogram),
             "Bad DST_INFO_IDX input to DST_subprogram_concrete_to_abstract -- "
             "should be DW_TAG_subprogram");
 ...
 */
DST_INFO_IDX
DST_mk_cloned_subprogram(USRCPOS decl, char *name, DST_INFO_IDX type,
 ...
 */
void DST_add_linkage_name_to_variable(DST_INFO_IDX variable_def,
                                      char *linkage_name) {
  DST_INFO *info;
  DST_ATTR_IDX attr_idx;
  DST_VARIABLE *attr;
 ...
 */
DST_INFO_IDX
DST_mk_entry_point(USRCPOS decl, char *name, DST_INFO_IDX type,
                   void *subpr) /* front-end routine */
{
  DST_INFO_IDX info_idx;
  DST_ATTR_IDX attr_idx;
 ...
 */
DST_INFO_IDX
DST_mk_common_block(char *name, void *subpr) {
  DST_INFO_IDX info_idx;
  DST_ATTR_IDX attr_idx;
  DST_flag flag = DST_no_flag;
 ...
 */
DST_INFO_IDX
DST_mk_common_incl(USRCPOS decl, DST_INFO_IDX comblk) {
  DST_INFO_IDX info_idx;
  DST_ATTR_IDX attr_idx;
  DST_flag flag = DST_no_flag;
 ...
 */
DST_INFO_IDX
DST_mk_imported_decl(char *mangled_name, char *name) {
  DST_INFO_IDX info_idx;
  DST_ATTR_IDX attr_idx;
...
DST_mk_module(USRCPOS decl, /* source location */
  ...
  DST_SET_declaration(flag);
  DST_INFO_IDX t = DST_init_info(info_idx, DW_TAG_module, flag, attr_idx);
  return t;
}
#endif /* KEY Bug 3507 */
...
DST_mk_variable_const(USRCPOS decl,      /* Source location */
                      ...
                      BOOL is_automatic, BOOL is_external,
...
DST_mk_variable(USRCPOS decl,      /* Source location */
                ...
                BOOL is_declaration, BOOL is_automatic,
                              ...
                              DST_INFO_IDX abstract_origin) /* For inlined */
{
  ...
  return DST_init_info(info_idx, DW_TAG_unspecified_parameters, flag, attr_idx);
}
                          ...
                          DST_INFO_IDX class_type) /* Type of class */
{
  ...
  return DST_init_info(info_idx, DW_TAG_ptr_to_member_type, flag, attr_idx);
}
 ...
 */
DST_INFO_IDX
DST_mk_subrange_type(DST_flag is_lb_cval, DST_cval_ref low,  /* lower bound */
                     DST_flag is_ub_cval, DST_cval_ref high) /* upper bound */
{
  DST_INFO_IDX info_idx;
  DST_ATTR_IDX attr_idx;
  DST_flag flag = DST_no_flag;
 ...
 */
extern DST_INFO_IDX DST_mk_string_type(USRCPOS decl, /* Source location */
                                       char *name,   /* Name of type */
...
DST_mk_structure_type(USRCPOS decl,         /* Source location */
                      ...
                      DST_INFO_IDX abstract_origin, BOOL is_incomplete) {
  DST_INFO_IDX info_idx;
  DST_ATTR_IDX attr_idx;
  DST_flag flag = DST_no_flag;
...
DST_mk_member(USRCPOS decl,           /* Source location */
              ...
              BOOL is_bitfield, BOOL is_static, BOOL is_declaration,
              BOOL is_artificial)
#else
DST_INFO_IDX
DST_mk_member(USRCPOS decl,           /* Source location */
              ...
              DST_bitsize bit_size,   /* Size of bitfield member */
              BOOL is_bitfield, BOOL is_static, BOOL is_declaration,
              BOOL is_artificial, DST_accessibility accessibility)
#endif
{
  DST_INFO_IDX info_idx;
  DST_ATTR_IDX attr_idx;
                               ...
                               DST_INFO_IDX type) /* Actual type */
{
  ...
  return DST_init_info(info_idx, DW_TAG_template_type_param, flag, attr_idx);
}
                                ...
                                DST_CONST_VALUE cval) /* Actual type */
{
  ...
  return DST_init_info(info_idx, DW_TAG_template_value_param, flag, attr_idx);
}
