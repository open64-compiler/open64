...
ST *New_Const_Sym(TCON_IDX tcon, TY_IDX ty) {
  ...
  if (iter == merge.end()) {
    ...
    st = (*iter).second;
    Is_True(ST_class(st) == CLASS_CONST && ST_sclass(st) == SCLASS_FSTATIC &&
                ST_export(st) == EXPORT_LOCAL && ST_is_initialized(st),
            ("Mismatched const ST"));
  }
  return st;
...
TCON Const_Val(WN *n) { return WN_val(n); }
...
#endif
  default:
    Is_True(FALSE, ("Make_Const can not handle %s", Mtype_Name(TCON_ty(c))));
    return NULL;
  }
...
WN *Make_Comparison_Result_Const(INT16 val) {
...
WN *Make_Integer_Const(INT16 mtype, TARG_INT val) {
...
WN *Make_Reduction_Identity(INT32 opr, TYPE_ID mtype) {
  ...
  case OPR_SUB:
    ...
    case MTYPE_U4:
    case MTYPE_U8:
      r = WN_CreateIntconst(OPCODE_make_op(OPR_INTCONST, ntype, MTYPE_V), 0);
      break;
  ...
  case OPR_DIV:
    ...
    case MTYPE_U4:
    case MTYPE_U8:
      r = WN_CreateIntconst(OPCODE_make_op(OPR_INTCONST, ntype, MTYPE_V), 1);
      break;
  ...
  case OPR_MIN:
    switch (mtype) {
    case MTYPE_I1:
      r = WN_CreateIntconst(OPCODE_make_op(OPR_INTCONST, ntype, MTYPE_V), 0x7f);
      break;
    ...
    case MTYPE_U1:
      r = WN_CreateIntconst(OPCODE_make_op(OPR_INTCONST, ntype, MTYPE_V), 0xff);
      break;
  ...
  case OPR_CIOR: /* C/C++ short-circuit "||" on int/float */
    ...
    case MTYPE_F10:
    case MTYPE_FQ:
      Is_True(opr == OPR_CAND || opr == OPR_CIOR, ("bad opr %d", (INT)opr));
      r = Make_Const(Host_To_Targ_Float(ntype, (opr == OPR_CAND) ? 1.0 : 0.0));
      break;
