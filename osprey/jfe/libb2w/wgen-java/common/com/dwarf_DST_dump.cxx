...
static const char rcs_id[] =
    "$Source: common/com/SCCS/s.dwarf_DST_dump.cxx $ $Revision: 1.13 $";
#endif
...
static void DST_put_idx_attribute(const char *at_name, DST_IDX i,
                                  BOOL is_type) {
...
#ifdef KEY
static void DST_put_C4_attribute(const char *at_name, UINT32 real,
                                 UINT32 imag) {
  DST_put_string(at_name);
...
static void DST_put_C8_attribute(const char *at_name, UINT64 real,
                                 UINT64 imag) {
  DST_put_string(at_name);
...
#ifdef KEY
static void DST_put_accessibility_attribute(const char *at_name,
...
static void DST_put_assoc(const char *at_name, DST_flag flag,
  ...
  if (DST_IS_assoc_fe(flag))
    ...
    DST_put_string("ST");
    DST_put_st_id(DST_ASSOC_INFO_st_level(assoc),
                  DST_ASSOC_INFO_st_index(assoc));
  }
  DST_nput_char(1, ')');
}
...
static void DST_put_const_attribute(const char *at_name, DST_CONST_VALUE cval) {
  ...
  case DST_FORM_DATA1:
    DST_put_UINT32_attribute(at_name, (UINT32)DST_CONST_VALUE_form_data1(cval));
    break;

  case DST_FORM_DATA2:
    DST_put_UINT32_attribute(at_name, (UINT32)DST_CONST_VALUE_form_data2(cval));
    break;

  case DST_FORM_DATA4:
    DST_put_UINT32_attribute(at_name, (UINT32)DST_CONST_VALUE_form_data4(cval));
    break;

  case DST_FORM_DATA8:
    DST_put_UINT64_attribute(at_name, (UINT64)DST_CONST_VALUE_form_data8(cval));
    break;
#ifdef KEY
  case DST_FORM_DATAC4:
    DST_put_C4_attribute(at_name, (UINT32)DST_CONST_VALUE_form_crdata4(cval),
                         (UINT32)DST_CONST_VALUE_form_cidata4(cval));
    break;

  case DST_FORM_DATAC8:
    DST_put_C8_attribute(at_name, (UINT64)DST_CONST_VALUE_form_crdata8(cval),
...
#ifdef KEY /* Bug 3507 */
static void DST_put_module(DST_flag flag, DST_MODULE *attr) {
  DST_put_string(":module:");
  DST_put_decl(DST_MODULE_decl(attr));
  DST_put_string_attribute(" name", DST_MODULE_name(attr));
}

static void DST_put_imported_decl(DST_flag flag, DST_IMPORTED_DECL *attr) {
...
static void DST_put_subprogram(DST_flag flag, DST_SUBPROGRAM *attr) {
  ...
  } else /* definition */
  {
    ...
    DST_put_idx_attribute(" clone_origin",
                          DST_SUBPROGRAM_def_clone_origin(attr), FALSE);
...
static void DST_put_inlined_subroutine(DST_flag flag,
  ...
  DST_put_idx_attribute(" abstract_origin",
                        DST_INLINED_SUBROUTINE_abstract_origin(attr), FALSE);
  if (DST_IS_FOREIGN_OBJ(DST_INLINED_SUBROUTINE_abstract_origin(attr)))
    DST_put_string_attribute(" abstract_name",
...
static void DST_put_typedef(DST_flag flag, DST_TYPEDEF *attr) {
  ...
  DST_put_idx_attribute(" abstract_origin", DST_TYPEDEF_abstract_origin(attr),
                        FALSE);
}
...
static void DST_put_subrange_type(DST_flag flag, DST_SUBRANGE_TYPE *attr) {
  ...
  if (DST_IS_lb_cval(flag))
    DST_put_INT32_attribute(" lower", DST_SUBRANGE_TYPE_lower_cval(attr));
  else
    DST_put_idx_attribute(" lower", DST_SUBRANGE_TYPE_lower_ref(attr), FALSE);
  ...
  if (DST_IS_ub_cval(flag))
    DST_put_INT32_attribute(p, DST_SUBRANGE_TYPE_upper_cval(attr));
  else
    DST_put_idx_attribute(p, DST_SUBRANGE_TYPE_upper_ref(attr), FALSE);
...
static void DST_put_string_type(DST_flag flag, DST_STRING_TYPE *attr) {
  ...
  if (DST_IS_cval(flag))
    DST_put_INT32_attribute(" length", DST_STRING_TYPE_len_cval(attr));
  else
    DST_put_idx_attribute(" length", DST_STRING_TYPE_len_ref(attr), FALSE);
}
...
static void DST_put_member(DST_flag flag, DST_MEMBER *attr) {
  ...
  DST_put_INT32_attribute(" data_member_location", DST_MEMBER_memb_loc(attr));
...
static void DST_put_template_type_param(DST_flag flag,
  ...
  DST_put_idx_attribute(" type", DST_TEMPLATE_TYPE_PARAMETER_type(attr), TRUE);
}
...
static INT32 DST_dump_info(INT32 indentation, DST_DW_tag tag, DST_flag flag,
                           DST_ATTR_IDX iattr, DST_INFO_IDX iinfo) {
  DST_write_line();
  ...
  case DW_TAG_compile_unit:
    DST_put_compile_unit(flag, DST_ATTR_IDX_TO_PTR(iattr, DST_COMPILE_UNIT));
    break;
#ifdef KEY /* Bug 3507 */
  case DW_TAG_module:
    DST_put_module(flag, DST_ATTR_IDX_TO_PTR(iattr, DST_MODULE));
    break;
  case DW_TAG_imported_declaration:
    DST_put_imported_decl(flag, DST_ATTR_IDX_TO_PTR(iattr, DST_IMPORTED_DECL));
    break;
#endif /* KEY Bug 3507 */
  case DW_TAG_subprogram:
    DST_put_subprogram(flag, DST_ATTR_IDX_TO_PTR(iattr, DST_SUBPROGRAM));
    break;
  ...
  case DW_TAG_entry_point:
    DST_put_entry_point(flag, DST_ATTR_IDX_TO_PTR(iattr, DST_ENTRY_POINT));
    break;
  case DW_TAG_common_block:
    DST_put_common_block(flag, DST_ATTR_IDX_TO_PTR(iattr, DST_COMMON_BLOCK));
    break;
  case DW_TAG_common_inclusion:
    DST_put_common_inclusion(flag, DST_ATTR_IDX_TO_PTR(iattr, DST_COMMON_INCL));
    break;
  case DW_TAG_lexical_block:
    DST_put_lexical_block(flag, DST_ATTR_IDX_TO_PTR(iattr, DST_LEXICAL_BLOCK));
    break;
  case DW_TAG_label:
    DST_put_label(flag, DST_ATTR_IDX_TO_PTR(iattr, DST_LABEL));
    break;
  case DW_TAG_variable:
    DST_put_variable(flag, DST_ATTR_IDX_TO_PTR(iattr, DST_VARIABLE));
    break;
  case DW_TAG_formal_parameter:
    DST_put_formal_parameter(flag,
                             DST_ATTR_IDX_TO_PTR(iattr, DST_FORMAL_PARAMETER));
    break;
  case DW_TAG_unspecified_parameters:
    DST_put_unspecified_parameters(
        flag, DST_ATTR_IDX_TO_PTR(iattr, DST_UNSPECIFIED_PARAMETERS));
    break;
  case DW_TAG_base_type:
    DST_put_basetype(flag, DST_ATTR_IDX_TO_PTR(iattr, DST_BASETYPE));
    break;
  case DW_TAG_const_type:
    DST_put_const_type(flag, DST_ATTR_IDX_TO_PTR(iattr, DST_CONST_TYPE));
    break;
  case DW_TAG_constant:
    DST_put_constant(flag, DST_ATTR_IDX_TO_PTR(iattr, DST_CONSTANT));
    break;
  case DW_TAG_volatile_type:
    DST_put_volatile_type(flag, DST_ATTR_IDX_TO_PTR(iattr, DST_VOLATILE_TYPE));
    break;
  case DW_TAG_pointer_type:
    DST_put_pointer_type(flag, DST_ATTR_IDX_TO_PTR(iattr, DST_POINTER_TYPE));
    break;
  case DW_TAG_reference_type:
    ...
    break;
  case DW_TAG_typedef:
    DST_put_typedef(flag, DST_ATTR_IDX_TO_PTR(iattr, DST_TYPEDEF));
    break;
  case DW_TAG_array_type:
    DST_put_array_type(flag, DST_ATTR_IDX_TO_PTR(iattr, DST_ARRAY_TYPE));
    break;
  case DW_TAG_subrange_type:
    DST_put_subrange_type(flag, DST_ATTR_IDX_TO_PTR(iattr, DST_SUBRANGE_TYPE));
    break;
  case DW_TAG_string_type:
    DST_put_string_type(flag, DST_ATTR_IDX_TO_PTR(iattr, DST_STRING_TYPE));
    break;
  ...
  case DW_TAG_class_type:
    DST_put_class_type(flag, DST_ATTR_IDX_TO_PTR(iattr, DST_CLASS_TYPE));
    break;
  case DW_TAG_union_type:
    DST_put_union_type(flag, DST_ATTR_IDX_TO_PTR(iattr, DST_UNION_TYPE));
    break;
  case DW_TAG_member:
    DST_put_member(flag, DST_ATTR_IDX_TO_PTR(iattr, DST_MEMBER));
    break;
  case DW_TAG_inheritance:
    DST_put_inheritance(flag, DST_ATTR_IDX_TO_PTR(iattr, DST_INHERITANCE));
    break;
  case DW_TAG_template_type_param:
    DST_put_template_type_param(
        flag, DST_ATTR_IDX_TO_PTR(iattr, DST_TEMPLATE_TYPE_PARAMETER));
    break;
  case DW_TAG_template_value_param:
    DST_put_template_value_param(
        flag, DST_ATTR_IDX_TO_PTR(iattr, DST_TEMPLATE_VALUE_PARAMETER));
    break;
  case DW_TAG_enumeration_type:
    DST_put_enumeration_type(flag,
                             DST_ATTR_IDX_TO_PTR(iattr, DST_ENUMERATION_TYPE));
    break;
  case DW_TAG_enumerator:
    DST_put_enumerator(flag, DST_ATTR_IDX_TO_PTR(iattr, DST_ENUMERATOR));
    break;
  case DW_TAG_subroutine_type:
    DST_put_subroutine_type(flag,
 ...
 */
void DST_dump(DST_DIR_IDX incl_dirs, DST_FILE_IDX files,
...
void DST_set_dump_filename(char *file_name) { dumpf_name = file_name; }
