 ...
 */
extern Elf64_Word ir_b_save_buf(const void *buf, Elf64_Word size,
                                unsigned int align, unsigned int padding,
                                Output_File *fl) {
  Current_Output = fl;
 ...
 */
extern Elf64_Word ir_b_copy_file(const void *buf, Elf64_Word size,
                                 void *tmpfl) {
...
static void save_buf_at_offset(const void *buf, Elf64_Word size, off_t offset,
                               Output_File *fl) {
...
static Elf64_Word ir_b_reserve_space(Elf64_Word size, unsigned int align,
                                     Output_File *fl) {
...
#include <ipa_cg.h>
// ******************** IPA weak symbols$
extern Elf64_Word ir_b_write_tree(WN *node, off_t base_offset, Output_File *fl,
                                  WN_MAP off_map, PU_Info *pu)
#else
extern Elf64_Word ir_b_write_tree(WN *node, off_t base_offset, Output_File *fl,
                                  WN_MAP off_map)
#endif
...
#define WN_ADDR(offset) ((WN *)(fl->map_addr + offset))
  ...
  if (off_map != WN_MAP_UNDEFINED &&
      (Write_BE_Maps || Write_ALIAS_CLASS_Map || Write_AC_INTERNAL_Map)) {
    ...
    if (Write_BE_Maps) {
      ...
      if (WN_MAP_Get(WN_MAP_PREFETCH, node)) {
        ...
        if (num_prefetch_ldsts == 0) {
          ...
          prefetch_ldsts = (WN **)malloc(max_num_prefetch_ldsts * sizeof(WN *));
          FmtAssert(prefetch_ldsts, ("No more memory for allocation."));
        } else if (max_num_prefetch_ldsts == num_prefetch_ldsts + 1) {
          max_num_prefetch_ldsts *= 2;
          prefetch_ldsts = (WN **)realloc(
              prefetch_ldsts, max_num_prefetch_ldsts * sizeof(WN *));
          FmtAssert(prefetch_ldsts, ("No more memory for allocation."));
        }
    ...
    if (Write_ALIAS_CLASS_Map) {
      ...
      if (WN_MAP32_Get(WN_MAP_ALIAS_CLASS, node) != 0) {
        if (alias_classes == NULL) {
          ...
          alias_classes = (WN **)malloc(max_alias_class_nodes * sizeof(WN *));
          ...
          alias_classes = (WN **)realloc(alias_classes,
                                         max_alias_class_nodes * sizeof(WN **));
          FmtAssert(alias_classes != NULL, ("No more memory."));
        }
    ...
    if (Write_AC_INTERNAL_Map) {
      if (opr == OPR_ILOAD || opr == OPR_MLOAD || opr == OPR_PARM ||
      ...
      if (WN_MAP_Get(WN_MAP_AC_INTERNAL, node) != NULL) {
        if (ac_internals == NULL) {
          ...
          ac_internals = (WN **)malloc(max_ac_internal_nodes * sizeof(WN *));
          ...
          ac_internals = (WN **)realloc(ac_internals,
                                        max_ac_internal_nodes * sizeof(WN **));
          FmtAssert(ac_internals != NULL, ("No more memory."));
        }
  ...
  if (Get_ipa_tlog_phase() == PHASE_IPA && WN_operator(node) == OPR_REGION &&
    ...
    if (!cg_node->Is_PU_Write_Complete() && !cg_node->EHinfo_Updated()) {
      ...
      SUMMARY_SYMBOL *sym_array =
          IPA_get_symbol_file_array(cg_node->File_Header(), sym_size);
      ...
      for (; types; types = INITV_next(types)) {
        ...
        INITV_Set_VAL(Initv_Table[types],
                      Enter_tcon(Host_To_Targ(MTYPE_U4, new_idx)), 1);
...
#if defined(KEY) && !defined(FRONT_END) && !defined(IR_TOOLS)
        kid = ir_b_write_tree(WN_kid(node, i), base_offset, fl, off_map, pu);
#else
        kid = ir_b_write_tree(WN_kid(node, i), base_offset, fl, off_map);
#endif
...
template <class T> struct WRITE_TABLE_OP {
  Output_File *fl;
...
static Elf64_Word write_table(TABLE &fld, off_t base_offset, Output_File *fl) {
  off_t cur_offset =
      ir_b_align(fl->file_size, __ALIGNOF(typename TABLE::base_type), 0);
  fl->file_size =
      ir_b_align(fl->file_size, __ALIGNOF(typename TABLE::base_type), 0);
...
Elf64_Word ir_b_write_global_symtab(off_t base_offset, Output_File *fl) {
  ...
  gsymtab.header[i++].Init(cur_offset, sizeof(FILE_INFO), sizeof(FILE_INFO),
                           __ALIGNOF(FILE_INFO), SHDR_FILE);
  ...
  gsymtab.header[i++].Init(cur_offset, Ty_tab.Size() * sizeof(TY), sizeof(TY),
                           __ALIGNOF(TY), SHDR_TY);

  cur_offset = write_table(Pu_Table, symtab_offset, fl);
  gsymtab.header[i++].Init(cur_offset, Pu_Table.Size() * sizeof(PU), sizeof(PU),
                           __ALIGNOF(PU), SHDR_PU);
  ...
  gsymtab.header[i++].Init(cur_offset, Tylist_Table.Size() * sizeof(TYLIST),
  ...
  cur_offset =
      ir_b_save_buf(TCON_strtab_buffer(), TCON_strtab_size(), 1, 0, fl) -
      symtab_offset;
...
Elf64_Word ir_b_write_local_symtab(const SCOPE &pu, off_t base_offset,
                                   Output_File *fl) {
...
extern Elf64_Word ir_b_write_dst(DST_TYPE dst, off_t base_offset,
                                 Output_File *fl) {
  Elf64_Word cur_offset;
  DST_BLOCK_IDX i;
  ...
  FOREACH_DST_BLOCK(i) {
    ...
    cur_offset = ir_b_save_buf(dst_blocks[i].offset, dst_blocks[i].size,
                               __ALIGNOF(INT64), 0, fl);
    dst_blocks[i].offset = (char *)(cur_offset - base_offset);
  }
  FOREACH_DST_BLOCK(i) {
    cur_offset = ir_b_save_buf((char *)&dst_blocks[i], sizeof(block_header),
                               __ALIGNOF(block_header), 0, fl);
  }
  cur_offset = ir_b_save_buf((char *)&((DST_Type *)dst)->last_block_header,
                             sizeof(mINT32), __ALIGNOF(INT32), 0, fl);
  return cur_offset - base_offset;
}
...
void IPA_irb_write_mod_ref_info(Output_File *fl) {
  ...
  header_loc =
      (INT)ir_b_save_buf(&temp, sizeof(Elf64_Word), sizeof(INT64), 0, fl);

  // 0th entry, store the offset
  // ** NOTE **: Only the first copy should be aligned.
  // pu_idx
  offset_mod_ref = ir_b_save_buf(&Mod_Ref_Info_Table[0].pu_idx, sizeof(PU_IDX),
                                 sizeof(INT64), 0, fl);

  // mod/ref size
  ir_b_save_buf(&Mod_Ref_Info_Table[0].size, sizeof(mUINT32), 0, 0, fl);

  // mod
  ir_b_save_buf(Mod_Ref_Info_Table[0].mod, Mod_Ref_Info_Table[0].size,
                0, // alignment
                0, // padding
                fl);

  // ref
  ir_b_save_buf(Mod_Ref_Info_Table[0].ref, Mod_Ref_Info_Table[0].size,
                0, // alignment
                0, // padding
                fl);

  for (INT i = 1; i < size; i++) {
    ir_b_save_buf(&Mod_Ref_Info_Table[i].pu_idx, sizeof(PU_IDX), 0, 0, fl);

    ir_b_save_buf(&Mod_Ref_Info_Table[i].size, sizeof(mUINT32), 0, 0, fl);

    ir_b_save_buf(Mod_Ref_Info_Table[i].mod, Mod_Ref_Info_Table[i].size, 0, 0,
                  fl);

    ir_b_save_buf(Mod_Ref_Info_Table[i].ref, Mod_Ref_Info_Table[i].size, 0, 0,
                  fl);
  }
  ...
  offset = (INT)ir_b_save_buf(&header, sizeof(BE_SUMMARY_HEADER), sizeof(INT64),
                              0, fl);

  *(HEADER_ADDR(header_loc)) = offset - cur_sec_disp;
  BE_SUMMARY_HEADER *header_addr = (BE_SUMMARY_HEADER *)(fl->map_addr + offset);
