...
static char *rcs_id =
    "$Source: common/com/SCCS/s.dwarf_DST.cxx $ $Revision: 1.5 $";
...
static DST_INFO_IDX *DST_get_ptr_to_firstChildField(DST_INFO *parent) {
...
#ifdef KEY /* Bug 3507 */
  case DW_TAG_module:
    field_ptr = &DST_MODULE_first_child(DST_ATTR_IDX_TO_PTR(attr, DST_MODULE));
    break;
#endif /* KEY Bug 3507 */
  ...
  case DW_TAG_array_type:
    field_ptr =
        &DST_ARRAY_TYPE_first_child(DST_ATTR_IDX_TO_PTR(attr, DST_ARRAY_TYPE));
    break;
  ...
  case DW_TAG_union_type:
    field_ptr =
        &DST_UNION_TYPE_first_child(DST_ATTR_IDX_TO_PTR(attr, DST_UNION_TYPE));
    break;

  case DW_TAG_class_type:
    field_ptr =
        &DST_CLASS_TYPE_first_child(DST_ATTR_IDX_TO_PTR(attr, DST_CLASS_TYPE));
    break;
...
/*
#if !(defined(MONGOOSE_BE)) || defined(_STANDALONE_INLINER) ||
defined(_SUPPORT_IPA)
*/
...
static DST_INFO_IDX *DST_get_ptr_to_lastChildField(DST_INFO *parent) {
...
#ifdef KEY /* Bug 3507 */
  case DW_TAG_module:
    field_ptr = &DST_MODULE_last_child(DST_ATTR_IDX_TO_PTR(attr, DST_MODULE));
    break;
#endif /* KEY Bug 3507 */
  ...
  case DW_TAG_entry_point:
    field_ptr =
        &DST_ENTRY_POINT_last_child(DST_ATTR_IDX_TO_PTR(attr, DST_ENTRY_POINT));
    break;
  ...
  case DW_TAG_array_type:
    field_ptr =
        &DST_ARRAY_TYPE_last_child(DST_ATTR_IDX_TO_PTR(attr, DST_ARRAY_TYPE));
    break;
  ...
  case DW_TAG_union_type:
    field_ptr =
        &DST_UNION_TYPE_last_child(DST_ATTR_IDX_TO_PTR(attr, DST_UNION_TYPE));
    break;

  case DW_TAG_class_type:
    field_ptr =
        &DST_CLASS_TYPE_last_child(DST_ATTR_IDX_TO_PTR(attr, DST_CLASS_TYPE));
    break;
...
void DST_preorder_visit(DST_INFO_IDX i, INT32 init_val,
                        INT32 (*action)(INT32, DST_DW_tag, DST_flag,
                                        DST_ATTR_IDX, DST_INFO_IDX)) {
  DST_INFO *info;
  ...
  val = (*action)(init_val, DST_INFO_tag(info), DST_INFO_flag(info),
                  DST_INFO_attributes(info), i);

  /* Now, visit each child */
  idx_ptr = DST_get_ptr_to_firstChildField(info);
  if (idx_ptr != NULL) {
    for (idx = *idx_ptr; !DST_IS_NULL(idx);
