...
/*-------------------------------------------------------------*/
static WN_STACK *WN_InitStack(void) {
  WN_STACK *wns;
  ...
  WN_STACK_stack(wns) =
      (WN **)MEM_POOL_Alloc(Malloc_Mem_Pool, sizeof(WN *) * 128);
...
/* push an element                                             */
/*-------------------------------------------------------------*/
inline void WN_Push(WN *wn, WN_STACK *wns) {
  *(WN_STACK_sp(wns)++) = wn;
  if ((WN_STACK_sp(wns) - WN_STACK_stack(wns)) == WN_STACK_size(wns)) {
    ...
    WN_STACK_stack(wns) = (WN **)MEM_POOL_Realloc(
        Malloc_Mem_Pool, WN_STACK_stack(wns), size, size * 2);
...
extern WN_ITER *WN_WALK_SCFIter(WN *wn) {
  ...
  FmtAssert(WN_operator(wn) >= OPERATOR_FIRST &&
                WN_operator(wn) <= OPERATOR_LAST,
            ("Bad OPERATOR %d", WN_operator(wn)));
  FmtAssert(OPCODE_is_scf(WN_opcode(wn)),
            ("Expecting a Structured Control Flow tree node"));
...
/*-------------------------------------------------------------*/
extern WN_ITER *WN_WALK_TreeNext(WN_ITER *wni) {
  INT i;
  WN *wn1;
...
extern void WN_INSERT_BlockBefore(WN *blck, WN *wn, WN *in) {
  ...
  FmtAssert(WN_operator(blck) == OPR_BLOCK, ("Expecting a BLOCK operator"));
...
extern void WN_INSERT_BlockAfter(WN *blck, WN *wn, WN *in) {
  ...
  FmtAssert(WN_operator(blck) == OPR_BLOCK, ("Expecting a BLOCK"));
...
WN *WN_EXTRACT_ItemsFromBlock(WN *parent, WN *first_item, WN *last_item) {
...
#ifdef Is_True_On
  {
    ...
    for (tmp = WN_first(parent); tmp && tmp != first_item; tmp = WN_next(tmp))
      ;
    Is_True(tmp == first_item,
            ("WN_Extract_ItemsFromBlock: item not on parent's list"));
  }
#endif
...
/*-------------------------------------------------------------*/
static void wn_loop_get_st_ofst(const WN *var, ST_IDX &var_st,
...
static OPCODE wn_loop_reverse_compare(OPCODE comparison) {
  ...
  switch (compare_opr) {
  case OPR_LT:
    return OPCODE_make_op(OPR_GT, compare_rtype, compare_desc);
  case OPR_LE:
    return OPCODE_make_op(OPR_GE, compare_rtype, compare_desc);
  case OPR_EQ:
    return OPCODE_make_op(OPR_EQ, compare_rtype, compare_desc);
  case OPR_NE:
    return OPCODE_make_op(OPR_NE, compare_rtype, compare_desc);
  case OPR_GE:
    return OPCODE_make_op(OPR_LE, compare_rtype, compare_desc);
  case OPR_GT:
    return OPCODE_make_op(OPR_LT, compare_rtype, compare_desc);
  default:
    FmtAssert(FALSE, ("wn_loop_reverse_compare: not a comparison operator"));
    return (OPCODE)0;
  }
}
...
static WN *WN_CreateDivceil(TYPE_ID type, WN *kid0, WN *kid1) {
  ...
  for (i = 0; i < nkids; i++) {
    if (WN_operator(kids[i]) != OPR_PARM) {
      ...
      kids[i] =
          WN_CreateParm(type, kids[i], Be_Type_Tbl(type), WN_PARM_BY_VALUE);
    }
  }
  ...
  case MTYPE_U8:
    ...
    break;
  default:
    FmtAssert(FALSE, ("Cannot create DIVCEIL intrinsic of type %d\n", type));
  }
...
static WN *WN_CreateDivfloor(TYPE_ID type, WN *kid0, WN *kid1) {
  ...
  case MTYPE_U8:
    ...
    break;
  default:
    FmtAssert(FALSE, ("Cannot create DIVFLOOR intrinsic of type %d\n", type));
  }
...
static void Flip_Le_And_Ge(WN *wn) {
  ...
  case OPR_LT:
    ...
    break;
  default:
    FmtAssert(0, ("Bad call to Flip_Le_And_Ge"));
    break;
  }
...
static INT WN_Symbol_Count(WN *wn, const ST_IDX sym, const WN_OFFSET ofst) {
  INT rval = (WN_operator(wn) == OPR_LDID && sym == WN_st_idx(wn) &&
...
static BOOL WN_Solve_For(WN *wn_top, const ST_IDX sym, const WN_OFFSET ofst) {
  ...
  FmtAssert(opr_base == OPR_GT || opr_base == OPR_LT || opr_base == OPR_LE ||
                opr_base == OPR_GE,
            ("Solve_For() called with bad RELOP"));
  ...
  while (1) {
    ...
    Is_True((lcount == 1 && rcount == 0) || (lcount == 0 && rcount == 1),
    ...
    if (rcount) {
      if (lopr == OPR_SUB) {
        ...
        if (MTYPE_is_unsigned(type))
          type = MTYPE_complement(type);
#endif
        OPCODE negop = OPCODE_make_op(OPR_NEG, type, MTYPE_V);
        r = WN_CreateExp1(negop, r);
    ...
    if (lopr == OPR_MPY) {
      ...
      BOOL use_ceil =
          WN_operator(wn_top) == OPR_GE || WN_operator(wn_top) == OPR_LT;
      if (use_ceil)
...
/*-------------------------------------------------------------*/
#ifdef KEY
extern WN *WN_LOOP_UpperBound(const WN *loop, OPCODE *compare, BOOL enhanced)
#else
...
#endif
{
  ...
  if (!MTYPE_is_integral(WN_rtype(lb)) || !MTYPE_is_integral(WN_rtype(ub)) ||
      !MTYPE_is_integral(WN_rtype(incr)) || !MTYPE_is_integral(trip_mtype))
    return NULL;
...
#ifdef TARG_IA64
static TY_IDX field_type(const TY_IDX ty_idx, UINT field_id) {
  UINT cur_field_id = 0;
  FLD_HANDLE fld = FLD_get_to_field(ty_idx, field_id, cur_field_id);
  Is_True(!fld.Is_Null(),
          ("Invalid field id %d for type 0x%x", field_id, ty_idx));
  return FLD_type(fld);
}
#else
...
#else
  FLD_HANDLE fld = FLD_get_to_field(WN_ty(wn), WN_field_id(wn), cur_field_id);
#endif
  Is_True(!fld.Is_Null(),
          ("Invalid field id %d for type 0x%x", WN_field_id(wn), WN_ty(wn)));
  return FLD_type(fld);
}
#endif // TARG_IA64
...
#ifdef KEY
      if (WN_operator(wn) == OPR_ISTORE && WN_field_id(wn) != 0 &&
          TY_kind(pointed) == KIND_STRUCT && !TY_is_union(pointed) &&
          type_ok(pointed))
        return field_type(wn);
#endif
      return pointed;
...
void WN_hl_object_ty(const WN *wn, TY_IDX &ty, UINT32 &fld_id) {
  ty = (TY_IDX)0;
  fld_id = 0;
 ...
 *
 ***********************************************************************/
extern void Add_Pragma_To_MP_Regions(WN_VECTOR *wnv, WN_PRAGMA_ID pragma_id,
                                     ST *st, WN_OFFSET ofst, WN_MAP parent_map,
  ...
  case WN_PRAGMA_SHARED: {
    ...
    //
    for (WN_VECTOR::iterator wni = wnv->begin(); wni != wnv->end(); wni++) {

      WN *region_wn = *wni;

#ifdef BACK_END
      Is_True(WN_opcode(region_wn) == OPC_REGION && REGION_is_mp(region_wn),
      ...
      if (Pragma_is_Parallel_Region(pragma)) {
...
#ifdef Is_True_On
        {
          ...
          while (pwn) {
            ...
            if ((p_id == WN_PRAGMA_LOCAL || p_id == WN_PRAGMA_LASTLOCAL ||
                 p_id == WN_PRAGMA_FIRSTPRIVATE) &&
                WN_st(pwn) == st && WN_offset(pwn) == ofst) {
...
#endif
        {
          WN_MAP_Set(parent_map, shared_pwn,
                     (void *)WN_region_pragmas(region_wn));
        }
      }
    }
    break;
  ...
  case WN_PRAGMA_LOCAL: {
    ...
    for (WN_VECTOR::iterator wni = wnv->begin(); wni != wnv->end(); wni++) {
      ...
      if (need_pragma == TRUE && (Pragma_is_Parallel_Region(pragma) ||
        ...
        if (last && WN_opcode(last) == OPC_PRAGMA &&
...
#endif
        {
          WN_MAP_Set(parent_map, local_pwn,
                     (void *)WN_region_pragmas(region_wn));
        }
        need_pragma = FALSE;
      }
