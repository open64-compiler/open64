...
INT number_PU_Infos(PU_Info *pu_tree, INT num_PUs) {
  PU_Info *pu;
...
char *write_PU_fields(PU_Info *pu_tree, char *addr, INT32 *p_num_PUs) {
...
char *write_PU_subsect_info(PU_Info *pu_tree, INT32 k, char *addr) {
  PU_Info *pu;
  for (pu = pu_tree; pu; pu = PU_Info_next(pu)) {
...
void Save_Local_Symtab(SYMTAB_IDX level, PU_Info *pu) {
  ...
  FmtAssert(level >= 2, ("saving symtab at too high a level (%d)", level));
  ...
  SCOPE *scope =
      (SCOPE *)MEM_POOL_Alloc(Scope_tab[CURRENT_SYMTAB].pool, sizeof(SCOPE));
  *scope = Scope_tab[level];
  SAVED_SCOPE *saved_scope =
      CXX_NEW(SAVED_SCOPE(scope, level), Scope_tab[CURRENT_SYMTAB].pool);
...
static char *PU_Info_subsect_name[WT_SUBSECTIONS] = {
    "WT_SYMTAB",   "WT_TREE", "WT_DEPGRAPH",    "WT_PREFETCH",   "WT_REGIONS",
    "WT_FEEDBACK", "WT_FREQ", "WT_AC_INTERNAL", "WT_ALIAS_CLASS"};
