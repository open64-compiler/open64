...
void Check_TCON(TCON *tc) {
  ...
  case MTYPE_FQ:
    Is_True(TCON_v3(*tc) == 0, ("High order word of %s TCON non zero %x",
 ...
 * ====================================================================
 */
static void TCON_F4_To_TCON_F8(TCON *r, TCON *c) {
...
TCON Create_Simd_Const(TYPE_ID ty, TCON t) {
  TCON c;
...
TCON Create_Simd_Prog_Const(TYPE_ID ty, INT64 val) {
  TCON c;
  ...
  case MTYPE_V16I1:
    TCON_clear(c);
    TCON_ty(c) = ty;
    (c).vals.ival.v3 = ((val + 15) << 24) | ((val + 14) << 16) |
                       ((val + 13) << 8) | (val + 12);
    (c).vals.ival.v2 =
        ((val + 11) << 24) | ((val + 10) << 16) | ((val + 9) << 8) | (val + 8);
    (c).vals.ival.v1 =
        ((val + 7) << 24) | ((val + 6) << 16) | ((val + 5) << 8) | (val + 4);
    (c).vals.ival.v0 =
        ((val + 3) << 24) | ((val + 2) << 16) | ((val + 1) << 8) | val;
    break;
  case MTYPE_V16I2:
    TCON_clear(c);
    TCON_ty(c) = ty;
...
static TCON Extract_Paired_Hi(TCON v) {
  TCON c;
  TCON_clear(c);
...
static TCON Extract_Paired_Lo(TCON v) {
  TCON c;
  TCON_clear(c);
...
TCON Extract_Double_Hi(TCON v) { return Extract_Paired_Hi(v); }

TCON Extract_Double_Lo(TCON v) { return Extract_Paired_Lo(v); }

TCON Extract_LongLong_Hi(TCON v) { return Extract_Paired_Hi(v); }

TCON Extract_LongLong_Lo(TCON v) { return Extract_Paired_Lo(v); }
