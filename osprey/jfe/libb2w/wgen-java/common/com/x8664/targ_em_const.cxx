...
static void emit_bytes(FILE *fl, char *str, INT32 len) {
...
static void Emit_Repeated_Constant(FILE *fl,        /* file which to write */
...
void Targ_Emit_Const(FILE *fl,      /* File to which to write */
  ...
  while (rc) {
    ...
    case MTYPE_F8: {
      /* Emit doubles as two words, to match how the constants are
         handled in the code. */
      if (CG_emit_non_gas_syntax) {
        Emit_Repeated_Constant(fl, ".word", TCON_word0(Extract_Double_Lo(tc)),
                               1, 4);
        Emit_Repeated_Constant(fl, ".word", TCON_word0(Extract_Double_Hi(tc)),
                               1, 4);
      } else {
        Emit_Repeated_Constant(fl,
                               ((loc % 4) == 0 ? AS_WORD : AS_WORD_UNALIGNED),
    ...
    case MTYPE_V16F4: {
      ...
      fprintf(fl, "\t%s float[4] %#g %#g %#g %#g\n", ASM_CMNT, TCON_R4(tc),
              TCON_R4(tc), TCON_R4(tc), TCON_R4(tc));
      rc = 0;
      break;
    }
    case MTYPE_V16F8: {
    ...
    case MTYPE_V16I2:
      if (CG_emit_non_gas_syntax) {
        Emit_Repeated_Constant(fl, ".half", TCON_v0(tc) & 0xffff, rc, 8);
        Emit_Repeated_Constant(fl, ".half", (TCON_v0(tc) >> 16) & 0xffff, rc,
                               8);
        Emit_Repeated_Constant(fl, ".half", TCON_v1(tc) & 0xffff, rc, 8);
        Emit_Repeated_Constant(fl, ".half", (TCON_v1(tc) >> 16) & 0xffff, rc,
                               8);
        Emit_Repeated_Constant(fl, ".half", TCON_v2(tc) & 0xffff, rc, 8);
        Emit_Repeated_Constant(fl, ".half", (TCON_v2(tc) >> 16) & 0xffff, rc,
                               8);
        Emit_Repeated_Constant(fl, ".half", TCON_v3(tc) & 0xffff, rc, 8);
        Emit_Repeated_Constant(fl, ".half", (TCON_v3(tc) >> 16) & 0xffff, rc,
                               8);
      } else {
        Emit_Repeated_Constant(fl,
                               ((loc % 2) == 0 ? AS_HALF : AS_HALF_UNALIGNED),
    ...
    default:
      ErrMsg(EC_Inv_Mtype, Mtype_Name(TCON_ty(tc)), "Targ_Emit_Const");
    }
  }
} /* Targ_Emit_Const */
...
void Targ_Emit_EH_Const(FILE *fl,      /* File to which to write */
                        TCON tc,       /* Constant to be written */
                        BOOL add_null, /* whether to add null to strings */
                        INTSC rc,      /* Repeat count */
                        INTSC loc,  /* Location (i.e. offset from some base) */
                        INT format) /* Indicates encoding */
{
  ...
  while (rc) {
    ...
    case MTYPE_F8: {
      /* Emit doubles as two words, to match how the constants are
         handled in the code. */
      Emit_Repeated_Constant(fl, encoding, TCON_word0(Extract_Double_Lo(tc)), 1,
                             4);
      Emit_Repeated_Constant(fl, encoding, TCON_word0(Extract_Double_Hi(tc)), 1,
                             4);
...
void Em_Targ_Emit_Const(void *scn,     /* Section to which to write */
                        TCON tc,       /* Constant to be written */
                        BOOL add_null, /* whether to add null to strings */
                        INTSC rc)      /* Repeat count */
{
  ...
  case MTYPE_STRING:
    for (count = 0; count < rc; count++) {
      Em_Add_Bytes_To_Scn(section, Index_to_char_array(TCON_cp(tc)),
                          TCON_len(tc) + (add_null ? 1 : 0), 1);
    }
    break;
