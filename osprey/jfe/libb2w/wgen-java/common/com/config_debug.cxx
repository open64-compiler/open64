...
#ifdef _KEEP_RCS_ID
/*REFERENCED*/
static char *config_debug_rcs_id =
    "$Source: /proj/osprey/CVS/open64/osprey1.0/common/com/config_debug.cxx,v "
    "$ $Revision: 1.1.1.1 $";
#endif /* _KEEP_RCS_ID */
...
static DEBUG_FLAGS Default_DEBUG = {
    ...
    FALSE, /* fullwarn */
    FALSE,
    FALSE, /* full_iface_check */
    FALSE,
    FALSE, /* i32_oflow_check */
    FALSE,
    FALSE, /* int_oflow_check */
    FALSE,
    FALSE, /* optimize space */
    FALSE,
    FALSE, /* parm_alias_check */
    FALSE,
    FALSE, /* ptr_incr_warn */
    TRUE,
    FALSE, /* printf_warn */
    ...
    FALSE, /* shift_warn */
    FALSE,
    FALSE, /* subscript_check */
    FALSE,
    FALSE, /* trap_uv */
    FALSE,
    FALSE, /* trap_uv_rjustify */
    FALSE,
    FALSE, /* trunc_check */
    FALSE,
    FALSE, /* trunc_warn */
    TRUE,
    FALSE, /* varargs_iface_check */
    TRUE,
    FALSE, /* varargs_prototypes */
    FALSE,
    FALSE, /* verbose_runtime */
    NULL,
    FALSE, /* warning */
    NULL,
    FALSE,        /* woff */
...
static OPTION_DESC Options_DEBUG[] = {
    ...
    {OVK_BOOL, OV_INTERNAL, TRUE, "const_mod_warning", "", 0, 0, 0,
     &ID.cmod_warn, &ID.cmod_warn_set,
     "Warn about attempts to store into a constant"},

    {OVK_BOOL, OV_INTERNAL, TRUE, "cray_port", "", 0, 0, 0, &ID.cray_port,
     &ID.cray_port_set, "Turn on all Cray porting options"},

    {OVK_INT32, OV_VISIBLE, TRUE, "div_check", "", DEFAULT_DIV_CHECK, 0, 3,
     &ID.div_check, &ID.div_check_set,
     "Check for divide by zero and/or division overflow"},

    {OVK_BOOL, OV_INTERNAL, TRUE, "equiv_warning", "", 0, 0, 0, &ID.equiv_warn,
     &ID.equiv_warn_set,
     "Warn about EQUIVALENCE affected by 32- vs. 64-bit compilation"},

    {OVK_LIST, OV_VISIBLE, TRUE, "error", "", 0, 0, 0, &ID.error, &ID.error_set,
     "Convert given warning messages to errors"},

    {OVK_BOOL, OV_VISIBLE, TRUE, "fullwarn", "", 0, 0, 0, &ID.fullwarn,
     &ID.fullwarn_set, "Turn on all warning messages"},

    {OVK_BOOL, OV_SHY, TRUE, "full_interface_check", "", 0, 0, 0,
     &ID.full_iface_check, &ID.full_iface_check_set,
     "Check all subprogram call interfaces"},

    {OVK_BOOL, OV_INTERNAL, TRUE, "int32_overflow_check", "", 0, 0, 0,
     &ID.i32_oflow_check, &ID.i32_oflow_check_set,
     "Check 32-bit integer code for overflow"},

    {OVK_BOOL, OV_INTERNAL, TRUE, "int_overflow_check", "", 0, 0, 0,
     &ID.int_oflow_check, &ID.int_oflow_check_set,
     "Check all integer code for overflow"},

    {OVK_BOOL, OV_INTERNAL, TRUE, "optimize_space", "", 0, 0, 0,
     &ID.optimize_space, &ID.optimize_space_set, "Optimize debug space"},

    {OVK_BOOL, OV_INTERNAL, TRUE, "parameter_alias_check", "", 0, 0, 0,
     &ID.parm_alias_check, &ID.parm_alias_check_set,
     "Check Fortran subprogram parameters for aliasing"},

    {OVK_BOOL, OV_INTERNAL, TRUE, "pointer_increment_warning", "", 0, 0, 0,
     &ID.ptr_incr_warn, &ID.ptr_incr_warn_set,
     "Check increments of Fortran pointers against pointee size"},

    {OVK_BOOL, OV_INTERNAL, TRUE, "printf_warning", "", 0, 0, 0,
     &ID.printf_warn, &ID.printf_warn_set,
     "Check literal printf format strings against parameter types"},

    {OVK_LIST, OV_VISIBLE, TRUE, "remark", "", 0, 0, 0, &ID.remark,
     &ID.remark_set, "Convert given warning messages to remarks"},

    {OVK_BOOL, OV_INTERNAL, TRUE, "shift_check", "", 0, 0, 0, &ID.shift_check,
     &ID.shift_check_set, "Check for variable shift counts larger than object"},

    {OVK_BOOL, OV_INTERNAL, TRUE, "shift_warning", "", 0, 0, 0, &ID.shift_warn,
     &ID.shift_warn_set, "Warn about constant shift counts larger than object"},

    {OVK_BOOL, OV_VISIBLE, TRUE, "subscript_check", "", 0, 0, 0,
     &ID.subscript_check, &ID.subscript_check_set,
     "Check for subscripts out of range"},

    {OVK_LIST, OV_VISIBLE, TRUE, "suppress", "", 0, 0, 0, &ID.woff,
     &ID.woff_set, "Suppress given warning messages"},

    {OVK_BOOL, OV_VISIBLE, TRUE, "trap_uninitialized", "", 0, 0, 0, &ID.trap_uv,
     &ID.trap_uv_set, "Trap references to uninitialized variables"},

    {OVK_BOOL, OV_INTERNAL, TRUE, "trapuv_right_justify", "", 0, 0, 0,
     &ID.trap_uv_rjustify, &ID.trap_uv_rjustify_set, ""},

    {OVK_BOOL, OV_VISIBLE, TRUE, "zero_uninitialized", "", 0, 0, 0, &ID.zero_uv,
     &ID.zero_uv_set, "Set uninitialized variables to zero"},

    {OVK_BOOL, OV_INTERNAL, TRUE, "trunc_check", "", 0, 0, 0, &ID.trunc_check,
     &ID.trunc_check_set, "Check 64- to 32-bit assignments for truncation"},

    {OVK_BOOL, OV_INTERNAL, TRUE, "trunc_warning", "", 0, 0, 0, &ID.trunc_warn,
     &ID.trunc_warn_set, "Warn about truncation on 64- to 32-bit assignments"},

    {OVK_BOOL, OV_VISIBLE, TRUE, "varargs_interface_check", "", 0, 0, 0,
     &ID.varargs_iface_check, &ID.varargs_iface_check_set,
     "Check interfaces on calls to varargs routines"},

    {OVK_BOOL, OV_VISIBLE, TRUE, "varargs_prototypes", "", 0, 0, 0,
     &ID.varargs_prototypes, &ID.varargs_prototypes_set,
     "Assume ANSI guarantee that varargs routines are prototyped"},

    {OVK_BOOL, OV_VISIBLE, TRUE, "verbose_runtime", "", 0, 0, 0,
     &ID.verbose_runtime, &ID.verbose_runtime_set,
     "Generate verbose messages for runtime errors"},

    {OVK_LIST, OV_VISIBLE, TRUE, "warning", "", 0, 0, 0, &ID.warning,
     &ID.warning_set, "Convert given messages to warnings"},

    {OVK_LIST, OV_VISIBLE, TRUE, "woff", "", 0, 0, 0, &ID.woff, &ID.woff_set,
     "Suppress given warning messages"},

    {OVK_BOOL, OV_VISIBLE, TRUE, "ir_version_check", "", 0, 0, 0,
     &ID.ir_version_check, NULL,
     "Check whether version of IR matches expected version"},

    {OVK_NAME, OV_VISIBLE, TRUE, "alignment", "", 0, 0, 0,
     &Debug_Alignment_Name, NULL,
     "Do unaligned load/stores either as normal, compose, or fixade"},

    {OVK_BOOL, OV_VISIBLE, TRUE, "ipa_version_check", "", 0, 0, 0,
     &ID.ipalno_version_check, NULL,
     "Check whether version of IPALNO matches expected version"},

    {OVK_BOOL, OV_VISIBLE, TRUE, "conform_check", "", 0, 0, 0,
     &ID.conform_check, &ID.conform_check_set,
     "Check for conforming array assignments"},

    /* Obsolete/unsupported options: */
    {OVK_REPLACED, OV_INTERNAL, TRUE, "bounds_check", NULL, 0, 0, 0,
     const_cast<char *>("-DEBUG:subscript_check"), NULL,
     "Check array subscripts against bounds"},

    {OVK_REPLACED, OV_INTERNAL, TRUE, "trapuv", NULL, 0, 0, 0,
     const_cast<char *>("-DEBUG:trap_uninitialized"), NULL,
     "Trap references to uninitialized variables"},

    {OVK_REPLACED, OV_INTERNAL, TRUE, "zerouv", NULL, 0, 0, 0,
     const_cast<char *>("-DEBUG:zero_uninitialized"), NULL,
     "Set uninitialized variables to zero"},

#ifdef KEY
    {OVK_BOOL, OV_VISIBLE, FALSE, "eh_frame", "", 0, 0, 0, &ID.emit_ehframe,
     NULL,
...
void DEBUG_Push_Config(BOOL use_default) {
  DEBUG_FLAGS *new_flags = (DEBUG_FLAGS *)malloc(sizeof(DEBUG_FLAGS));
