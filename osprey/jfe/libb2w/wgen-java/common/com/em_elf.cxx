...
void Em_Change_Section_Alignment(pSCNINFO scn, Elf64_Word scn_align) {
  if (SCNINFO_align(scn) < scn_align)
    SCNINFO_align(scn) = scn_align;
}
...
*/
Elf64_Xword Em_Add_Bytes_To_Scn(pSCNINFO scninfo, const void *input_buf,
                                Elf64_Xword length, Elf64_Word align) {
  Elf64_Xword index;
  ...
  if (SCNINFO_align(scninfo) < align)
    SCNINFO_align(scninfo) = align;
 ...
 */
Elf64_Xword Em_Add_Zeros_To_Scn(pSCNINFO scninfo, Elf64_Word length,
                                Elf64_Word align) {
  Elf64_Xword index;
 ...
 */
Elf64_Xword Em_Add_Address_To_Scn(pSCNINFO scninfo, Elf64_Word symindex,
                                  Elf64_Sxword addend, Elf64_Word align) {
  Elf64_Xword index;
 ...
 */
Elf64_Xword Em_Add_Displacement_To_Scn(pSCNINFO scninfo, Elf64_Word symindex,
                                       Elf64_Sxword addend, Elf64_Word align) {
  Elf64_Xword index;
  INT32 value = addend;
...
static void print_elf_error(void) {
  ...
  err = elf_errno();
  if (err != 0)
    ErrMsg(EC_Elf_Error, elf_errmsg(err));
}
...
*/
static Elf_Scn *Create_New_Section(char *scn_name, Elf64_Word scn_type,
...
size_t Em_Get_Section_Index(pSCNINFO scn) { return SCNINFO_index(scn); }

/* For a given 'scn' return its current offset */
Elf64_Xword Em_Get_Section_Offset(pSCNINFO scn) { return SCNINFO_offset(scn); }
...
static void Create_New_Relocation_Section(BOOL is_rela, pSCNINFO scninfo) {
  ...
  if (is_rela) {
    newscn =
        Em_New_Section(relname, SHT_RELA, 0,
    ...
    newscn =
        Em_New_Section(relname, SHT_REL, 0,
...
void Em_Add_New_Rel(Elf64_Word symindex, unsigned char reltype,
                    Elf64_Addr reloffset, pSCNINFO scninfo) {
  if (symindex == 0) {
  ...
  if (Sixtyfour_Bit) {
    ...
    REL64_type2(reloc) = 0;
    REL64_type3(reloc) = 0;
    Em_Add_Bytes_To_Scn(SCNINFO_relinfo(scninfo), &reloc, sizeof(Elf64_Rel),
                        ELF64_FSZ_XWORD);
  } else {
    Elf32_Rel reloc;
    ...
    */
    ...
    Em_Add_Bytes_To_Scn(SCNINFO_relinfo(scninfo), &reloc, sizeof(Elf32_Rel),
                        ELF32_FSZ_WORD);
  }
}

/* Add a new relocation entry to the Rela section for 'scninfo'. */
void Em_Add_New_Rela(Elf64_Word symindex, unsigned char reltype,
                     Elf64_Addr reloffset, Elf64_Sxword addend,
                     pSCNINFO scninfo) {
  ...
  if (Sixtyfour_Bit) {
    ...
    Em_Add_Bytes_To_Scn(SCNINFO_relainfo(scninfo), &reloc, sizeof(Elf64_Rela),
                        ELF64_FSZ_XWORD);
  } else {
    Elf32_Rela reloc;
    ...
    Set_REL32_info(reloc, symindex, reltype);
    REL_addend(reloc) = addend;
    Em_Add_Bytes_To_Scn(SCNINFO_relainfo(scninfo), &reloc, sizeof(Elf32_Rela),
                        ELF32_FSZ_WORD);
  }
}
...
void Em_Add_New_Composite_Rela(Elf64_AltRela *preloc, pSCNINFO scninfo) {
  ...
  if (Sixtyfour_Bit) {
    Em_Add_Bytes_To_Scn(SCNINFO_relainfo(scninfo), preloc, sizeof(Elf64_Rela),
                        ELF64_FSZ_XWORD);
  } else {
    Elf32_Rela reloc;
...
*/
void Em_Define_Symbol(Elf64_Word symindex, Elf64_Addr symvalue,
                      Elf64_Xword symsize, pSCNINFO scninfo) {
  Elf64_Half scnindex;
...
Elf64_Word Em_Add_New_Symbol(char *symname, Elf64_Addr symvalue,
                             Elf64_Xword symsize, unsigned char symbind,
                             unsigned char symtype, unsigned char symother,
                             Elf64_Half symshndx) {
  Elf64_Word strindex;
  Elf64_Word symindex;
  ...
  if (Sixtyfour_Bit) {
    ...
    symindex = (Elf64_Word)Em_Add_Bytes_To_Scn(
        Symtab_Info, &sym64, sizeof(Elf64_Sym), ELF64_FSZ_XWORD);
    ...
    symindex = (Elf64_Word)Em_Add_Bytes_To_Scn(
        Symtab_Info, &sym32, sizeof(Elf32_Sym), ELF32_FSZ_WORD);
...
*/
Elf64_Word Em_Add_New_Common_Symbol(char *symname, Elf64_Xword symsize,
                                    unsigned char symbind,
                                    unsigned char symother) {
  return Em_Add_New_Symbol(symname, 0, symsize, symbind, STT_OBJECT, symother,
                           SHN_COMMON);
}
#endif /* MONGOOSE_BE */
...
*/
Elf64_Word Em_Add_New_Undef_Symbol(char *symname, unsigned char symbind,
                                   unsigned char symtype,
                                   unsigned char symother) {
  return Em_Add_New_Symbol(symname, 0, 0, symbind, symtype, symother,
                           SHN_UNDEF);
}
...
*/
Elf64_Word Em_Add_New_Weak_Symbol(char *weakname, unsigned char symtype,
                                  unsigned char symother, Elf64_Word symindex) {
  if (symindex == 0) {
  ...
  if (Sixtyfour_Bit) {
    ...
    return Em_Add_New_Symbol(weakname, symtable[symindex].st_value,
                             symtable[symindex].st_size, STB_WEAK, symtype,
                             symother, symtable[symindex].st_shndx);
  } else {
    Elf32_Sym *symtable;
...
void Generate_Addr_Reset(pSCNINFO scn, BOOL is_events, Elf64_Xword ev_ofst) {
   ...
   */
  SCNINFO_ev_offset(events_scn) =
      (Elf64_Word)Em_Add_Bytes_To_Scn(events_scn, &opcode, 1, 1);
  evscn_ofst = Em_Add_Bytes_To_Scn(events_scn, &short_ofst, 4, 1);
  Em_Add_New_Rel(Em_Create_Section_Symbol(scn), R_SCN_DISP, evscn_ofst,
                 events_scn);
...
void Em_Add_New_Interface(Elf64_Word length, Elf64_Byte *data) {
  if (Interface_Scn == NULL) {
    Interface_Scn = Em_New_Section(SECT_IFACE_NAME, SHT_IRIX_IFACE, SHF_NOSTRIP,
                                   ELF32_FSZ_WORD, ELF32_FSZ_WORD);
...
extern void Em_Add_Comment(char *s) {
  char *buff;
  if (Comment_Scn == NULL) {
    Comment_Scn = Em_New_Section(ELF_COMMENT, SHT_PROGBITS, 0, 0, sizeof(char));
  }
  if (strchr(s, ':') == NULL) {
...
*/
static void Create_Elf_Header(INT isa, BOOL old_abi, BOOL big_endian, BOOL pic,
  ...
  */
  ...
  Shstrtab_Info.scnptr =
      Create_New_Section(ELF_SHSTRTAB, SHT_STRTAB, SHF_ALLOC, sizeof(char));

  /* create symbol string tbl */
  Strtab_Info.scnptr =
      Create_New_Section(ELF_STRTAB, SHT_STRTAB, SHF_ALLOC, sizeof(char));
...
int Em_Begin_File(char *ofilename, BOOL update, BOOL elf64, BOOL old_abi,
                  INT isa, BOOL big_endian, BOOL pic, BOOL cpic, BOOL xgot,
                  BOOL gp_groups, BOOL elf_trace) {
  int ofiledes;
  Elf_Cmd cmd;
...
void Em_End_File(void) {
  ...
  if (Options_Scn != NULL)
    Em_End_Section(Options_Scn);
  /* write out the .comment data */
  if (Comment_Scn != NULL)
    Em_End_Section(Comment_Scn);
  /* write out the .interface data */
  if (Interface_Scn != NULL)
    Em_End_Section(Interface_Scn);

  if (elf_update(Elf_Ptr, ELF_C_WRITE) == -1 || elf_end(Elf_Ptr) == 0) {
    print_elf_error();
  }
}

pSCNINFO Em_New_Section(char *scnname, Elf64_Word scntype, Elf64_Xword scnflags,
                        Elf64_Xword scnentsize, Elf64_Xword scnalign) {
  Elf_Scn *scn;
  pSCNINFO scninfo;
...
Elf64_Word Em_Create_Section_Symbol(pSCNINFO scninfo) {
  ...
  if (symindex == 0) {
    symindex = Em_Add_New_Symbol(Em_Get_Section_Name(scninfo), (Elf64_Addr)0,
                                 (Elf64_Xword)0, STB_LOCAL, STT_SECTION, 0,
