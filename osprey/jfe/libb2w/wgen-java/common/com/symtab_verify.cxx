...
static void TCON_Verify(TCON tc) {
  ...
  switch (TCON_ty(tc)) {
  case MTYPE_STRING:
    Is_True(Targ_String_Address(tc) != NULL, ("Invalid TCON string pointer"));
    break;
  default:
    break;
  }
...
void ST_Verify_Class_Sclass(ST_CLASS sym_class, ST_SCLASS storage_class) {
  ...
  case CLASS_VAR:
    ...
    case SCLASS_THREAD_PRIVATE_FUNCS:
      break;
    default:
      Fail_FmtAssertion(msg, Sclass_Name(storage_class), Class_Name(sym_class));
      break;
    }
    break;
  case CLASS_FUNC:
    Is_True(storage_class == SCLASS_EXTERN || storage_class == SCLASS_TEXT,
            (msg, Sclass_Name(storage_class), Class_Name(sym_class)));
    break;
  case CLASS_CONST:
    Is_True(storage_class == SCLASS_FSTATIC || storage_class == SCLASS_EXTERN,
            (msg, Sclass_Name(storage_class), Class_Name(sym_class)));
    break;
  case CLASS_PREG:
  ...
  case CLASS_BLOCK:
    ...
    break;
  case CLASS_NAME:
    Is_True(storage_class == SCLASS_UNKNOWN || storage_class == SCLASS_COMMENT,
            (msg, Sclass_Name(storage_class), Class_Name(sym_class)));
    break;
  default:
    Fail_FmtAssertion(msg, Sclass_Name(storage_class), Class_Name(sym_class));
...
void ST_Verify_Sclass_Export(ST_SCLASS storage_class, ST_EXPORT export_class,
                             const ST *st) {
  ...
  case SCLASS_COMMON:
  case SCLASS_DGLOBAL:
    if (export_class == EXPORT_LOCAL || export_class == EXPORT_LOCAL_INTERNAL) {
...
static void ST_Verify_Flags(const ST &s) {
  ...
  if (ST_is_weak_symbol(s)) {
    ...
    if (ST_base_idx(s) != ST_st_idx(s))
      Is_True(
          ST_sclass(s) == SCLASS_EXTERN || ST_sclass(s) == SCLASS_TEXT ||
              ST_sclass(s) == SCLASS_DGLOBAL || ST_sclass(s) == SCLASS_UGLOBAL,
          (msg, "Storage class", Sclass_Name(ST_sclass(s)), "ST_WEAK_SYMBOL"));
  }
  ...
  if (ST_is_initialized(s)) {
    ...
    Is_True(
        ST_sclass(s) == SCLASS_PSTATIC || ST_sclass(s) == SCLASS_FSTATIC ||
            ST_sclass(s) == SCLASS_EXTERN || ST_sclass(s) == SCLASS_DGLOBAL ||
            ST_sclass(s) == SCLASS_UGLOBAL || ST_sclass(s) == SCLASS_CPLINIT ||
  ...
  if (ST_is_return_var(s))
    Is_True(
        ...
        (msg, "Storage class", Sclass_Name(ST_sclass(s)), "ST_IS_RETURN_VAR"));

  if (ST_is_value_parm(s))
    Is_True(
        ST_sclass(s) == SCLASS_FORMAL || ST_sclass(s) == SCLASS_FORMAL_REF,
        (msg, "Storage class", Sclass_Name(ST_sclass(s)), "ST_IS_VALUE_PARM"));
  ...
  if (ST_emit_symbol(s))
    Is_True(ST_sym_class(s) == CLASS_NAME || ST_sym_class(s) == CLASS_FUNC ||
                ST_sym_class(s) == CLASS_VAR,
            (msg, "Class", Class_Name(ST_sym_class(s)), "ST_EMIT_SYMBOL"));

  if (ST_has_nested_ref(s))
    Is_True(
        ST_sclass(s) == SCLASS_AUTO || ST_sclass(s) == SCLASS_FORMAL ||
            ST_sclass(s) == SCLASS_FORMAL_REF || ST_sclass(s) == SCLASS_PSTATIC,
        (msg, "Storage class", Sclass_Name(ST_sclass(s)), "ST_HAS_NESTED_REF"));

  if (ST_init_value_zero(s)) {
    Is_True(ST_sym_class(s) == CLASS_VAR,
            (msg, "Class", Class_Name(ST_sym_class(s)), "ST_INIT_VALUE_ZERO"));

    Is_True(ST_sclass(s) == SCLASS_EXTERN || ST_sclass(s) == SCLASS_UGLOBAL ||
  ...
  if (ST_gprel(s) || ST_not_gprel(s)) {
    ...
    Is_True(ST_sclass(s) != SCLASS_AUTO && ST_sclass(s) != SCLASS_FORMAL &&
  ...
  if (ST_is_optional_argument(s))
    Is_True(ST_sclass(s) == SCLASS_FORMAL_REF || ST_sclass(s) == SCLASS_FORMAL,
            (msg, "Storage class", Sclass_Name(ST_sclass(s)),
  ...
  if (ST_is_temp_var(s))
    Is_True(
        ST_sclass(s) == SCLASS_AUTO || ST_sclass(s) == SCLASS_FORMAL ||
            ST_sclass(s) == SCLASS_FORMAL_REF,
        (msg, "Storage class", Sclass_Name(ST_sclass(s)), "ST_IS_TEMP_VAR"));
  ...
  if (ST_is_const_var(s)) {
    ...
    Is_True(
        ST_sclass(s) != SCLASS_AUTO && ST_sclass(s) != SCLASS_FORMAL &&
            ST_sclass(s) != SCLASS_FORMAL_REF,
        (msg, "Storage class", Sclass_Name(ST_sclass(s)), "ST_IS_CONST_VAR"));
  }

  if (ST_pt_to_unique_mem(s)) {
    Is_True(ST_sym_class(s) == CLASS_VAR,
            (msg, "Class", Class_Name(ST_sym_class(s)), "ST_PT_TO_UNIQUE_MEM"));
  }
  ...
  if (ST_is_shared_auto(s)) {
    ...
    Is_True(
        ST_sclass(s) == SCLASS_AUTO,
        (msg, "Storage class", Sclass_Name(ST_sclass(s)), "ST_IS_SHARED_AUTO"));
  }

  Is_True(!(ST_is_weak_symbol(s) && ST_is_split_common(s)),
...
static void ST_Verify_Fields(const ST &s) {
  ...
  if (ST_sym_class(s) == CLASS_CONST) {
    Is_True(0 < ST_tcon(s) && ST_tcon(s) < TCON_Table_Size(), (msg, "tcon"));
  ...
  case CLASS_BLOCK:
    if (ST_blk(s) == 0)
      ...
      Is_True(ST_blk(s) > 0 && ST_blk(s) < Blk_Table.Size(), (msg, "name"));
    break;
  default:
  case CLASS_UNK:
    ...
    break;
  case CLASS_PREG:
    Is_True(((&s == Return_Val_Preg) ||
             (0 < TY_IDX_index(ST_type(s)) &&
  ...
  case CLASS_NAME:
    ...
    Is_True(
        (ST_sclass(s) == SCLASS_UNKNOWN || ST_sclass(s) == SCLASS_COMMENT) &&
  ...
  if (ST_base_idx(s) != ST_st_idx(s)) {
    ...
    if (ST_storage_class(*sb) != SCLASS_UNKNOWN) {
      ...
      Is_True(ST_export(sb) == EXPORT_LOCAL && ST_class(sb) == CLASS_BLOCK,
              (msg, "export scope, for based st with SCLASS_UNKNOWN should be "
                    "EXPORT_LOCAL and class should be CLASS_BLOCK"));
    }

    // Property 2: verify that ST_symclass of s and of sb are identical?
    if (ST_sym_class(s) == CLASS_BLOCK) {
      Is_True(ST_sym_class(s) == ST_sym_class(sb), (msg, "storage_class"));
    }
...
#endif
      if (!(ofst + ST_size(&s) <= ST_size(sb)) &&
          (ST_class(sb) != CLASS_BLOCK || !STB_is_basereg(sb)))
        DevWarn("ofst +  size of a ST (%s) should be less than size of its "
                "base (%s)",
                ST_name(s), ST_name(sb));
...
void INITO::Verify(UINT level) const {
  ...
  Is_True(ST_is_initialized(St_Table[st_idx]), ("ST_IS_INITIALIZED not set"));
  Is_True(0 < val && val < INITV_Table_Size(),
          ("Invalid field for INITO: val"));
  Is_True(level == ST_IDX_level(st_idx), ("INITO/st_idx level mismatch"));
#endif
}
...
#endif
  case INITVKIND_ZERO:
    Is_True(repeat1 == 0, (msg, "repeat1: should be 0"));
    Is_True(Repeat2() != 0, (msg, "repeat2: should be 0"));
    break;
  case INITVKIND_ONE:
    Is_True(repeat1 == 0, (msg, "repeat1: should be 0"));
    Is_True(Repeat2() != 0, (msg, "repeat2: should be 1"));
    break;
  case INITVKIND_VAL:
    Is_True(repeat1 == 0, (msg, "repeat1: should be 0"));
    break;
...
static void FLD_Verify_all(FLD_HANDLE fld_handle, UINT64 record_size) {
  ...
  Is_True(iter != last && FLD_last_field(iter), ("Missing last field in FLD"));
#endif
} // FLD_Verify_all
...
#ifdef Is_True_On
  Is_True(LABEL_name_idx(*this) < STR_Table_Size(), ("Invalid LABEL name"));
#endif // Is_True_On
} // LABEL::Verify()
...
#ifdef Is_True_On
  Is_True(PREG_name_idx(*this) < STR_Table_Size(), ("Invalid PREG name"));
#endif // Is_True_On
} // PREG::Verify()
...
#ifdef Is_True_On
  Is_True(level == ST_IDX_level(st_idx), ("ST_ATTR/st_idx level mismatch"));
  Is_True(ST_IDX_index(st_idx) > 0 &&
              ST_IDX_index(st_idx) < ST_Table_Size(ST_IDX_level(st_idx)),
...
void TY_Verify_Kind_Mtype(TY_KIND kind, mTYPE_ID mtype) {
  ...
  switch (kind) {
  case KIND_SCALAR:
    Is_True(mtype != MTYPE_UNKNOWN && mtype != MTYPE_V && mtype != MTYPE_A4 &&
                mtype != MTYPE_A8,
  ...
  case KIND_ARRAY:
    ...
    break;
  case KIND_STRUCT:
    Is_True(mtype == MTYPE_M, (msg, "For KIND_STRUCT, mtype must be M"));
    break;
  case KIND_POINTER: {
    TYPE_ID a_type =
        (Pointer_Size == MTYPE_byte_size(MTYPE_A4)) ? MTYPE_A4 : MTYPE_A8;
  ...
  case KIND_FUNCTION:
    ...
    break;
  case KIND_VOID:
    Is_True(mtype == MTYPE_V, (msg, "For KIND_VOID, mtype must be VOID"));
    break;
  default:
...
void TY::Verify(UINT) const {
  ...
  case KIND_POINTER:
    ...
    Is_True(u1.fld == 0, (msg, "non-zero TY::fld for KIND_POINTER"));
...
void PU::Verify(UINT) const {
  ...
  if (PU_is_mainpu(*this) || PU_has_return_address(*this))
    Is_True(PU_no_inline(*this),
            (msg, "Recursive or main pus and pus that contain return address, "
                  "should be marked no-inline"));
  ...
  if (PU_has_exc_scopes(*this))
    Is_True(
        PU_cxx_lang(*this) || PU_java_lang(*this), // czw
        (msg,
         "exception scopes can only be set for a C++ and java language pu"));
  ...
  if ((PU_c_lang(*this) && PU_ftn_lang(*this)) ||
    ...
    Fail_FmtAssertion(msg,
                      " PU src lang bits cannot be set to multiple languagess");
...
#ifdef Is_True_On
  Is_True(unused == 0, ("Invalid FILE_INFO field: unused"));
#endif // Is_True_On
} // FILE_INFO::Verify()
...
template <class T> struct verify_op {
  UINT level;
  verify_op(UINT lev) : level(lev) {}
  void operator()(UINT idx, T *entry) const;
};

template <class T> inline void verify_op<T>::operator()(UINT, T *entry) const {
  entry->Verify(level);
}

// specialization for verifying TCONs

template <> inline void verify_op<TCON>::operator()(UINT, TCON *tc) const {
  TCON_Verify(*tc);
}
