...
static INT Error_Return_Func(void) {
  if (verbose_info && last_node != NULL) {
    printf("last_node at %p, opcode = %s\n", last_node,
           OPCODE_name(WN_opcode(last_node)));
  }
  return ERROR_VALUE;
}
...
template <class Shdr>
static const Shdr *get_section_header(const Shdr *shdr, UINT n, Elf64_Word type,
                                      Elf64_Word info) {
  for (INT i = 1; i < n; ++i) {
...
get_section(void *handle, Elf64_Word type, Elf64_Word info) {
  ...
  if (eh->e_ident[EI_CLASS] == ELFCLASS32) {
    ...
    const Elf32_Shdr *shdr =
        (const Elf32_Shdr *)((char *)handle + ehdr->e_shoff);
...
#endif
  {
    const Elf64_Shdr *shdr = (const Elf64_Shdr *)((char *)handle + eh->e_shoff);
...
#define CONVERT_OFFSET(typ, fld) (fld) = (typ)(base + (INTPTR)(fld))
...
INT WN_get_global_symtab(void *handle) {
  ...
  for (i = 0; i < GLOBAL_SYMTAB_TABLES; ++i) {
    ...
    case SHDR_ST:
      Scope_tab[GLOBAL_SYMTAB].st_tab->Transfer((ST *)addr,
    ...
    case SHDR_INITO:
      Scope_tab[GLOBAL_SYMTAB].inito_tab->Transfer((INITO *)addr,
                                                   hdr.size / hdr.entsize);
      break;
    ...
    case SHDR_ST_ATTR:
      Scope_tab[GLOBAL_SYMTAB].st_attr_tab->Transfer((ST_ATTR *)addr,
                                                     hdr.size / hdr.entsize);
      break;
    }
  }
...
INT WN_get_symtab(void *handle, PU_Info *pu) {
  ...
  const char *base =
      (char *)handle + shdr.offset + PU_Info_subsect_offset(pu, WT_SYMTAB);
  Elf64_Word size = PU_Info_subsect_size(pu, WT_SYMTAB);
  const LOCAL_SYMTAB_HEADER_TABLE *lsymtab = (LOCAL_SYMTAB_HEADER_TABLE *)base;
  ...
  for (i = 0; i < LOCAL_SYMTAB_TABLES; ++i) {
    ...
    case SHDR_ST:
      ...
      Scope_tab[CURRENT_SYMTAB].st_tab->Transfer((ST *)addr,
                                                 hdr.size / hdr.entsize);
      break;
    ...
    case SHDR_LABEL:
      ...
      Scope_tab[CURRENT_SYMTAB].label_tab->Transfer((LABEL *)addr,
                                                    hdr.size / hdr.entsize);
      break;
    ...
    case SHDR_PREG:
      ...
      Scope_tab[CURRENT_SYMTAB].preg_tab->Transfer((PREG *)addr,
                                                   hdr.size / hdr.entsize);
      break;
    ...
    case SHDR_INITO:
      ...
      Scope_tab[CURRENT_SYMTAB].inito_tab->Transfer((INITO *)addr,
                                                    hdr.size / hdr.entsize);
      break;
    ...
    case SHDR_ST_ATTR:
      ...
      Scope_tab[CURRENT_SYMTAB].st_attr_tab->Transfer((ST_ATTR *)addr,
                                                      hdr.size / hdr.entsize);
      break;
    }
  }
...
void Set_Verbose_Info(BOOL val) { verbose_info = val; }

template <class ELF>
static INT check_elf_header(char *baseaddr, Elf64_Word size, const ELF &tag) {
...
static INT check_elf_header(char *baseaddr, Elf64_Word size) {
...
template <class ELF>
static INT check_section_headers(char *baseaddr, Elf64_Word size,
  ...
  for (INT i = 1; i < ehdr->e_shnum; i++) {
    if (shdr[i].sh_offset >= size || shdr[i].sh_offset + shdr[i].sh_size > size)
      return ERROR_RETURN;
...
#ifndef __ALWAYS_USE_64BIT_ELF__
  if (st == ELFCLASS32) {
    if ((st = check_section_headers(baseaddr, size, file_revision, ELF32())) <=
        0)
      return st;
  } else
#endif
  {
    if ((st = check_section_headers(baseaddr, size, file_revision, ELF64())) <=
        0)
      return st;
  }
...
static void *read_file(char *filename, off_t *mapped_size,
                       char *file_revision) {
  int fd;
  INT st;
...
Elf64_Word Get_Elf_Section_Size(void *handle, Elf64_Word type,
                                Elf64_Word info) {
...
void *WN_open_input(char *filename, off_t *mapped_size) {
  if (filename == 0) {
    errno = ENOENT;
    return (void *)(INTPTR)ERROR_RETURN;
  }
...
void *WN_get_depgraph(void *handle, PU_Info *pu) {
  ...
  tree_base =
      (char *)handle + shdr.offset + PU_Info_subsect_offset(pu, WT_TREE);
...
INT WN_get_prefetch(void *handle, PU_Info *pu) {
  ...
  for (;;) {
    ...
    if (pf_ptr->wn_pref_1L == (WN *)-1) {
      ...
      pf_ptr->wn_pref_1L = (WN *)(tree_base + (INTPTR)pf_ptr->wn_pref_1L);
    }

    if (pf_ptr->wn_pref_2L == (WN *)-1) {
      pf_ptr->wn_pref_2L = NULL;
    } else {
      pf_ptr->wn_pref_2L = (WN *)(tree_base + (INTPTR)pf_ptr->wn_pref_2L);
    }
...
static void WN_get_mod_ref_table(void *handle) {
  ...
  for (INT i = 0; i < header->size; i++) {
    ...
    Mod_Ref_Info_Table[index].mod =
        CXX_NEW_ARRAY(mUINT8, bv_size, Malloc_Mem_Pool);
    memcpy(Mod_Ref_Info_Table[index].mod, p, bv_size);
    p += bv_size;

    Mod_Ref_Info_Table[index].ref =
        CXX_NEW_ARRAY(mUINT8, bv_size, Malloc_Mem_Pool);
...
static inline void WN_MAP_put(WN_MAP wn_map, WN *wn, INT32 value) {
  WN_MAP32_Set(wn_map, wn, value);
}

static inline void WN_MAP_put(WN_MAP wn_map, WN *wn, INT64 value) {
  WN_MAP64_Set(wn_map, wn, value);
}

static inline void WN_MAP_put(WN_MAP wn_map, WN *wn, void *value) {
  WN_MAP_Set(wn_map, wn, value);
}

template <class MAP_ENTRY_TYPE>
static inline INT WN_read_generic_map(void *handle, PU_Info *pu,
  ...
  for (;;) {
    ...
    cur_addr = (char *)ir_b_align((off_t)cur_addr, sizeof(MAP_ENTRY_TYPE), 0);
...
INT WN_get_INT32_map(void *handle, PU_Info *pu, INT32 subsection_type,
                     WN_MAP value_map) {
  return WN_read_generic_map(handle, pu, subsection_type, value_map, (INT32)0);
}

INT WN_get_voidptr_map(void *handle, PU_Info *pu, INT32 subsection_type,
                       WN_MAP value_map) {
  return WN_read_generic_map(handle, pu, subsection_type, value_map, (void *)0);
}
...
static void open_specified_input(char *input_file, char **ir_input,
  ...
  *fhandle = WN_open_input(input_file, mapped_size);
  if (*fhandle == (void *)REVISION_MISMATCH) {
    ...
    ErrMsg(EC_IR_Revision,
           "abi of whirl file doesn't match abi from command-line", *ir_input);
...
void *Open_Input_Info(char *input_file) {
  open_specified_input(input_file, &global_ir_file, &global_fhandle,
                       &global_mapped_size);
...
void *Open_Global_Input(char *input_file) {
  open_specified_input(input_file, &global_ir_file, &global_fhandle,
                       &global_mapped_size);
  return global_fhandle;
}
...
void *Open_Local_Input(char *input_file) {
  open_specified_input(input_file, &local_ir_file, &local_fhandle,
                       &local_mapped_size);
...
PU_Info *Read_Global_Info(INT32 *p_num_PUs, bool input_o) // czw
{
  PU_Info *pu_tree;
...
void Read_Local_Info(MEM_POOL *pool, PU_Info *pu) {
  ...
  if (WN_get_INT32_map(local_fhandle, pu, WT_ALIAS_CLASS, WN_MAP_ALIAS_CLASS) ==
      -1) {
    ErrMsg(EC_IR_Scn_Read, "alias class map", local_ir_file);
  }

  if (WN_get_voidptr_map(local_fhandle, pu, WT_AC_INTERNAL,
                         WN_MAP_AC_INTERNAL) == -1) {
