...
static BOOL Array_Is_HFA(const TY_IDX ty, Mtype_Return_Level level,
                         TYPE_ID &hfa_mtype) {
  ...
  else if (TY_kind(ety) == KIND_ARRAY) {

    if (!Array_Is_HFA(ety, level, hfa_mtype))
      return FALSE;
  }

  else if (TY_kind(ety) == KIND_STRUCT) {
...
static BOOL Struct_Is_HFA(const TY_IDX ty, Mtype_Return_Level level,
                          TYPE_ID &hfa_mtype) {
  mUINT64 offset = 0;
  ...
  do {
    ...
    else if (TY_kind(fty) == KIND_ARRAY) {
...
static BOOL Struct_Has_Two_Floats(const TY_IDX ty, TYPE_ID &ftype1,
                                  TYPE_ID &ftype2) {
  if (TY_is_union(ty))
    return FALSE;
  ...
  if (FLD_last_field(fld2) && FLD_type(fld1) != 0 &&
      TY_kind(FLD_type(fld1)) == KIND_SCALAR && FLD_type(fld2) != 0 &&
      TY_kind(FLD_type(fld2)) == KIND_SCALAR && no_overlap(fld1, fld2)) {
...
static inline PREG_NUM Get_Current_Float_Preg_Num(Preg_Range pr) {
  PREG_NUM i;
...
static PLOC Get_Parameter_Location(TY_IDX ty, BOOL is_output) {
  ...
  case MTYPE_A4:
    if (Target_Byte_Sex == BIG_ENDIAN) {
      ...
      ploc.start_offset += (MTYPE_RegisterSize(SIM_INFO.int_type) - ploc.size);
    } else {
  ...
  case MTYPE_M: {
    ...
    if (RETURN_INFO_count(info) && MTYPE_float(RETURN_INFO_mtype(info, 0)) &&
...
struct PSTRUCT {
  ...
  PSTRUCT()
      : is_struct(FALSE), first_call(TRUE), is_hfa(FALSE), hfa_mtype(MTYPE_V),
        offset(0), size(0) {}
};
