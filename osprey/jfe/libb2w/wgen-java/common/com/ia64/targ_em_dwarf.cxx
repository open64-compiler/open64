...
translate_reloc32(Dwarf_Relocation_Data rentry,
                  ...
                  Dwarf_Ptr buffer, Dwarf_Unsigned bufsize) {
...
Dwarf_Ptr Em_Dwarf_Symbolic_Relocs_To_Elf(
    next_buffer_retriever get_buffer, next_bufsize_retriever get_bufsize,
    advancer_to_next_stream advance_stream, Dwarf_Signed buffer_scndx,
    Dwarf_Relocation_Data reloc_buf, Dwarf_Unsigned entry_count, int is_64bit,
    Cg_Dwarf_Sym_To_Elfsym_Ofst translate_symndx,
  ...
  for (i = 0; i < entry_count; ++i) {
	...
	fprintf(TFile, "translating pair (0x%lx) aimed at offset %llu\n",
        ...
        translate_symndx(reloc_buf->drd_symbol_index, &scn_sym1, &offset1);
        ++reloc_buf;
        ++i;
        translate_symndx(reloc_buf->drd_symbol_index, &scn_sym2, &offset2);
        if (scn_sym1 != scn_sym2) {
          fprintf(stderr, "ERROR: Cannot compute label difference "
                          "across sections");
          exit(-1);
        }
	  ...
	  fprintf(TFile, "buffer = 0x%lx, bufsize = %llu\n", buffer, bufsize);
        ...
        if (is_64bit) {
          ((UINT64_unaligned *)((char *)buffer + reloc_buf->drd_offset -
                                offset_offset))
              ->val += offset2 - offset1;
        } else {
          ((UINT32_unaligned *)((char *)buffer + reloc_buf->drd_offset -
                                offset_offset))
              ->val += offset2 - offset1;
        }
      }
	...
	fprintf(TFile, "translating simple (0x%lx) aimed at offset %llu\n",
      ...
      if (is_64bit) {
        *((Elf64_AltRel *)cur_reloc) =
            *translate_reloc64(reloc_buf, translate_symndx,
                               // DON'T TRY THIS AT HOME
                               (Dwarf_Ptr)((char *)buffer - offset_offset),
                               bufsize + offset_offset);
      } else {
        *((Elf32_Rel *)cur_reloc) =
            *translate_reloc32(reloc_buf, translate_symndx,
                               // DON'T TRY THIS AT HOME
                               (Dwarf_Ptr)((char *)buffer - offset_offset),
                               bufsize + offset_offset);
      }
