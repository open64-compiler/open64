...
static TCON Targ_Ipower(TCON base, UINT64 exp, BOOL neg_exp, BOOL *folded,
                        TYPE_ID btype);
...
#ifdef QUAD_PRECISION_SUPPORTED
static long double R16_To_RLD(QUAD_TYPE qt) {
  Quad_Representation repr;
...
static QUAD_TYPE RLD_To_R16(long double q) {
  Quad_Representation repr;
...
void Check_TCON(TCON *tc) {
  ...
  case MTYPE_F4:
    Is_True(TCON_v1(*tc) | TCON_v2(*tc) | TCON_v3(*tc) == 0,
            ("High order word of %s TCON non zero %x", Mtype_Name(TCON_ty(*tc)),
             TCON_v1(*tc)));
    break;
  case MTYPE_I8:
  case MTYPE_U8:
  ...
  case MTYPE_FQ: {
    ...
    */
    if (q.hi == 0.0 && q.lo != 0.0)
      ErrMsg(EC_Ill_Quad_Const, TCON_u0(*tc), TCON_u1(*tc), TCON_u2(*tc),
             TCON_u3(*tc));
  } break;
#endif
  default:
...
static TCON complex_sqrt(TCON c0) {
  ...
  case MTYPE_CQ: {
...
#endif
    qmag = __c_q_sqrt(
        __c_q_add(__c_q_mul(qi, qi, &err), __c_q_mul(qr, qr, &err), &err),
        &err);
    if (__c_q_eq(qmag, q0, &err)) {
      ...
      if (__c_q_lt(qi, q0, &err))
        rqi = __c_q_neg(rqi, &err);
...
static TCON complex_divide(TCON c0, TCON c1) {
  ...
  case MTYPE_CQ: {
    ...
    ar = c1r;
    ai = c1i;
    if (__c_q_lt(ar, q0, &err))
      ar = __c_q_neg(ar, &err);
    if (__c_q_lt(ai, q0, &err))
      ai = __c_q_neg(ai, &err);
    if (__c_q_lt(ar, ai, &err)) {
      t4 = __c_q_div(c1r, c1i, &err);
      t2 = __c_q_add(c1i, __c_q_mul(c1r, t4, &err), &err);
      t1 = __c_q_div(__c_q_sub(__c_q_mul(c0i, t4, &err), c0r, &err), t2, &err);
      t3 = __c_q_div(__c_q_add(__c_q_mul(c0r, t4, &err), c0i, &err), t2, &err);
    } else {
      t4 = __c_q_div(c1i, c1r, &err);
      t2 = __c_q_add(c1r, __c_q_mul(c1i, t4, &err), &err);
      t1 = __c_q_div(__c_q_sub(c0i, __c_q_mul(c0r, t4, &err), &err), t2, &err);
      t3 = __c_q_div(__c_q_add(__c_q_mul(c0i, t4, &err), c0r, &err), t2, &err);
    }
...
static TCON Targ_Convert_Length(TCON c0, /* The constant to be converted */
...
#define BOTH_OPNDS(op0, op1, type)                                             \
  if (type != MTYPE_U8) {                                                      \
    op0 = Targ_Conv(type, op0);                                                \
    op1 = Targ_Conv(type, op1);                                                \
  }                                                                            \
  TCON_ty(op0) = type
...
#undef DEBUG_FOLD
  ...
  printf("Folding %s on 0x%llx (%s), 0x%llx (%s), result", OPCODE_name(op),
         TCON_I8(c0), Mtype_Name(TCON_ty(c0)), TCON_I8(c1),
         Mtype_Name(TCON_ty(c1)));
#endif
  ...
  *folded = TRUE;
  ...
  if (OPERATOR_is_compare(opr)) {
    ...
    switch (desc) {
      // the following is for case "desc=MTYPE_B", it will only be "1" and
      // // "0", but there is no macro to access this content, so only need to
      // // access this with TCON_I4.
      // TCON_I4(c) is defined as TCON_v0(c) in head file.
    case MTYPE_B:
      switch (opr) {
      case OPR_EQ:
        TCON_I4(c0) = TCON_I4(c0) == TCON_I4(c1);
        break;
      case OPR_NE:
        TCON_I4(c0) = TCON_I4(c0) != TCON_I4(c1);
        break;
    ...
    case MTYPE_U4:
      switch (opr) {
      case OPR_EQ:
        TCON_I4(c0) = TCON_U4(c0) == TCON_U4(c1);
        break;
      case OPR_NE:
        TCON_I4(c0) = TCON_U4(c0) != TCON_U4(c1);
        break;
    ...
    case MTYPE_I8:
      switch (opr) {
      case OPR_EQ:
        TCON_v0(c0) = TCON_I8(c0) == TCON_I8(c1);
        break;
      case OPR_NE:
        TCON_v0(c0) = TCON_I8(c0) != TCON_I8(c1);
        break;
    ...
    case MTYPE_U8:
      switch (opr) {
      case OPR_EQ:
        TCON_v0(c0) = TCON_U8(c0) == TCON_U8(c1);
        break;
      case OPR_NE:
        TCON_v0(c0) = TCON_U8(c0) != TCON_U8(c1);
        break;
    ...
    case MTYPE_F4:
      switch (opr) {
      case OPR_EQ:
        TCON_v0(c0) = TCON_R4(c0) == TCON_R4(c1);
        break;
      case OPR_NE:
        TCON_v0(c0) = TCON_R4(c0) != TCON_R4(c1);
        break;
    ...
    case MTYPE_F8:
      switch (opr) {
      case OPR_EQ:
        TCON_v0(c0) = TCON_R8(c0) == TCON_R8(c1);
        break;
      case OPR_NE:
        TCON_v0(c0) = TCON_R8(c0) != TCON_R8(c1);
        break;
    ...
    case MTYPE_FQ:
      ...
      switch (opr) {
      case OPR_EQ:
        TCON_v0(c0) = __c_q_eq(q0, q1, &err);
        break;
      case OPR_NE:
        TCON_v0(c0) = __c_q_ne(q0, q1, &err);
        break;
      case OPR_LT:
        TCON_v0(c0) = __c_q_lt(q0, q1, &err);
        break;
      case OPR_LE:
        TCON_v0(c0) = __c_q_le(q0, q1, &err);
        break;
      case OPR_GT:
        TCON_v0(c0) = __c_q_gt(q0, q1, &err);
        break;
      case OPR_GE:
        TCON_v0(c0) = __c_q_ge(q0, q1, &err);
        break;
      }
      TCON_v1(c0) = 0;
      TCON_v2(c0) = 0;
    ...
    case MTYPE_C4:
      ...
      case OPR_EQ:
        TCON_v0(c0) =
            (TCON_R4(c0) == TCON_R4(c1)) && (TCON_IR4(c0) == TCON_IR4(c1));
        break;
      case OPR_NE:
        TCON_v0(c0) =
            (TCON_R4(c0) != TCON_R4(c1)) || (TCON_IR4(c0) != TCON_IR4(c1));
    ...
    case MTYPE_C8:
      ...
      case OPR_EQ:
        TCON_v0(c0) =
            (TCON_R8(c0) == TCON_R8(c1)) && (TCON_IR8(c0) == TCON_IR8(c1));
        break;
      case OPR_NE:
        TCON_v0(c0) =
            (TCON_R8(c0) != TCON_R8(c1)) || (TCON_IR8(c0) != TCON_IR8(c1));
    ...
    case MTYPE_C10:
      ...
      case OPR_EQ:
        TCON_v0(c0) =
            (TCON_R16(c0) == TCON_R16(c1) && TCON_IR16(c0) == TCON_IR16(c1));
        break;
      case OPR_NE:
        TCON_v0(c0) =
            (TCON_R16(c0) != TCON_R16(c1) || TCON_IR16(c0) != TCON_IR16(c1));
    ...
    */
    ...
    case OPC_U8ASHR:
      c0 = Targ_Conv(optype, c0);
      TCON_I8(c0) >>= ((TCON_I8U8I4U4(c1) >= 0) && (TCON_I8U8I4U4(c1) <= 63))
                          ? TCON_I8U8I4U4(c1)
                          : 63;
      break;
    case OPC_I4ASHR:
    case OPC_U4ASHR:
      c0 = Targ_Conv(optype, c0);
      TCON_I4(c0) >>= ((TCON_I8U8I4U4(c1) >= 0) && (TCON_I8U8I4U4(c1) <= 31))
                          ? TCON_I8U8I4U4(c1)
                          : 31;
      break;

    case OPC_F4F8CVT:
      Is_True(TCON_ty(c0) == MTYPE_F8,
              ("Illegal operand to %s", OPCODE_name(op)));
      c0 = Targ_Conv(MTYPE_F4, c0);
      break;
    case OPC_F8F4CVT:
      Is_True(TCON_ty(c0) == MTYPE_F4,
              ("Illegal operand to %s", OPCODE_name(op)));
...
#ifdef TARG_NEEDS_QUAD_OPS
    case OPC_I4FQRND:
      if (__c_q_ge(R16_To_RQ(TCON_R16(c0)), __c_q_flotj(0, &err), &err))
        TCON_R16(c0) = RQ_To_R16(
            __c_q_add(R16_To_RQ(TCON_R16(c0)), __c_q_ext(.5, &err), &err));
      else
        TCON_R16(c0) = RQ_To_R16(
            __c_q_sub(R16_To_RQ(TCON_R16(c0)), __c_q_ext(.5, &err), &err));
...
#ifdef TARG_NEEDS_QUAD_OPS
    case OPC_FQABS:
      if (__c_q_lt(R16_To_RQ(TCON_R16(c0)), __c_q_flotj(0, &err), &err))
    ...
    case OPC_FQADD:
      ...
      TCON_R16(c0) = RQ_To_R16(
          __c_q_add(R16_To_RQ(TCON_R16(c0)), R16_To_RQ(TCON_R16(c1)), &err));
    ...
    case OPC_CQADD:
      TCON_R16(c0) = RQ_To_R16(
          __c_q_add(R16_To_RQ(TCON_R16(c0)), R16_To_RQ(TCON_R16(c1)), &err));
      TCON_IR16(c0) = RQ_To_R16(
          __c_q_add(R16_To_RQ(TCON_IR16(c0)), R16_To_RQ(TCON_IR16(c1)), &err));
    ...
    case OPC_FQSUB:
      ...
      TCON_R16(c0) = RQ_To_R16(
          __c_q_sub(R16_To_RQ(TCON_R16(c0)), R16_To_RQ(TCON_R16(c1)), &err));
    ...
    case OPC_CQSUB:
      ...
      TCON_R16(c0) = RQ_To_R16(
          __c_q_sub(R16_To_RQ(TCON_R16(c0)), R16_To_RQ(TCON_R16(c1)), &err));
      TCON_IR16(c0) = RQ_To_R16(
          __c_q_sub(R16_To_RQ(TCON_IR16(c0)), R16_To_RQ(TCON_IR16(c1)), &err));
    ...
    case OPC_FQMPY:
      ...
      TCON_R16(c0) = RQ_To_R16(
          __c_q_mul(R16_To_RQ(TCON_R16(c0)), R16_To_RQ(TCON_R16(c1)), &err));
    ...
    case OPC_C10MPY:
      TCON_R16(t1) =
          TCON_R16(c0) * TCON_R16(c1) - TCON_IR16(c0) * TCON_IR16(c1);
      TCON_IR16(c0) =
          TCON_R16(c0) * TCON_IR16(c1) + TCON_IR16(c0) * TCON_R16(c1);
    ...
    case OPC_FQDIV:
      ...
      if (__c_q_eq(R16_To_RQ(TCON_R16(c1)), __c_q_flotj(0, &err), &err)) {
        /* divide by zero; so don't fold */
        /*	 ErrMsg( EC_Ill_Divide );*/
        *folded = FALSE;
        /* break; */
      ...
      TCON_R16(c0) = RQ_To_R16(
          __c_q_div(R16_To_RQ(TCON_R16(c0)), R16_To_RQ(TCON_R16(c1)), &err));
    ...
    case OPC_F4RECIP:
      c0 = Targ_WhirlOp(OPC_F4DIV, Host_To_Targ_Float(MTYPE_F4, 1.0), c0,
                        folded);
      break;
    case OPC_F8RECIP:
      c0 = Targ_WhirlOp(OPC_F8DIV, Host_To_Targ_Float(MTYPE_F8, 1.0), c0,
                        folded);
      break;
    case OPC_F10RECIP:
      c0 = Targ_WhirlOp(OPC_F10DIV, Host_To_Targ_Float(MTYPE_F10, 1.0), c0,
                        folded);
      break;
    case OPC_FQRECIP:
      c0 = Targ_WhirlOp(OPC_FQDIV, Host_To_Targ_Float(MTYPE_FQ, 1.0), c0,
                        folded);
      break;
    case OPC_C4RECIP:
      c0 = Targ_WhirlOp(OPC_C4DIV, Host_To_Targ_Float(MTYPE_C4, 1.0), c0,
                        folded);
      break;
    case OPC_C8RECIP:
      c0 = Targ_WhirlOp(OPC_C8DIV, Host_To_Targ_Float(MTYPE_C8, 1.0), c0,
                        folded);
      break;
    case OPC_C10RECIP:
      c0 = Targ_WhirlOp(OPC_C10DIV, Host_To_Targ_Float(MTYPE_C10, 1.0), c0,
                        folded);
      break;
    case OPC_CQRECIP:
      c0 = Targ_WhirlOp(OPC_CQDIV, Host_To_Targ_Float(MTYPE_CQ, 1.0), c0,
                        folded);
      break;
    ...
    case OPC_F4MIN:
      TCON_R4(c0) = TCON_R4(c0) < TCON_R4(c1) ? TCON_R4(c0) : TCON_R4(c1);
      break;
    case OPC_F4MAX:
      TCON_R4(c0) = TCON_R4(c0) < TCON_R4(c1) ? TCON_R4(c1) : TCON_R4(c0);
      break;
    case OPC_F8MIN:
      TCON_R8(c0) = TCON_R8(c0) < TCON_R8(c1) ? TCON_R8(c0) : TCON_R8(c1);
      break;
    case OPC_F8MAX:
      TCON_R8(c0) = TCON_R8(c0) < TCON_R8(c1) ? TCON_R8(c1) : TCON_R8(c0);
      break;
    case OPC_F10MIN:
      TCON_R16(c0) = TCON_R16(c0) < TCON_R16(c1) ? TCON_R16(c0) : TCON_R16(c1);
      break;
    case OPC_F10MAX:
      TCON_R16(c0) = TCON_R16(c0) < TCON_R16(c1) ? TCON_R16(c1) : TCON_R16(c0);
      break;
    ...
    case OPC_FQMIN:
      ...
      TCON_R16(c0) =
          __c_q_lt(R16_To_RQ(TCON_R16(c0)), R16_To_RQ(TCON_R16(c1)), &err)
              ? TCON_R16(c0)
              : TCON_R16(c1);
...
static BOOL Targ_Atoll(char *str, INT64 *i64, BOOL sgned) {
  ...
  while (*str) {
    ...
    if (valid && ++digits >= 19) {
      ...
      if ((sgned && mhi > INT32_MAX + (neg)) || (!sgned && mhi > UINT32_MAX)) {
        valid = FALSE;
      }
    }
    m = 10 * m + d;
...
TCON Targ_Atoc(TYPE_ID ty, char *str) {
  ...
  case MTYPE_I1:
    if (!Targ_Atoll(str, &m, TRUE) || m > SCHAR_MAX || m < SCHAR_MIN) {
  ...
  case MTYPE_I2:
    if (!Targ_Atoll(str, &m, TRUE) || m > INT16_MAX || m < INT16_MIN) {
      ErrMsg(EC_Large_Const, str);
    }
    TCON_v0(c) = m;
    TCON_v1(c) = 0;
  ...
  case MTYPE_I4:
    if (!Targ_Atoll(str, &m, TRUE) || m > INT32_MAX || m < INT32_MIN) {
      ErrMsg(EC_Large_Const, str);
    }
    TCON_v0(c) = m;
    TCON_v1(c) = 0;
  ...
  case MTYPE_U1:
    if (!Targ_Atoll(str, &m, FALSE) || (UINT64)m > UCHAR_MAX) {
      ErrMsg(EC_Large_Const, str);
    }
    TCON_u0(c) = (UINT8)m;
    TCON_u1(c) = 0;
  ...
  case MTYPE_U2:
    if (!Targ_Atoll(str, &m, FALSE) || (UINT64)m > UINT16_MAX) {
      ErrMsg(EC_Large_Const, str);
    }
    TCON_u0(c) = (UINT16)m;
    TCON_u1(c) = 0;
  ...
  case MTYPE_U4:
    if (!Targ_Atoll(str, &m, FALSE) || (UINT64)m > UINT32_MAX) {
      ErrMsg(EC_Large_Const, str);
    }
    TCON_u0(c) = (UINT32)m;
    TCON_u1(c) = 0;
...
char *Targ_Print(char *fmt, TCON c) {
  INT slen, i;
  char *bytes;
  INT spos;
...
TCON Host_To_Targ_Float(TYPE_ID ty, double v) {
  ...
  case MTYPE_FQ:
    TCON_clear(c);
    c = Targ_Conv(MTYPE_FQ, Host_To_Targ_Float(MTYPE_F8, v));
    TCON_ty(c) = ty;
    return c;
...
TCON Host_To_Targ_Float_4(TYPE_ID ty, float v) {
  ...
  case MTYPE_FQ:
    TCON_clear(c);
    c = Targ_Conv(MTYPE_FQ, Host_To_Targ_Float_4(MTYPE_F8, v));
    TCON_ty(c) = ty;
    return c;
...
INT fp_class_e(long double x) { return FP_QNAN; }
...
char *Targ_String_Address(TCON c) {
  Is_True(
      TCON_ty(c) == MTYPE_STRING,
      ("Bad type of const to Host_To_Targ_String: %s", Mtype_Name(TCON_ty(c))));
 ...
 */
TCON Str_To_Tcon(TYPE_ID ty, char *buf) {
  union {
...
BOOL Targ_Is_Integral(TCON tc, INT64 *iv) {
  ...
  default:
    ErrMsg(EC_Inv_Mtype, Mtype_Name(TCON_ty(tc)), "Targ_Is_Integral");
  }
  return FALSE;
} /* Targ_Is_Integral */
...
/**********************************************************************/
WN *Coerce_To_Integer(subtree) WN *subtree;
{
  ...
  if (Is_Const(subtree, &operand)) {
    ...
    case MTYPE_FQ:
      ...
      if (__c_q_eq(R16_To_RQ(TCON_R16(converted)), R16_To_RQ(TCON_R16(operand)),
                   &err)) {
        TCON_ty(truncated) = MTYPE_I4;
        coerced_subtree = Make_Const(truncated);
      }
    ...
    default:
      ErrMsg(EC_Inv_Mtype, Mtype_Name(TCON_ty(operand)), "Coerce_To_Integer");
      break;
    }
  }
...
BOOL Targ_Is_Zero(TCON t) {
  ...
  case MTYPE_F8:
    /* make sure is not -0.0 (sign-bit set);
     * -0.0 will == 0.0, so add check for sign bit. */
    return (TCON_R8(t) == 0.0 && (TCON_v0(t) | TCON_v1(t)) == 0);
  case MTYPE_F10:
    /* make sure is not -0.0 (sign-bit set);
     * -0.0 will == 0.0, so add check for sign bit. */
  ...
  case MTYPE_FQ: {
    ...
    return (__c_q_eq(R16_To_RQ(TCON_R16(t)), __c_q_extd(0.0, &dummy_err),
                     &dummy_err));
  }
#endif
 ...
 *   Targ_IntrinsicOp
 *
 *  fold intrinsics. Arguments are essentially the same as Targ_Op and
 * Targ_WhirlOp
 *
 */
...
#define RAD_TO_DEGQ (180.0l / M_PIL)
  ...
  case INTRN_FQSIGN:
    ...
    if (TCON_I4(t))
      c0 = Targ_WhirlOp(OPC_FQNEG, c0, c0, folded);
    break;
  ...
  case INTRN_FQATAND:
    ...
    break;
  case INTRN_FQATAN2:
    TCON_R16(c0) = RLD_To_R16(
        atan2l(R16_To_RLD(TCON_R16(c0)), R16_To_RLD(TCON_R16(c[1]))));
    break;
  case INTRN_FQATAN2D:
    TCON_R16(c0) = RLD_To_R16(RAD_TO_DEGQ * atan2l(R16_To_RLD(TCON_R16(c0)),
                                                   R16_To_RLD(TCON_R16(c[1]))));
    break;
