...
void Count_WN_Operator(OPERATOR opr, TYPE_ID rtype, INT32 &bbs, INT32 &stmts,
                       INT32 &calls) {
  ...
  if (OPERATOR_is_non_scf(opr)) {
     ...
     * but can't tell just from opcode */
             ...
             OPERATOR_is_expression(opr) && !OPERATOR_is_load(opr) &&
...
void Print_PU_Stats(void) {
  ...
  if (Get_Trace(TKIND_INFO, TINFO_STATS) == 0)
    return;
