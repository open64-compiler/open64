...
BOOL Similar_BE_Types(TY_IDX t1_idx, TY_IDX t2_idx) {
  ...
  case KIND_SCALAR:
  case KIND_POINTER:
    return TY_mtype(t1) == TY_mtype(t2) && TY_size(t1) == TY_size(t2);
...
BOOL Equivalent_Types(TY_IDX t1, TY_IDX t2, QUAL_CHECK consider_qualifiers) {
  ...
  BOOL match_q = (consider_qualifiers == QUAL_IGNORE ||
  ...
  case KIND_SCALAR:
    return (TY_mtype(ty1) == TY_mtype(ty2) && TY_size(ty1) == TY_size(ty2) &&
            match_q);

  case KIND_POINTER:
    return match_q && Equivalent_Types(TY_pointed(ty1), TY_pointed(ty2),
                                       consider_qualifiers);

  case KIND_FUNCTION:
    return match_q &&
           Equivalent_Types(Tylist_Table[TY_tylist(ty1)],
                            Tylist_Table[TY_tylist(ty2)], consider_qualifiers);

  case KIND_ARRAY:
    return (
        match_q &&
        Equivalent_Types(TY_etype(ty1), TY_etype(ty2), consider_qualifiers) &&
        ARB_are_equivalent(TY_arb(ty1), TY_arb(ty2)));

  case KIND_STRUCT:
#ifdef KEY
    match_q = (TY_return_in_mem(t1) == TY_return_in_mem(t2) &&
               TY_copy_constructor(t1) == TY_copy_constructor(t2) && match_q);
#endif
