...
WN *WN_Simplify_Tree(WN *t, ALIAS_MANAGER *alias_manager) {
  ...
  if (op == OPC_BLOCK) {
    ...
    r = WN_SimplifyIload(op, WN_load_offset(t), WN_ty(t), WN_field_id(t),
                         WN_load_addr_ty(t), k0);
    if (r) {
#ifdef BACK_END
      if (alias_manager) {
    ...
    r = WN_SimplifyIstore(op, WN_load_offset(t), WN_ty(t), WN_field_id(t), k0,
                          k1);
    if (r) {
#ifdef BACK_END
      if (alias_manager) {
...
/* Assume that all children are already simplified, rebuild a tree using the new
 * children. This only applies itself to a subset of expression nodes.
 */
WN *WN_Simplify_Rebuild_Expr_Tree(WN *t, ALIAS_MANAGER *alias_manager) {
  ...
  if (opr == OPR_ILOAD) {
    k0 = WN_kid0(t);
#if defined(KEY) && defined(Is_True_On)
    if (Enable_WN_Simp_Expr_Limit == -1 ||
        (Enable_WN_Simp_Expr_Limit != -1 &&
         cur_idx < Enable_WN_Simp_Expr_Limit))
#endif
      r = WN_SimplifyIload(op, WN_load_offset(t), WN_ty(t), WN_field_id(t),
                           WN_load_addr_ty(t), k0);
    if (r) {
...
static simpnode WN_SimpCreateExp3(OPCODE opc, simpnode k0, simpnode k1,
                                  simpnode k2) {
  simpnode wn;
...
static simpnode WN_SimpCreateExtract(OPCODE opc, INT16 boffset, INT16 bsize,
                                     simpnode k0) {
  simpnode wn;
  wn = WN_Create(opc, 1);
  WN_kid0(wn) = k0;
...
static simpnode WN_SimpCreateDeposit(OPCODE opc, INT16 boffset, INT16 bsize,
