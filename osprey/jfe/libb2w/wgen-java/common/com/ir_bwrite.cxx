...
static void ir_bwrite_signal_handler(int sig, int err_num) {
  ...
  if (Doing_mmapped_io && errno == 0)
    Fatal_Error("I/O error in %s: %s",
                Current_Output ? Current_Output->file_name : "mmapped object",
                err_str);
  switch (sig) {
  case SIGBUS:
...
static Section *get_section(Elf64_Word sh_info, char *name, Output_File *fl) {
  ...
  if (fl->num_of_section == 0) {
    ...
    fl->section_list = (Section *)realloc(
        fl->section_list, fl->max_num_of_section * sizeof(Section));
    FmtAssert(fl->section_list, ("No more memory"));
  }
...
static void write_output(UINT64 e_shoff,
  ...
  ehdr->e_flags =
      Config_ELF_From_Target(!Use_32_Bit_Pointers, FALSE, (INT)Target_ISA);
  ...
  if ((INTPTR)str + strlen(ELF_SHSTRTAB) + 1 > e_shoff + (INTPTR)base_addr)
...
Output_File *WN_open_output(char *file_name) {
  ...
  if (old_sigsegv == 0)
    old_sigsegv = signal(
        SIGSEGV, reinterpret_cast<void (*)(int)>(ir_bwrite_signal_handler));

  if (old_sigbus == 0)
    old_sigbus = signal(
        SIGBUS, reinterpret_cast<void (*)(int)>(ir_bwrite_signal_handler));
...
#else
  fl->file_size = Use_32_Bit_Pointers ? sizeof(Elf32_Ehdr) : sizeof(Elf64_Ehdr);
#endif
...
void WN_write_tree(PU_Info *pu, WN_MAP off_map, Output_File *fl) {
  ...
  *(Elf64_Word *)(fl->map_addr + tree_base) = this_tree;
  ...
  PU_Info_subsect_offset(pu, WT_TREE) = tree_base - cur_section->shdr.sh_offset;
...
extern "C" void WN_write_strtab(const void *strtab, UINT64 size,
                                Output_File *fl) {
...
template <class T>
void write_profile(off_t base, const T &data, Output_File *fl, mUINT32 &num,
                   mUINT32 &offset) {
  offset = ir_b_save_buf(&(data.front()),
...
template <class T>
void write_target_profile(off_t base, T &data, Output_File *fl, mUINT32 &num,
                          mUINT32 &offset, mUINT32 &target_offset) {
  num = data.size();
  fl->file_size = ir_b_align(fl->file_size, sizeof(mINT64), 0);
...
void WN_write_feedback(PU_Info *pu, Output_File *fl) {
  ...
  pu_hdr.pu_checksum =
      Convert_Feedback_Info(Cur_PU_Feedback, PU_Info_tree_ptr(pu), pu_handle);
#ifdef KEY
  pu_hdr.pu_size = 0;
  ...
  write_target_profile(feedback_base, pu_handle.Get_Switch_Table(), fl,
                       pu_hdr.pu_num_switch_entries, pu_hdr.pu_switch_offset,
  ...
  write_profile(feedback_base, pu_handle.Get_Loop_Table(), fl,
                pu_hdr.pu_num_loop_entries, pu_hdr.pu_loop_offset);

  write_profile(feedback_base, pu_handle.Get_Short_Circuit_Table(), fl,
                pu_hdr.pu_num_scircuit_entries, pu_hdr.pu_scircuit_offset);

  write_profile(feedback_base, pu_handle.Get_Call_Table(), fl,
                pu_hdr.pu_num_call_entries, pu_hdr.pu_call_offset);

#ifdef KEY
  write_profile(feedback_base, pu_handle.Get_Icall_Table(), fl,
                pu_hdr.pu_num_icall_entries, pu_hdr.pu_icall_offset);

  write_profile(feedback_base, pu_handle.Get_Value_Table(), fl,
                pu_hdr.pu_num_value_entries, pu_hdr.pu_value_offset);
...
void IPA_write_summary(void (*IPA_irb_write_summary)(Output_File *),
                       Output_File *fl) {
  Section *cur_section;
...
static inline MAP_ENTRY_TYPE WN_MAP_retrieve(WN_MAP, WN *, MAP_ENTRY_TYPE);

static inline INT32 WN_MAP_retrieve(WN_MAP wn_map, WN *wn, INT32) {
  return WN_MAP32_Get(wn_map, wn);
}

static inline INT64 WN_MAP_retrieve(WN_MAP wn_map, WN *wn, INT64) {
  return WN_MAP64_Get(wn_map, wn);
}

static inline void *WN_MAP_retrieve(WN_MAP wn_map, WN *wn, void *) {
...
template <class MAP_ENTRY_TYPE>
static inline void
WN_write_generic_map(PU_Info *pu, WN_MAP off_map, Output_File *fl,
                     INT32 subsection_type, WN_MAP value_map,
                     const char *subsection_name, MAP_ENTRY_TYPE) {
  ...
  for (i = 0; nodes_represented[i]; i++) {
    ...
    MAP_ENTRY_TYPE map_value =
        WN_MAP_retrieve(value_map, node, (MAP_ENTRY_TYPE)0);
...
void WN_write_INT32_map(PU_Info *pu, WN_MAP off_map, Output_File *fl,
                        INT32 subsection_type, WN_MAP value_map,
...
void WN_write_voidptr_map(PU_Info *pu, WN_MAP off_map, Output_File *fl,
                          INT32 subsection_type, WN_MAP value_map,
                          const char *subsection_name) {
  WN_write_generic_map(pu, off_map, fl, subsection_type, value_map,
...
void IPA_copy_PU(PU_Info *pu, char *section_base, Output_File *outfile) {
  ...
  ir_b_save_buf(subsect, PU_Info_subsect_size(pu, WT_SYMTAB), sizeof(mINT64), 0,
                outfile);
  ...
  padding = (char *)dummy - (char *)WN_StartAddress(dummy) + sizeof(Elf64_Word);
...
#ifdef linux
extern "C" void WN_write_elf_symtab(const void *symtab, UINT64 size,
                                    UINT64 entsize, UINT align,
                                    Output_File *fl) {
