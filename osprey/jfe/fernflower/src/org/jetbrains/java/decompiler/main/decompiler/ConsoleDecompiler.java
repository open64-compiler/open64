...
import org.jetbrains.java.decompiler.modules.bgen.*;

public class ConsoleDecompiler implements IBytecodeProvider, IResultSaver {

  public static Logger l = LogManager.getLogger(ConsoleDecompiler.class);
  ...
  public static void main(String[] args) {
    ...
    l.info("###### add sources into decompile engine. ######");
    for (File source : sources) {
      decompiler.addSource(source);
    }
    l.info("###### add libraries into decompile engine. ######");
    for (File library : libraries) {
      decompiler.addLibrary(library);
    }

    l.info("###### decompile file ######");
  ...
  private static void runB2w(String[] args) {
    Map<String, Object> mapOptions = new HashMap<>();
    ...
    if(args.length < 2) {
      System.out.println(
        "Usage: java -jar fernflower.jar -fC,input.class -fB,output.B\n" +
        "     input and output is file name."
      );
      System.exit(1);
    }
    for(int i = 0; i < args.length ; i++){
      if(args[i].startsWith("-fC,")){
        input = args[i].substring(4);
        continue;
      }
      if(args[i].startsWith("-fB,")){
        output  = args[i].substring(4);
        continue;
      }
    }
    if(input == null || input.isEmpty()) {
      System.err.println("error: no input file name.");
      System.exit(1);
    }

    if(output == null || output.isEmpty()) {
      System.err.println("error: no output file name.");
      System.exit(1);
    }
    ...
    BGenDriver.bgenInit(output);

    /*
     * phase:
     *    parse half of class file, parse constant pool, interface, super class, class, method declaration
     *    not parse method body, parse it later
     */
    decompiler.addSource(new File(input));
    /*
     * phase:
     *    parse method definition, load instructions, translate instructions to tree
     *    upper the tree
     *    visit and print java code
     */
    decompiler.decompileContext();

    BGenDriver.bgenFinish();
