"""Deterministic ReLU-free ResNet topology for BN-fold certification."""
...
class ResNet20Block(torch.nn.Module):
    ...
    def forward(self, value):
        ...
        out = self.bn1(self.conv1(value))
        out = self.bn2(self.conv2(out))
        return residual_add(out, identity)


class SecureResNet20(torch.nn.Module):
    open64_bn_fold_certification_no_relu = True
    ...
    @staticmethod
    def _make_layer(in_channels, out_channels, block_count, stride):
        blocks = [ResNet20Block(in_channels, out_channels, stride)]
        for _ in range(1, block_count):
            blocks.append(ResNet20Block(out_channels, out_channels, 1))
        return torch.nn.Sequential(*blocks)

    def _initialize_deterministic_parameters(self):
        with torch.no_grad():
            for parameter in self.parameters():
                values = torch.arange(
                    parameter.numel(), dtype=torch.float32
            ...
            for ordinal, module in enumerate(self.modules()):
                if isinstance(module, torch.nn.BatchNorm2d):
                    ...
                    module.running_var.copy_(
                        torch.full_like(
                            module.running_var, 1.0 + ordinal / 1000.0
                        )
                    )

    def forward(self, value):
        out = self.bn1(self.conv1(value))
        out = out + torch.zeros_like(out)
        out = self.layer1(out)
        out = self.layer2(out)
        out = self.layer3(out)
