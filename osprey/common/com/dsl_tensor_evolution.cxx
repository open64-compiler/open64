        ...
        (const DSL_TENSOR_EVOLUTION_GRAPH *graph, FILE *diagnostic)
{
    ...
    if (!graph->edges.empty())
        return DSL_Tensor_Evolution_Report
                   (diagnostic, "AIO-1 graph contains evolution edges", 0);

    for (UINT32 i = 0; i < graph->nodes.size(); ++i) {
        const DSL_TENSOR_EVOLUTION_NODE_RECORD &node = graph->nodes[i];
        DSL_IR_VALUE_RECORD value;
        if (node.id != i + 1 ||
            node.kind != DSL_TENSOR_EVOLUTION_NODE_SEMANTIC ||
            node.owner_pu_st != graph->owner_pu_st ||
            node.semantic_root_id != node.id || node.flags != 0 ||
            node.reserved != 0 ||
        ...
        for (UINT32 j = 0; j < i; ++j) {
            if (graph->nodes[j].semantic_value_id == node.semantic_value_id)
                return DSL_Tensor_Evolution_Report
                           (diagnostic, "duplicate semantic root", node.id);
        }
    }
    return TRUE;
}
        ...
        (FILE *file, const DSL_TENSOR_EVOLUTION_GRAPH *graph)
{
    ...
    for (UINT32 i = 0; i < graph->nodes.size(); ++i) {
        ...
        fprintf(file,
                ...
                "dtype=%s shape=%s semantic_root=%u flags=0x%x\n",
                ...
                node.semantic_root_id, node.flags);
    }
}
