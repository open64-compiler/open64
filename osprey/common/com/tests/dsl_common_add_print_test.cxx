...
Create_Common_Add_Test_Tree(WN **dsl_marker_out)
{
    ...
    DSL_FIXTURE_VALUE const_i32_2x2_zero =
	DSL_Fixture_Create_Tensor_Const ("const_i32_2x2_zero",
					 "tensor_i32_2x2_zero",
					 "int32",
					 2,
					 "[2,2]",
					 "row_major",
					 "0");
    DSL_FIXTURE_VALUE const_i32_2x2_one =
	DSL_Fixture_Create_Tensor_Const ("const_i32_2x2_one",
					 "tensor_i32_2x2_one",
					 "int32",
					 2,
					 "[2,2]",
					 "row_major",
					 "1");
    WN *dsl_marker = DSL_Fixture_Create_Common_Add (&const_i32_2x2_zero,
						    &const_i32_2x2_one);

    if (dsl_marker_out != NULL)
        *dsl_marker_out = dsl_marker;

    WN_INSERT_BlockLast(block, const_i32_2x2_zero.value);
    WN_INSERT_BlockLast(block, const_i32_2x2_one.value);
...
static int
Check_Native_Ingestion_Fixture_Value(const DSL_FIXTURE_VALUE *fixture)
{
    TENSOR_DESCRIPTOR_RECORD descriptor;
    int failed = 0;

    if (fixture == NULL) {
	fprintf(stderr, "native ingestion fixture value is null\n");
	return 1;
    }

    if (!TY_get_tensor_descriptor_record(fixture->ty, &descriptor)) {
	fprintf(stderr, "native ingestion fixture descriptor missing\n");
	return 1;
    }

    if (TY_kind(fixture->ty) != KIND_TENSOR) {
	fprintf(stderr, "native ingestion fixture tensor type is not KIND_TENSOR\n");
	failed = 1;
    }

    if (descriptor.dtype == 0 ||
	strcmp(&Str_Table[descriptor.dtype], "int32") != 0 ||
	descriptor.logical_shape == 0 ||
	strcmp(&Str_Table[descriptor.logical_shape], "[2,2]") != 0 ||
	descriptor.layout == 0 ||
	strcmp(&Str_Table[descriptor.layout], "row_major") != 0 ||
	descriptor.rank != 2) {
	fprintf(stderr, "native ingestion fixture descriptor changed\n");
	failed = 1;
    }

    if (!ST_tensor_metadata_is_bound(fixture->st,
				     TY_TENSOR_SCHEMA_SOURCE_LAYER_NAME) ||
	strcmp(ST_tensor_metadata(fixture->st,
				  TY_TENSOR_SCHEMA_SOURCE_LAYER_NAME),
	       "dsl_ingestion_fixture") != 0 ||
	!ST_tensor_metadata_is_bound(fixture->st,
				     TY_TENSOR_SCHEMA_LOWERING_HINT) ||
	strcmp(ST_tensor_metadata(fixture->st,
				  TY_TENSOR_SCHEMA_LOWERING_HINT),
	       "native_fixture") != 0) {
	fprintf(stderr, "native ingestion fixture compiler metadata changed\n");
	failed = 1;
    }
...
Check_Common_Add_On_Tensor_Constants(void)
{
    ...
    DSL_FIXTURE_VALUE const_i32_2x2_one =
	DSL_Fixture_Create_Tensor_Const ("const_i32_2x2_one",
					 "tensor_i32_2x2_one_check",
					 "int32",
					 2,
					 "[2,2]",
					 "row_major",
					 "1");
    WN *common_add = DSL_Fixture_Create_Common_Add (&const_i32_2x2_zero,
						    &const_i32_2x2_one);
    DSL_OPCODE_ANNOTATION annotation;
    FILE *dump = tmpfile();
    char *text;
    ...
    failed |= Check_Native_Ingestion_Fixture_Value(&const_i32_2x2_zero);
    failed |= Check_Native_Ingestion_Fixture_Value(&const_i32_2x2_one);
    failed |= Check_Tensor_Const_Annotation(const_i32_2x2_zero.value,
					   "name=const_i32_2x2_zero",
					  "value=0");
    failed |= Check_Tensor_Const_Annotation(const_i32_2x2_one.value,
...
Check_Tensor_Dsl_Symtab_Print(void)
{
    ...
    char *text;
    TENSOR_DESCRIPTOR_RECORD descriptor;
    TENSOR_DESCRIPTOR_RECORD missing_descriptor;
    int failed = 0;
    int saw_dtype = 0;
    int saw_shape = 0;
    int saw_layout = 0;
    ...
    TENSOR_DESCRIPTOR_RECORD_Init(&descriptor);
    if (!TY_get_tensor_descriptor_record(tensor_ty, &descriptor)) {
        fprintf(stderr, "tensor descriptor record probe failed\n");
        failed = 1;
    } else if (descriptor.descriptor_id != TY_IDX_index(tensor_ty) ||
	     descriptor.type_core_id != descriptor.descriptor_id ||
	     descriptor.trait_set_id != descriptor.descriptor_id ||
	     descriptor.representation_id != descriptor.descriptor_id ||
	     descriptor.lineage_id != descriptor.descriptor_id ||
	     descriptor.ty != tensor_ty ||
	     descriptor.element_ty != MTYPE_To_TY(MTYPE_I4) ||
	     descriptor.rank != 2 ||
	     descriptor.attribute_count != TY_tensor_attribute_count(tensor_ty)) {
        fprintf(stderr, "tensor descriptor record fixed fields changed\n");
        failed = 1;
    } else if (descriptor.dtype == 0 ||
	     strcmp(&Str_Table[descriptor.dtype], "int32") != 0 ||
	     descriptor.logical_shape == 0 ||
	     strcmp(&Str_Table[descriptor.logical_shape], "[2,2]") != 0 ||
	     descriptor.layout == 0 ||
	     strcmp(&Str_Table[descriptor.layout], "row_major") != 0 ||
	     descriptor.lineage != 0 ||
	     descriptor.sharding != 0) {
        fprintf(stderr, "tensor descriptor record slot projection changed\n");
        failed = 1;
    }

    TENSOR_DESCRIPTOR_RECORD_Init(&missing_descriptor);
    if (TY_get_tensor_descriptor_record(TY_IDX_ZERO, &missing_descriptor) ||
            missing_descriptor.descriptor_id != TENSOR_DESCRIPTOR_INVALID_ID ||
            missing_descriptor.rank != -1 ||
            missing_descriptor.dtype != 0) {
        fprintf(stderr, "invalid tensor descriptor record probe changed\n");
        failed = 1;
    }
    ...
    if (strstr(text, "DSL Tensor Type Extensions:") == NULL ||
        ...
        strstr(text, "TensorDescriptorIR view:") == NULL ||
        strstr(text, "TensorTypeCore:") == NULL ||
        strstr(text, "kind = <missing>") == NULL ||
        strstr(text, "dtype = int32") == NULL ||
        strstr(text, "logical_shape = [2,2]") == NULL ||
        strstr(text, "TensorTraitSet:") == NULL ||
        strstr(text, "traits = <missing>") == NULL ||
        strstr(text, "TensorRepresentationDescriptor:") == NULL ||
        strstr(text, "shape = [2,2]") == NULL ||
        strstr(text, "layout = row_major") == NULL ||
        strstr(text, "sharding = <missing>") == NULL ||
        strstr(text, "TensorLineageMetadata:") == NULL ||
        strstr(text, "lineage = <missing>") == NULL) {
...
Check_DSL_Opcode_Registry(void)
{
    ...
    const UINT32 wrapper_seed_count = 8;
    DSL_DOMAIN_ID common_id;
    DSL_DOMAIN_ID cnn_id;
    DSL_DOMAIN_ID transformer_id;
    ...
    if (strstr(text, "DSL Opcode Registry: entries=69") == NULL ||
