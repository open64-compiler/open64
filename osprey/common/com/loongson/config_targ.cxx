...
Targ_Name ( TARGET_PROCESSOR b)
{
  ...
  switch ( b ) {
    case TARGET_LOONGSON:	return "Loongson";
	default:
      r = bnb[bnb_used].name;
      bnb_used = (bnb_used + 1) % 4;
...
Prepare_Target ( void )
{
  ...
  if ( ABI_Name != NULL ) {
    if ( strcmp ( ABI_Name, "n32" ) == 0 ) {
      Target_ABI = ABI_n32;
      isa_default = TARGET_ISA_LOONGSON;		
      targ_default = TARGET_LOONGSON;
  ...
  if ( ISA_Name != NULL ) {
    ...
    if ( strcasecmp ( ISA_Name, "loongson" ) == 0 ) {
      isa = TARGET_ISA_LOONGSON;
      targ_default = TARGET_LOONGSON;
    } else
    {
  ...
  if ( Processor_Name != NULL ) {
    ...
    if ( strcasecmp ( Processor_Name, "loongson" ) == 0 ) {
      targ = TARGET_LOONGSON;
    }else {
   ...
   */
  switch ( Target ) {
    case TARGET_LOONGSON:
	if (Target_ABI == ABI_UNDEF) Target_ABI = ABI_n32;
	Target_ISA = TARGET_ISA_LOONGSON;
	Target = TARGET_LOONGSON;
	break;
    case TARGET_UNDEF:
      Target = targ_default;
